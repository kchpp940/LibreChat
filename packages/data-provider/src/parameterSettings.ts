...
const settingKeyToCapabilityKey: Record<string, keyof import('./schemas').TModelCapability> = {
  ...
  maxOutputTokens: 'supports_max_tokens',
  maxTokens: 'supports_max_tokens',
  reasoning_effort: 'reasoning_effort',
  thinking: 'thinking',
  thinkingBudget: 'thinking',
  thinkingLevel: 'thinking',
  effort: 'anthropic_effort',
  thinkingDisplay: 'anthropic_thinking',
  promptCache: 'prompt_caching',
  imageDetail: 'image_detail',
  resendFiles: 'resend_files',
  promptPrefix: 'prompt_prefix',
  web_search: 'web_search',
  disableStreaming: 'streaming',
};

export function filterSettingsByCapability(
  settings: SettingsConfiguration,
  capability: import('./schemas').TModelCapability | null,
): SettingsConfiguration {
  if (!capability) {
    return settings;
  }
  return settings.filter((setting) => {
    const capKey = settingKeyToCapabilityKey[setting.key];
    if (!capKey) {
      return true;
    }
    return capability[capKey] === true;
  });
}
