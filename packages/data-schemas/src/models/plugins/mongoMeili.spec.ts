...
describe('Meilisearch Mongoose plugin', () => {
  ...
  describe('Legacy mixed pagination and degraded mode', () => {
    beforeEach(() => {
      mockSearch.mockClear();
    });

    test('meiliSearch returns degraded=true when contentTypes filter is active and legacy docs exist', async () => {
      const messageModel = createMessageModel(mongoose) as unknown as SchemaWithMeiliMethods;
      await messageModel.deleteMany({});

      const msgId1 = new mongoose.Types.ObjectId().toString();
      const msgId2 = new mongoose.Types.ObjectId().toString();
      ...
      mockSearch.mockResolvedValueOnce({
        hits: [
          { messageId: msgId1, user: 'user-1', text: 'tool output result' },
          { messageId: msgId2, user: 'user-1', text: 'plain text message' },
        ],
        estimatedTotalHits: 2,
      });

      const result = (await messageModel.meiliSearch(
        'tool',
        { filter: `user = "user-1"`, contentTypes: ['tool_call'] } as Record<string, unknown>,
        true,
      )) as Record<string, unknown>;

      expect(result.degraded).toBe(true);
      expect((result.indexingStatus as Record<string, unknown>)?.hasLegacyDocs).toBe(true);
      expect((result.indexingStatus as Record<string, unknown>)?.needsReindex).toBe(true);
      expect((result.hits as unknown[]).length).toBe(2);
    });

    test('meiliSearch returns degraded=false when contentTypes filter is active but no legacy docs', async () => {
      const messageModel = createMessageModel(mongoose) as unknown as SchemaWithMeiliMethods;
      await messageModel.deleteMany({});
      ...
      mockSearch.mockResolvedValueOnce({
        hits: [
          { messageId: msgId1, user: 'user-1', text: 'tool output result', contentTypes: ['tool_call'] },
          { messageId: msgId2, user: 'user-1', text: 'plain text message', contentTypes: ['text'] },
        ],
        estimatedTotalHits: 2,
      });
      ...
      expect((result.indexingStatus as Record<string, unknown>)?.hasLegacyDocs).toBe(false);
      expect((result.hits as unknown[]).length).toBe(1);
      const firstHit = (result.hits as Record<string, unknown>[])[0];
      expect(firstHit.messageId).toBe(msgId1);
    });

    test('first page with non-matching legacy docs still returns all results in degraded mode', async () => {
      const messageModel = createMessageModel(mongoose) as unknown as SchemaWithMeiliMethods;
      await messageModel.deleteMany({});

      const legacyMsgId1 = new mongoose.Types.ObjectId().toString();
      const legacyMsgId2 = new mongoose.Types.ObjectId().toString();

      await messageModel.collection.insertMany([
        {
          messageId: legacyMsgId1,
          conversationId: new mongoose.Types.ObjectId().toString(),
          user: 'user-1',
          isCreatedByUser: true,
          text: 'user question about weather',
          _meiliIndex: true,
          expiredAt: null,
        },
        {
          messageId: legacyMsgId2,
          conversationId: new mongoose.Types.ObjectId().toString(),
          user: 'user-1',
          isCreatedByUser: true,
      ...
      mockSearch.mockResolvedValueOnce({
        hits: [
          { messageId: legacyMsgId1, user: 'user-1', text: 'user question about weather' },
          { messageId: legacyMsgId2, user: 'user-1', text: 'user question about code' },
        ],
        estimatedTotalHits: 2,
      });
      ...
      const hitTypes = (result.hits as Record<string, unknown>[]).map(
        (h) => h.contentTypes as string[],
      );
      expect(hitTypes.every((types) => !types.includes('tool_call'))).toBe(true);
    });

    test('no contentTypes filter does not set degraded even with legacy docs', async () => {
      const messageModel = createMessageModel(mongoose) as unknown as SchemaWithMeiliMethods;
      await messageModel.deleteMany({});
      ...
      await messageModel.collection.insertOne({
        ...
        user: 'user-1',
        isCreatedByUser: true,
        text: 'legacy message without contentTypes',
        _meiliIndex: true,
        expiredAt: null,
      });

      mockSearch.mockResolvedValueOnce({
        hits: [{ messageId: msgId, user: 'user-1', text: 'legacy message without contentTypes' }],
    ...
    test('meiliSearch passes combined filter with contentTypes and legacy fallback to MeiliSearch', async () => {
      const messageModel = createMessageModel(mongoose) as unknown as SchemaWithMeiliMethods;
      ...
      await messageModel.meiliSearch(
        'test query',
        { filter: `user = "user-1"`, contentTypes: ['error', 'file'] } as Record<string, unknown>,
        false,
      );
      ...
      expect(mockSearch).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          filter: expect.stringContaining("contentTypes = 'error'"),
        }),
      );

      const callArgs = mockSearch.mock.calls[0][1] as { filter: string };
      expect(callArgs.filter).toContain("contentTypes = 'file'");
      expect(callArgs.filter).toContain('contentTypes IS EMPTY OR contentTypes NOT EXISTS');
      expect(callArgs.filter).toContain('user = "user-1"');
    });

    test('meiliSearch with contentTypes filter removes contentTypes from search params', async () => {
      const messageModel = createMessageModel(mongoose) as unknown as SchemaWithMeiliMethods;
      ...
      await messageModel.meiliSearch(
        'test',
        { filter: `user = "user-1"`, contentTypes: ['artifact'] } as Record<string, unknown>,
        false,
      );

      const callArgs = mockSearch.mock.calls[0][1] as Record<string, unknown>;
      expect(callArgs.contentTypes).toBeUndefined();
    });
  });
});
