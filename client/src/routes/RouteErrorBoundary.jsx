...
export default function RouteErrorBoundary() {
    ...
    const typedError = useRouteError();
    const errorDetails = {
    ...
    const handleDownloadLogs = async () => {
        try {
            ...
            const errorLog = {
                ...
                error: {
                    ...errorDetails,
                    stack: errorDetails.stack != null && errorDetails.stack.trim() !== ''
    ...
    return (<div role="alert" className="flex min-h-screen flex-col items-center justify-center bg-surface-primary bg-gradient-to-br">
      <div className="bg-surface-primary/60 mx-4 w-11/12 max-w-4xl rounded-2xl border border-border-light p-8 shadow-2xl backdrop-blur-xl">
        ...
        {(typeof errorDetails.status === 'number' ||
            typeof errorDetails.statusText === 'string') && (<div className="mb-4 rounded-xl border border-yellow-500/20 bg-yellow-500/5 p-4 text-sm text-text-primary">
        ...
        {errorDetails.stack != null && errorDetails.stack.trim() !== '' && (<details className="group mb-4 rounded-xl border border-border-light p-4">
            <summary className="mb-2 flex cursor-pointer items-center justify-between text-sm font-medium text-text-primary">
              ...
              <div className="flex items-center">
                <Button variant="outline" size="sm" onClick={handleCopyStack} className="ml-2 px-2 py-1 text-xs" aria-label={localize('com_ui_copy_stack_trace')}>
                  {localize('com_ui_copy')}
                </Button>
              </div>
            </summary>
            <div className="overflow-x-auto rounded-lg bg-black/5 p-4 dark:bg-white/5">
              {formatStackTrace(errorDetails.stack).map(({ number, content }) => (<div key={number} className="flex">
        ...
        {errorDetails.data != null && (<details className="group mb-4 rounded-xl border border-border-light p-4">
            <summary className="mb-2 flex cursor-pointer items-center justify-between text-sm font-medium text-text-primary">
        ...
        <div className="mt-6 flex flex-col gap-4">
          ...
          <div className="mt-4 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            <Button variant="submit" onClick={() => window.location.reload()} className="w-full sm:w-auto" aria-label={localize('com_ui_refresh_page')}>
              {localize('com_ui_refresh_page')}
            </Button>
            <Button variant="outline" onClick={handleDownloadLogs} className="w-full sm:w-auto" aria-label={localize('com_ui_download_error_logs')}>
