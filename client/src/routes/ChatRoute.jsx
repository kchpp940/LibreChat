...
import { mergeQuerySettingsWithSpec, processValidSettings, getDefaultModelSpec, getModelSpecPreset, isNotFoundError, isTemporaryConversation, logger, clearMessagesCache, } from '~/utils';
import { useGetConvoIdQuery, useGetStartupConfig, useGetEndpointsQuery, useProjectQuery, } from '~/data-provider';
import { useAssistantListMap, useIdChangeEffect, useAppStartup, useNewConvo, useLocalize, } from '~/hooks';
...
const isValidChatProjectId = (projectId) => projectId != null && /^[a-f\d]{24}$/i.test(projectId);
export default function ChatRoute() {
    ...
    const setIsTemporary = useRecoilCallback(({ set }) => (value) => {
     ...
     */
    const verifiedChatProjectId = !projectNotFound && projectQuery.data?._id === chatProjectId ? chatProjectId : null;
    const projectTemplate = useMemo(() => (verifiedChatProjectId ? { chatProjectId: verifiedChatProjectId } : {}), [verifiedChatProjectId]);
    /**
     ...
     */
    const projectScopeMissing = Boolean(chatProjectId) &&
    ...
    const initialConvoQuery = useGetConvoIdQuery(conversationId, {
        enabled: isAuthenticated && conversationId !== Constants.NEW_CONVO && !hasSetConversation.current,
    });
    ...
    useEffect(() => {
        ...
        const newConvoNeedsInit = isNewConvo && (!conversation || (isDraftNewConvo && draftProjectMismatch));
        const shouldSetConvo = (startupConfig &&
            rolesLoaded &&
        ...
        if (isNewConvo && endpointsQuery.data && modelsQuery.data) {
            ...
            logger.log('conversation', 'ChatRoute initialConvoQuery isNotFoundError', initialConvoQuery.error);
            newConversation({
                modelsData: modelsQuery.data,
        ...
        else if (assistantListMap[EModelEndpoint.assistants] &&
            assistantListMap[EModelEndpoint.azureAssistants]) {
    ...
    if (endpointsQuery.isLoading || modelsQuery.isLoading) {
        return (<div className="flex h-screen items-center justify-center" aria-live="polite" role="status">
    ...
    return (<ToolCallsMapProvider conversationId={conversation.conversationId ?? ''}>
