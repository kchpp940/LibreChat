...
function FilterChip({ type, label, isSelected, onClick }) {
    return (<button onClick={onClick} className={cn('flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition-all', isSelected
...
export default function Search() {
    ...
    const { data: searchMessages, isLoading, isError, fetchNextPage, isFetchingNextPage, hasNextPage: _hasNextPage, } = useMessagesInfiniteQuery({
    ...
    const messages = useMemo(() => {
        const msgs = searchMessages?.pages.flatMap((page) => page.messages.map((message) => {
    ...
    return (<MinimalMessagesWrapper ref={containerRef} className="relative flex h-full pt-4">
      ...
      <div className="sticky top-0 z-10 flex flex-wrap gap-2 border-b border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-800">
        {FILTER_OPTIONS.map((option) => (<FilterChip key={option.type} type={option.type} label={localize(option.label)} isSelected={selectedTypes.includes(option.type)} onClick={() => toggleFilter(option.type)}/>))}
      </div>

      {(messages && messages.length === 0) || messages == null ? (<div className="absolute inset-0 flex items-center justify-center">
          ...
          {messages.map((msg) => (<SearchMessage key={msg.messageId} message={msg} searchHits={searchHits[msg.messageId]}/>))}
