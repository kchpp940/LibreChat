...
export default function StartupLayout({ isAuthenticated }) {
    const [error, setError] = useState(null);
    const [headerText, setHeaderText] = useState(null);
    const [startupConfig, setStartupConfig] = useState(null);
    const { data, isFetching, error: startupConfigError, } = useGetStartupConfig({
    ...
    useEffect(() => {
        if (isAuthenticated) {
            const hasPendingRedirect = new URLSearchParams(window.location.search).has(REDIRECT_PARAM) ||
