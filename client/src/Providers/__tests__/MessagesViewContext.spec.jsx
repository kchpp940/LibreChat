...
import { useOptionalMessagesOperations, useOptionalMessagesConversation, } from '../MessagesViewContext';
describe('useOptionalMessagesOperations', () => {
    ...
    it('noop stubs do not throw when called', () => {
        const { result } = renderHook(() => useOptionalMessagesOperations());
        expect(() => result.current.ask({})).not.toThrow();
        expect(() => result.current.regenerate({})).not.toThrow();
        expect(() => result.current.handleContinue({})).not.toThrow();
