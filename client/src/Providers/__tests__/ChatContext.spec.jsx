...
describe('ChatContext', () => {
    it('throws when useChatContext is called outside a provider', () => {
        ...
        expect(() => render(<TestConsumer />)).toThrow('useChatContext must be used within a ChatContext.Provider');
        spy.mockRestore();
    });
    it('provides context value when wrapped in provider', () => {
        const mockHelpers = { index: 0 };
        render(<ChatContext.Provider value={mockHelpers}>
        <TestConsumer />
