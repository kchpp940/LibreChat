...
export function useToolToggle({ conversationId, storageContextKey, toolKey: _toolKey, localStorageKey, isAuthenticated: externalIsAuthenticated, setIsDialogOpen, authConfig, }) {
    const key = conversationId ?? Constants.NEW_CONVO;
    const [ephemeralAgent, setEphemeralAgent] = useRecoilState(ephemeralAgentByConvoId(key));
    const authQuery = useVerifyAgentToolAuth({ toolId: authConfig?.toolId || '' }, {
        enabled: !!authConfig?.toolId,
        ...authConfig?.queryOptions,
    });
    const isAuthenticated = useMemo(() => externalIsAuthenticated ?? (authConfig ? (authQuery?.data?.authenticated ?? false) : false), [externalIsAuthenticated, authConfig, authQuery.data?.authenticated]);
    ...
    const [isPinned, setIsPinned] = useLocalStorage(`${localStorageKey}pinned`, false);
    ...
    const debouncedChange = useMemo(() => debounce(handleChange, 50, { leading: true }), [handleChange]);
    return {
        toggleState: toolValue, // Return the actual value from ephemeralAgent
        handleChange,
        isToolEnabled,
        toolValue,
        setToggleState: (value) => handleChange({ value }), // Adapter for direct setting
        ephemeralAgent,
        debouncedChange,
        setEphemeralAgent,
