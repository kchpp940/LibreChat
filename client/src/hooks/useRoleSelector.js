...
export function useRoleSelector(permissionType) {
    ...
    const { data: customRoleData = null, isLoading: isCustomRoleLoading, isError: isCustomRoleError, } = useGetRole(isSelectedCustomRole ? selectedRole : '_', { enabled: isSelectedCustomRole });
    const resolvePermissions = useCallback((role, customData) => {
    ...
    const defaultValues = useMemo(() => resolvePermissions(selectedRole, customRoleData), [resolvePermissions, selectedRole, customRoleData]);
    ...
    const roleDropdownItems = useMemo(() => availableRoleNames.map((role) => ({ label: role, onClick: () => setSelectedRole(role) })), [availableRoleNames]);
    return {
        selectedRole,
        setSelectedRole,
