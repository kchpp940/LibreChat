...
import { apiBaseUrl, SystemRoles, setTokenHeader, isSystemRoleName, buildLoginRedirectUrl, } from 'librechat-data-provider';
...
const AuthContextProvider = ({ authConfig, children, }) => {
    ...
    const setUserContext = useMemo(() => debounce((userContext) => {
        ...
        const finalRedirect = logoutRedirect ??
            postLoginRedirect ??
    ...
    }, 50), [navigate, setUser, setQueriesEnabled]);
    ...
    const loginUser = useLoginUserMutation({
        ...
        onError: (error) => {
            ...
            const loginPath = redirectTo && isSafeRedirect(redirectTo)
        ...
        refreshToken.mutate(undefined, {
            onSuccess: (data) => {
                ...
                if (token) {
                    ...
                    const strippedPath = baseUrl && (rawPath === baseUrl || rawPath.startsWith(baseUrl + '/'))
                    ...
                    const redirect = storedRedirect && isSafeRedirect(storedRedirect) ? storedRedirect : fallbackRedirect;
