...
export default function useChatHelpers(index = 0, paramId) {
    ...
    const setSiblingIdx = useSetRecoilState(store.messagesSiblingIdxFamily(latestMessage?.parentMessageId ?? null));
    const setMessages = useCallback((messages) => {
        ...
        for (const messageCacheId of messageCacheIds) {
            queryClient.setQueryData([QueryKeys.messages, messageCacheId], messages);
        }
    ...
    askRef.current = _ask;
    const ask = useCallback((...args) => askRef.current(...args), []);
    const regenerateRef = useRef(_regenerate);
    regenerateRef.current = _regenerate;
    const regenerate = useCallback((...args) => regenerateRef.current(...args), []);
    const continueGeneration = useCallback(() => {
        ...
        const parentMessage = messages?.find((element) => element.messageId == currentLatest.parentMessageId);
    ...
    const handleStopGenerating = useCallback((e) => {
        e.preventDefault();
        stopGenerating();
    }, [stopGenerating]);
