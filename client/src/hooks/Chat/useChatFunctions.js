...
import { Constants, QueryKeys, ContentTypes, EModelEndpoint, getEndpointField, isAgentsEndpoint, parseCompactConvo, replaceSpecialVars, isAssistantsEndpoint, getDefaultParamsEndpoint, } from 'librechat-data-provider';
import { logger, hasStreamStartFailed, createDualMessageContent, getRouteChatProjectId, } from '~/utils';
...
const logChatRequest = (request) => {
    logger.log('=====================================\nAsk function called with:');
    logger.dir(request);
    logger.log('=====================================');
};
const getAppendParentMessageId = ({ latestMessage, currentMessages, }) => {
    if (!latestMessage) {
    ...
    const failedUserMessage = currentMessages.find((message) => message.messageId === latestMessage.parentMessageId);
    if (failedUserMessage?.isCreatedByUser !== true) {
        return latestMessage.messageId;
    }
    return failedUserMessage.parentMessageId ?? Constants.NO_PARENT;
};
const hasPendingAssistantParent = (message) => !!message?.messageId &&
    message.isCreatedByUser !== true &&
    message.messageId.endsWith('_') &&
    message.createdAt == null &&
    message.updatedAt == null &&
    !hasStreamStartFailed(message);
const isAssistantResponseForParent = (message, parentMessageId) => !!message &&
    !message.isCreatedByUser &&
    !!parentMessageId &&
    message.parentMessageId === parentMessageId;
export function getPreliminaryRegenerateResponseMessageId(responseMessageId) {
    if (typeof responseMessageId !== 'string' || responseMessageId.length === 0) {
        return null;
    }
    return `${responseMessageId.replace(/_+$/, '')}_`;
}
export function getRegenerateTargetResponseMessage({ messages, parentMessageId, targetResponseMessageId, latestMessage, }) {
    if (!parentMessageId) {
        return null;
    }
    if (targetResponseMessageId) {
        const targetResponse = messages.find((message) => message.messageId === targetResponseMessageId &&
...
export function getRegenerateSubmissionMessages({ messages, targetResponseMessage, initialResponseId, }) {
    if (targetResponseMessage?.messageId) {
        const targetIndex = messages.findIndex((message) => message.messageId === targetResponseMessage.messageId);
        if (targetIndex >= 0) {
...
export default function useChatFunctions({ index = 0, files, setFiles, getMessages, setMessages, isSubmitting, latestMessage, setSubmission, conversation: immutableConversation, }) {
     ...
     */
    const drainPendingManualSkills = useRecoilCallback(({ snapshot, reset }) => (convoId) => {
        const loadable = snapshot.getLoadable(store.pendingManualSkillsByConvoId(convoId));
        const skills = loadable.state === 'hasValue' ? loadable.contents : [];
        if (skills.length > 0) {
            reset(store.pendingManualSkillsByConvoId(convoId));
        }
        return skills;
    }, []);
    const ask = ({ text, overrideConvoId, overrideUserMessageId, parentMessageId = null, conversationId = null, messageId = null, }, { editedContent = null, editedMessageId = null, isRegenerate = false, isContinued = false, isEdited = false, overrideMessages, overrideFiles, targetResponseMessageId, overrideManualSkills, addedConvo, } = {}) => {
        ...
        if (parentMessageId == null && hasPendingAssistantParent(latestMessage)) {
            logger.warn('[useChatFunctions] Refusing to append to a preliminary assistant message', latestMessage);
            return false;
        }
        ...
        const chatProjectId = conversationId === Constants.NEW_CONVO
            ? getRouteChatProjectId()
            : (conversation?.chatProjectId ?? null);
        const conversationForPayload = chatProjectId != null ? { ...(conversation ?? {}), chatProjectId } : (conversation ?? {});
         ...
         */
        const targetParentMessage = currentMessages.find((msg) => msg.messageId === targetParentMessageId);
        ...
        }, convo, chatProjectId ? { chatProjectId } : {});
        ...
        const reuseFiles = (isRegenerate || (overrideFiles != null && overrideFiles.length)) &&
            submissionFiles &&
        ...
        const responseMessageId = editedMessageId ??
            (isRegenerate
                ? getPreliminaryRegenerateResponseMessageId(targetResponseMessage?.messageId ?? targetResponseMessageId)
                : null) ??
            null;
        const initialResponseId = responseMessageId ?? `${isRegenerate ? messageId : intermediateId}`.replace(/_+$/, '') + '_';
        ...
        if (isAssistantsEndpoint(endpoint)) {
            ...
            if (editedContent && latestMessage?.content) {
                ...
                initialResponse.content = createDualMessageContent(conversation, addedConvo, endpointsConfig, startupConfig?.modelSpecs?.list);
            }
            else {
                initialResponse.content = [];
            }
    ...
    const regenerate = (message, options) => {
        const messages = getMessages();
        const parentMessageId = message.isCreatedByUser === true ? message.messageId : message.parentMessageId;
        const targetResponseMessageId = message.isCreatedByUser === true ? undefined : message.messageId;
