...
describe('useFocusChatEffect', () => {
    ...
    describe('Basic functionality', () => {
        test('should focus textarea when location.state.focusChat is true', () => {
            renderHook(() => useFocusChatEffect(mockTextAreaRef));
        ...
        test('should not focus textarea when textAreaRef.current is null', () => {
            ...
            renderHook(() => useFocusChatEffect(nullTextAreaRef));
            expect(mockNavigate).not.toHaveBeenCalled();
        });
    ...
    describe('URL parameter handling', () => {
        ...
        const testUrlScenario = ({ windowLocationSearch, reactRouterSearch, expectedUrl, testDescription, }) => {
            test(`${testDescription}`, () => {
                ...
                renderHook(() => useFocusChatEffect(mockTextAreaRef));
                expect(mockNavigate).toHaveBeenCalledWith(expectedUrl, expect.objectContaining({
                    replace: true,
                    state: {},
                }));
            });
        ...
        test('should use window.location.search instead of location.search', () => {
            ...
            '/c/new?agent_id=test_agent_id', expect.objectContaining({
                replace: true,
                state: {},
            }));
        });
        ...
        testUrlScenario({
            ...
            expectedUrl: '/c/new?agent_id=agent123',
            testDescription: 'should use window.location.search even when both have agent_id but with different values',
        });
        testUrlScenario({
        ...
        testUrlScenario({
            windowLocationSearch: '?agent_id=agent123&prompt=test&model=gpt-4&temperature=0.7&max_tokens=1000',
            reactRouterSearch: '?endpoint=openAI',
            expectedUrl: '/c/new?agent_id=agent123&prompt=test&model=gpt-4&temperature=0.7&max_tokens=1000',
        ...
        test('should handle navigation immediately after URL parameter changes', () => {
            ...
            const { rerender } = renderHook(() => useFocusChatEffect(mockTextAreaRef));
            expect(mockNavigate).toHaveBeenCalledWith('/c/new?endpoint=openAI&model=gpt-4', expect.objectContaining({
                replace: true,
                state: {},
            }));
            jest.clearAllMocks();
            ...
            rerender();
            expect(mockNavigate).toHaveBeenCalledWith('/c/new_changed?agent_id=agent123', expect.objectContaining({
                replace: true,
                state: {},
            }));
        });
        test('should handle undefined or null search params gracefully', () => {
            ...
            renderHook(() => useFocusChatEffect(mockTextAreaRef));
            expect(mockNavigate).toHaveBeenCalledWith('/c/new', expect.objectContaining({
                replace: true,
                state: {},
            }));
            jest.clearAllMocks();
