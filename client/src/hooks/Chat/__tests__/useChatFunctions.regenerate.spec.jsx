...
jest.mock('recoil', () => ({
    ...
    useSetRecoilState: (atom) => String(atom).includes('isSubmitting') ? mockSetIsSubmitting : mockSetShowStopButton,
    useRecoilCallback: (factory) => factory({
        snapshot: {
...
describe('useChatFunctions regenerate', () => {
    ...
    it('keys a non-tail regenerate to the selected assistant response', () => {
        ...
        const setMessages = jest.fn((nextMessages) => {
        ...
        const { result } = renderHook(() => useChatFunctions({
            isSubmitting: false,
        ...
        expect(setMessages.mock.calls.at(-1)?.[0].map((message) => message.messageId)).toEqual(['user-1', 'assistant-1_']);
