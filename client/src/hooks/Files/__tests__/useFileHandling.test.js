...
const mockProcessFileForUpload = jest.fn(async (_file, _quality, _onProgress) => _file);
...
jest.mock('~/data-provider', () => ({
    ...
    useUploadFileMutation: jest.fn((_opts) => ({
        mutate: mockMutate,
    })),
}));
jest.mock('~/hooks/useLocalize', () => {
    const fn = jest.fn(() => mockLocalize);
    fn.TranslationKeys = {};
...
jest.mock('../useClientResize', () => ({
    __esModule: true,
    default: jest.fn(() => ({
        resizeImageIfNeeded: jest.fn(async (file) => ({ file, resized: false })),
    })),
}));
...
describe('useFileHandling', () => {
    beforeEach(() => {
        jest.clearAllMocks();
        mockProcessFileForUpload.mockImplementation(async (file) => file);
        mockConversation = {};
    ...
    describe('endpointOverride', () => {
        ...
        it('uses endpointOverride for validation instead of conversation endpoint', async () => {
            ...
            const useFileHandling = await loadHook();
            const { result } = renderHook(() => useFileHandling({ endpointOverride: EModelEndpoint.agents }));
            const textFile = new File(['hello'], 'test.txt', { type: 'text/plain' });
            await act(async () => {
                await result.current.handleFiles([textFile]);
            });
        ...
        it('awaits HEIC conversion before uploading the converted file', async () => {
            ...
            mockProcessFileForUpload.mockImplementationOnce(async (_file, _quality, onProgress) => {
                onProgress?.(1);
