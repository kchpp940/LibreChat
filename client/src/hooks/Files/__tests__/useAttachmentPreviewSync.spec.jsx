...
jest.mock('~/data-provider', () => ({
    useFilePreview: (...args) => mockUseFilePreview(...args),
}));
...
function setup({ attachment, isSubmitting, preview, isFetching = false, seedLiveMap = true, }) {
    ...
    mockUseFilePreview.mockImplementation((_id, opts) => {
 ...
 */
function setupWithTransitions(initialPreview, { isSubmittingAtMount = true } = {}) {
    ...
    const flagRef = { current: undefined };
    const FlagProbe = ({ id }) => {
    ...
    const { rerender } = renderHook(({ attachment }) => useAttachmentPreviewSync(attachment), {
        initialProps: { attachment: makeAttachment({ status: 'pending' }) },
        wrapper: ({ children }) => (<RecoilRoot initializeState={(snap) => {
...
describe('useAttachmentPreviewSync', () => {
    ...
    it('does NOT enable polling when the attachment has no file_id', () => {
        const noId = { ...makeAttachment({ status: 'pending' }) };
        delete noId.file_id;
        const ctx = setup({
            attachment: noId,
            isSubmitting: true,
    ...
    it('returns ready when no preview and no status (legacy back-compat)', () => {
        ...
        const ctx = setup({ attachment: legacy, isSubmitting: false });
    ...
    it('upserts the resolved preview into messageAttachmentsMap when preview reports ready', () => {
        ...
        const updated = ctx.map[messageId]?.[0];
    ...
    describe('previewJustResolved signal (auto-open trigger)', () => {
        ...
        it('does NOT flip the flag on a navigation-time pending→ready (hook mounted with isSubmitting=false)', () => {
            ...
            const ctx = setupWithTransitions({ file_id: fileId, status: 'pending' }, { isSubmittingAtMount: false });
            ctx.setPreview({
                file_id: fileId,
                status: 'ready',
