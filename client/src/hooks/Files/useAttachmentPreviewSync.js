 ...
 */
export default function useAttachmentPreviewSync(attachment) {
    ...
    const flagJustResolved = useRecoilCallback(({ set }) => (id) => {
     ...
     * removed. */
    const readInitialIsSubmitting = useRecoilCallback(({ snapshot }) => () => snapshot.getLoadable(store.isSubmittingFamily(0)).valueMaybe() ?? false, []);
    ...
    const baseStatus = file?.status ?? 'ready';
    ...
    const polled = previewQuery.data;
    const effectiveStatus = polled?.status ?? baseStatus;
    ...
    useEffect(() => {
        ...
        setAttachmentsMap((prevMap) => {
            ...
            const existingIndex = messageAttachments.findIndex((a) => a.file_id === fileId);
            ...
            if (existingIndex >= 0) {
                const existing = messageAttachments[existingIndex];
            ...
            const inserted = { ...attachment, ...resolvedFields };
