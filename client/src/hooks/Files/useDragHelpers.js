...
import { Tools, QueryKeys, Constants, inferMimeType, EToolResources, EModelEndpoint, mergeFileConfig, AgentCapabilities, resolveEndpointType, isAssistantsEndpoint, getEndpointFileConfig, defaultAgentCapabilities, } from 'librechat-data-provider';
...
export default function useDragHelpers() {
    ...
    const isAssistants = useMemo(() => isAssistantsEndpoint(conversation?.endpoint), [conversation?.endpoint]);
    const { handleFiles } = useFileHandling();
    const handleOptionSelect = useCallback((toolResource) => {
    ...
    const handleDrop = useCallback((item) => {
        ...
        const agent = agentId
            ...
            : undefined;
        const currentEndpointType = resolveEndpointType(endpointsConfig, currentEndpoint, agent?.provider);
        ...
        const allImages = item.files.every((f) => inferMimeType(f.name, f.type)?.startsWith('image/'));
        const shouldShowModal = allImages ||
