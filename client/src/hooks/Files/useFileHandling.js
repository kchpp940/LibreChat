...
const noop = () => { };
const useFileHandlingCore = (params, fileState) => {
    ...
    const setEphemeralAgent = useSetRecoilState(ephemeralAgentByConvoId(conversation?.conversationId ?? Constants.NEW_CONVO));
    const isTemporary = useRecoilValue(store.isTemporary);
    const setError = (error) => setErrors((prevErrors) => [...prevErrors, error]);
    const { addFile, replaceFile, updateFileById, deleteFileById } = useUpdateFiles(params?.fileSetter ?? setFiles);
    ...
    const endpointType = useMemo(() => endpointTypeOverride ?? endpointOverride ?? conversation?.endpointType, [endpointTypeOverride, endpointOverride, conversation?.endpointType]);
    const endpoint = useMemo(() => endpointOverride ?? conversation?.endpoint ?? 'default', [endpointOverride, conversation?.endpoint]);
    ...
    const startUpload = async (extendedFile) => {
        ...
        formData.append('file', extendedFile.file, encodeURIComponent(filename));
        ...
        const formVersion = (formData.get('version') ?? '');
        if (!formVersion) {
            formData.append('version', version);
        }
    ...
    const loadImage = (extendedFile, preview) => {
        const img = new Image();
        img.onload = async () => {
    ...
    const handleFiles = async (_files, _toolResource) => {
        ...
        for (const originalFile of fileList) {
            ...
            try {
                ...
                const heicProcessedFile = isHEIC
                    ? await import('~/utils/heicConverter').then(({ processFileForUpload }) => processFileForUpload(originalFile, 0.9, (conversionProgress) => {
    ...
    const handleFileChange = (event, _toolResource) => {
...
export const useFileHandlingNoChatContext = (params, fileState) => useFileHandlingCore(params, fileState);
const useFileHandling = (params) => {
