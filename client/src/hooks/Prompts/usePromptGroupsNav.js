...
export default function usePromptGroupsNav(hasAccess = true) {
    ...
    const cursorHistoryRef = useRef([null]); // Start with null for first page
    ...
    useEffect(() => {
        if (!hasAccess)
            return;
        const filtersChanged = prevFiltersRef.current.name !== name || prevFiltersRef.current.category !== category;
        if (filtersChanged) {
