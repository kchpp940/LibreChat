 ...
 * ```
 */
export default function useRenderChangeLog(name, values) {
    const previousValuesRef = useRef(null);
    useEffect(() => {
        if (process.env.NODE_ENV === 'production') {
            return;
        }
        if (typeof window === 'undefined' || !window.__LC_RENDER_DEBUG__) {
        ...
        const changedEntries = Object.entries(values).filter(([key, value]) => !Object.is(previousValues[key], value));
        if (changedEntries.length > 0) {
            console.log(`[render-debug] ${name}`, Object.fromEntries(changedEntries.map(([key, value]) => [
                key,
                {
