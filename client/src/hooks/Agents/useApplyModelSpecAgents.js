...
export function useApplyModelSpecEffects() {
    ...
    const applyModelSpecEffects = useCallback(({ convoId, specName, startupConfig, }) => {
...
export function useApplyAgentTemplate() {
     ...
     */
    const applyAgentTemplateWithSpec = useCallback(({ targetId, sourceId, ephemeralAgent, specName, startupConfig, }) => {
        if (!specName) {
        ...
        const mergedAgent = {
            ...
            artifacts: ephemeralAgent?.artifacts ??
