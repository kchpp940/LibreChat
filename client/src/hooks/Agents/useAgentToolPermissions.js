 ...
 */
export default function useAgentToolPermissions(agentId, ephemeralAgent) {
    const agentsMap = useAgentsMapContext();
    const selectedAgent = useMemo(() => {
        return agentId != null && agentId !== '' ? agentsMap?.[agentId] : undefined;
    }, [agentId, agentsMap]);
    const { data: agentData } = useGetAgentByIdQuery(agentId);
    const tools = useMemo(() => agentData?.tools || selectedAgent?.tools, [agentData?.tools, selectedAgent?.tools]);
    const provider = useMemo(() => agentData?.provider || selectedAgent?.provider, [agentData?.provider, selectedAgent?.provider]);
