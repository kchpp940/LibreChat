...
import { Constants, QueryKeys, dataService, EModelEndpoint, isAssistantsEndpoint, } from 'librechat-data-provider';
...
export default function useSelectAgent() {
    ...
    const updateConversation = useCallback(async (agent, template) => {
    ...
    const onSelect = useCallback(async (value) => {
        ...
        try {
            const fullAgent = await queryClient.fetchQuery([QueryKeys.agent, agent.id], () => dataService.getAgentById({
                agent_id: agent.id,
            }));
            if (fullAgent) {
