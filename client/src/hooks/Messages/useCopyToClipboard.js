...
import { SPAN_REGEX, CLEANUP_REGEX, COMPOSITE_REGEX, STANDALONE_PATTERN, INVALID_CITATION_REGEX, } from '~/utils/citations';
const refTypeMap = {
    search: 'organic',
    ref: 'references',
    news: 'topStories',
...
export default function useCopyToClipboard({ text, content, searchResults, }) {
    ...
    const copyToClipboard = useCallback((setIsCopied) => {
        ...
        if (citationManager.citations.size > 0) {
            ...
            const sortedCitations = Array.from(citationManager.citations.entries()).sort((a, b) => a[1].referenceNumber - b[1].referenceNumber);
...
function processCitations(text, searchResults) {
    ...
    const replacements = [];
    const compositeCitationsMap = new Map();
