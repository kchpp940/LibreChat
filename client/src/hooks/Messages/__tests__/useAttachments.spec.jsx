...
jest.mock('~/hooks/useLocalize', () => () => (key) => key);
...
function setup({ attachments, liveMap, }) {
    const Seed = () => {
...
describe('useAttachments', () => {
    ...
    it('returns live entries when no DB attachments exist (active SSE turn)', () => {
        ...
        expect(result.current.attachments).toHaveLength(1);
        expect(result.current.attachments[0].text).toBe('<table>x</table>');
    });
    it('overlays live lifecycle fields onto matching DB attachment by file_id', () => {
        ...
        expect(result.current.attachments).toHaveLength(1);
        const merged = result.current.attachments[0];
    ...
    it('leaves DB attachments untouched when no live entry shares the file_id', () => {
        ...
        expect(result.current.attachments).toHaveLength(1);
        expect(result.current.attachments[0].file_id).toBe('fid-A');
