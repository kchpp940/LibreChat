...
function createWrapper(queryClient = createQueryClient(), initializeState, initialEntry = '/c/conversation-1') {
...
describe('useLatestMessage', () => {
    ...
    it('returns the tail message from the active conversation cache', () => {
        ...
        queryClient.setQueryData([QueryKeys.messages, conversation.conversationId], [userMessage, assistantMessage]);
        const { result } = renderHook(() => useLatestMessage(0), {
    ...
    it('uses the new-conversation route cache while the conversation atom has the server id', () => {
        ...
        queryClient.setQueryData([QueryKeys.messages, 'new'], [
            {
                ...userMessage,
    ...
    it('keeps latestMessageId stable across token-only cache writes', () => {
        ...
        act(() => {
            queryClient.setQueryData([QueryKeys.messages, conversation.conversationId], [userMessage, { ...assistantMessage, text: 'Hi there, still streaming' }]);
        });
...
describe('getMessageBranchSiblingParentIds', () => {
    it('returns only parent keys that have branch choices', () => {
        expect(getMessageBranchSiblingParentIds([
            userMessage,
            olderAssistantMessage,
            olderFollowUpUserMessage,
            olderFollowUpAssistantMessage,
            assistantMessage,
        ], conversation.conversationId)).toEqual([userMessage.messageId]);
    });
});
describe('getBranchSiblingIndexesForTarget', () => {
    it('returns sibling indexes that select the branch containing the target message', () => {
        expect(getBranchSiblingIndexesForTarget([
            userMessage,
            olderAssistantMessage,
            olderFollowUpUserMessage,
            olderFollowUpAssistantMessage,
            assistantMessage,
        ], olderFollowUpAssistantMessage.messageId, conversation.conversationId)).toEqual([
            {
                parentMessageId: userMessage.messageId,
                siblingIdx: 1,
            },
