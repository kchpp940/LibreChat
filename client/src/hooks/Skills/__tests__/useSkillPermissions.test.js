...
const useResourcePermissionsMock = useResourcePermissions;
const useAuthContextMock = useAuthContext;
...
function mockUser(id, role = SystemRoles.USER) {
    useAuthContextMock.mockReturnValue({
        user: { id, role },
        roles: { [role]: {} },
    });
}
function mockPermissions(bits, isLoading = false) {
    useResourcePermissionsMock.mockReturnValue({
        hasPermission: (required) => (bits & required) === required,
        isLoading,
        permissionBits: bits,
    });
}
describe('useSkillPermissions', () => {
    ...
    it('grants DELETE + SHARE when the ACL owner role bits are present', () => {
        ...
        mockPermissions(PermissionBits.VIEW | PermissionBits.EDIT | PermissionBits.DELETE | PermissionBits.SHARE);
        const { result } = renderHook(() => useSkillPermissions(makeSkill()));
        expect(result.current.canEdit).toBe(true);
        expect(result.current.canDelete).toBe(true);
        expect(result.current.canShare).toBe(true);
