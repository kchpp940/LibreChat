...
function abandonOtherQueues(activeUserId) {
 ...
 */
function resolveDefault(author, userId, defaultActiveOnShare) {
...
export default function useSkillActiveState() {
    ...
    const flush = useCallback(async () => {
        ...
        while (queue.pending !== null && !queue.abandoned) {
            ...
            try {
                ...
                const data = error
                    ?.response?.data;
                const message = data?.code === 'MAX_SKILL_STATES_EXCEEDED'
    ...
    const isActive = useCallback((skill) => {
    ...
    const toggle = useCallback((skill) => {
        if (!canToggle || !userId) {
            return;
        }
        const queue = getWriteQueue(userId);
        const cached = queryClient.getQueryData([QueryKeys.skillStates]) ?? EMPTY_STATES;
