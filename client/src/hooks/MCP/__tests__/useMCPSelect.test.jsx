...
const createMCPServers = (serverNames) => {
...
describe('useMCPSelect', () => {
    ...
    describe('Race Conditions and Infinite Loops Prevention', () => {
        ...
        it('should handle switching conversation IDs without issues', async () => {
            ...
            const { result, rerender } = renderHook(({ conversationId }) => useMCPSelect({ conversationId, servers }), {
                wrapper: Wrapper,
    ...
    describe('Ephemeral Agent Synchronization', () => {
        it('should sync mcpValues when ephemeralAgent is updated externally', async () => {
            ...
            const TestComponent = () => {
                ...
                const [ephemeralAgent, setEphemeralAgent] = useRecoilState(ephemeralAgentByConvoId(Constants.NEW_CONVO));
    ...
    describe('Environment-Keyed Storage (storageContextKey)', () => {
        it('should use storageContextKey as atom key for new conversations', async () => {
            ...
            const { result: resultA } = renderHook(() => useMCPSelect({ conversationId: null, storageContextKey, servers }), { wrapper: Wrapper });
            act(() => {
            ...
            const { result: resultB } = renderHook(() => useMCPSelect({ conversationId: null, servers }), { wrapper: Wrapper });
        ...
        it('should use conversationId as atom key for existing conversations even with storageContextKey', async () => {
            ...
            const { Wrapper, servers } = createWrapper(['server1', 'server2']);
            const storageContextKey = '__defaults__';
            const { result } = renderHook(() => useMCPSelect({ conversationId, storageContextKey, servers }), { wrapper: Wrapper });
            act(() => {
        ...
        it('should dual-write to environment key when storageContextKey is provided', async () => {
            const { Wrapper, servers } = createWrapper(['server1', 'server2']);
            const storageContextKey = '__defaults__';
            const { result } = renderHook(() => useMCPSelect({ conversationId: null, storageContextKey, servers }), { wrapper: Wrapper });
            act(() => {
                result.current.setMCPValues(['server1', 'server2']);
            });
            await waitFor(() => {
        ...
        it('should isolate per-conversation state from environment defaults', async () => {
            ...
            const { result: newConvoResult } = renderHook(() => useMCPSelect({ conversationId: null, storageContextKey, servers }), { wrapper: Wrapper });
            act(() => {
            ...
            const { result: existingResult } = renderHook(() => useMCPSelect({ conversationId: 'existing-convo', storageContextKey, servers }), { wrapper: Wrapper });
