...
export function useMCPServerManager({ conversationId, storageContextKey, } = {}) {
    ...
    const selectableServers = useMemo(() => availableMCPServers.filter((s) => s.config.chatMenu !== false && !s.consumeOnly), [availableMCPServers]);
    ...
    const checkEffectivePermission = useCallback((effectivePermissions, permissionBit) => {
    ...
    const updateServerInitState = useCallback((serverName, updates) => {
    ...
    const cleanupServerState = useCallback((serverName) => {
    ...
    const startServerPolling = useCallback((serverName) => {
        ...
        const getPollInterval = (attempt) => {
            if (attempt < 12)
                return 5000; // First minute: every 5s (12 polls)
            if (attempt < 22)
                return 6000; // Second minute: every 6s (10 polls)
        ...
        const connectionData = queryClient.getQueryData([
        ...
        const pollOnce = async () => {
            try {
                ...
                if (pollAttempts > maxAttempts || elapsedTime > oauthTimeoutMs) {
                    console.warn(`[MCP Manager] OAuth timeout for ${serverName} after ${(elapsedTime / 1000).toFixed(0)}s (attempt ${pollAttempts})`);
                    showToast({
                ...
                if (pollAttempts % 5 === 0 || pollAttempts <= 2) {
                    console.debug(`[MCP Manager] Polling ${serverName} attempt ${pollAttempts}/${maxAttempts}, next in ${nextInterval / 1000}s`);
                }
    ...
    const initializeServer = useCallback(async (serverName, autoOpenOAuth = true) => {
    ...
    const cancelOAuthFlow = useCallback((serverName) => {
    ...
    const isInitializing = useCallback((serverName) => {
        return getServerInitState(serverInitStates, serverName).isInitializing;
    }, [serverInitStates]);
    const isCancellable = useCallback((serverName) => {
    ...
    const handleConfigSave = useCallback((targetName, authData) => {
    ...
    const handleConfigRevoke = useCallback((targetName) => {
        if (selectedToolForConfig && selectedToolForConfig.name === targetName) {
            const payload = {
                pluginKey: `${Constants.mcp_prefix}${targetName}`,
                action: 'uninstall',
    ...
    const revokeOAuthForServer = useCallback((serverName) => {
        const payload = {
    ...
    const handleSave = useCallback((authData) => {
    ...
    const getServerStatusIconProps = useCallback((serverName) => {
        const mcpData = queryClient.getQueryData([
            QueryKeys.mcpTools,
        ]);
        ...
        const hasPendingOAuth = serverStatus?.requiresOAuth === true && serverStatus.connectionState === 'connecting';
