...
function hasSubmissionUserMessage(submission, messages, conversationId) {
    const userMessageId = submission?.userMessage?.messageId;
    if (!userMessageId || !conversationId || !messages?.length) {
        return false;
    }
    return messages.some((message) => message.isCreatedByUser === true &&
        message.messageId === userMessageId &&
        message.conversationId === conversationId);
}
function resumeStateMatchesSubmission(streamStatus, submission) {
...
function getResumeBranchTargetMessageId(resumeState, messages) {
...
function preferDefinedString(value, fallback) {
 ...
 */
function buildSubmissionFromResumeState(resumeState, streamId, messages, conversationId) {
    const userMessageData = resumeState.userMessage;
    const responseMessageId = resumeState.responseMessageId ?? `${userMessageData?.messageId ?? 'resume'}_`;
    // Try to find existing user message in the messages array (from database)
    const existingUserMessage = messages.find((m) => m.isCreatedByUser && m.messageId === userMessageData?.messageId);
    // Try to find existing response message in the messages array (from database)
    const existingResponseMessage = messages.find((m) => !m.isCreatedByUser &&
 ...
 */
export default function useResumeOnLoad(conversationId, getMessages, runIndex = 0, messagesLoaded = true) {
    ...
    const restoreResumeBranch = useRecoilCallback(({ set }) => (resumeState, messages, activeConversationId) => {
        const targetMessageId = getResumeBranchTargetMessageId(resumeState, messages);
        const branchIndexes = getBranchSiblingIndexesForTarget(messages, targetMessageId, activeConversationId);
    ...
    const hasHydratedMessageMatch = submissionConvoId == null &&
        hasSubmissionUserMessage(currentSubmission, loadedMessages, conversationId);
    const hasActiveSubmissionForThisConvo = !!currentSubmission && (hasExplicitSubmissionMatch || hasHydratedMessageMatch);
    const hasStaleSubmissionForDifferentConvo = !!currentSubmission && submissionConvoId != null && submissionConvoId !== conversationId;
    const shouldCheck = resumableEnabled &&
    ...
    const { data: streamStatus, isSuccess, isFetching, } = useStreamStatus(conversationId, shouldCheck);
    useEffect(() => {
        ...
        if (hasStaleSubmissionForDifferentConvo) {
            console.log('[ResumeOnLoad] Found stale submission for different conversation, will check for resume', {
        ...
        if (streamStatus.resumeState) {
            ...
            const submission = buildSubmissionFromResumeState(streamStatus.resumeState, streamStatus.streamId, messages, conversationId);
            ...
            const submission = {
                messages,
                userMessage: lastUserMessage ?? { messageId: 'resume', conversationId, text: '' },
                initialResponse: {
                    messageId: 'resume_',
                    conversationId,
                    text: '',
                ...
                conversation: { conversationId, title: 'Resumed Chat' },
                isRegenerate: false,
                isTemporary: false,
                endpointOption: {},
                // Signal to useResumableSSE to subscribe to existing stream instead of starting new
