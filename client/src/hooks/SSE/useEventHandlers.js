...
import { Constants, EndpointURLs, ContentTypes, tPresetSchema, tMessageSchema, tConvoUpdateSchema, isAssistantsEndpoint, } from 'librechat-data-provider';
import { logger, setDraft, scrollToEnd, getAllContentText, upsertConvoInAllQueries, } from '~/utils';
import { startupConfigKey, queueTitleGeneration, markTitleGenerationProcessed, useConversationCache, } from '~/data-provider';
...
const hasRealTitle = (title) => title != null && title !== '' && title !== 'New Chat';
export const buildCreatedInitialResponse = ({ initialResponse, userMessage, isRegenerate = false, }) => ({
    ...initialResponse,
    parentMessageId: isRegenerate && initialResponse.parentMessageId
        ? initialResponse.parentMessageId
        : userMessage.messageId,
    messageId: isRegenerate && initialResponse.messageId
        ? initialResponse.messageId
        : `${userMessage.messageId}_`,
    conversationId: userMessage.conversationId ?? initialResponse.conversationId,
});
export const isInitialNewConversationSubmission = ({ userMessage, }) => userMessage?.parentMessageId === Constants.NO_PARENT;
export const mergeRegenerateFinalMessages = ({ messages, responseMessage, initialResponseId, }) => {
    const finalMessages = [];
    let inserted = false;
...
export const getExistingConversationAbortMessages = ({ messages, currentMessages, regenerateMessages, isRegenerate = false, }) => {
    if (!isRegenerate) {
        return [...messages];
    }
...
const createErrorMessage = ({ errorMetadata, getMessages, submission, error, }) => {
    const currentMessages = getMessages();
    const latestMessage = currentMessages?.[currentMessages.length - 1];
    let errorMessage;
    const text = submission.initialResponse.text.length > 45 ? submission.initialResponse.text : '';
    const errorText = (errorMetadata?.text || text || error?.message) ??
...
export const getConvoTitle = ({ parentId, cache, currentTitle, conversationId, }) => {
...
export default function useEventHandlers({ setMessages, getMessages, setCompleted, isAddedRequest = false, setConversation, newConversation, runIndex = 0, setShowStopButton, }) {
    ...
    const lastConversationIdRef = useRef(paramId);
    const preserveSubagentAtomsForNewConvoIdRef = useRef(null);
    useEffect(() => {
        ...
        if (shouldResetSubagentAtomsOnConversationChange(previous, paramId, preserveNewConversationId)) {
            resetSubagentAtoms();
        }
    ...
    const messageHandler = useCallback((data, submission) => {
    ...
    const cancelHandler = useCallback((data, submission) => {
        ...
        if (isNewConvo) {
            cache.removeConversation(submission.conversation.conversationId);
        }
    ...
    }, [setMessages, setConversation, isAddedRequest, queryClient, setIsSubmitting, cache]);
    const syncHandler = useCallback((data, submission) => {
        ...
        chatStreamDispatch({
            type: 'MESSAGE_UPDATE',
            payload: { responseMessage: nextResponseMessage },
        });
        ...
        if (setConversation && !isAddedRequest) {
            ...
            if (requestMessage.parentMessageId === Constants.NO_PARENT) {
                ...
                cache.updateConversation(update.conversationId, (_c) => update, true);
            }
    ...
    }, [queryClient, setMessages, isAddedRequest, announcePolite, setConversation, setShowStopButton, chatStreamDispatch, cache]);
    const createdHandler = useCallback((data, submission) => {
        ...
        if (conversationId) {
            applyAgentTemplate({
                ...
                startupConfig: queryClient.getQueryData(startupConfigKey(true)),
            });
        }
    ...
    const titleHandler = useCallback((event) => {
        ...
        cache.setSingleConversation(conversationId, (convo) => convo ? { ...convo, title } : convo);
    ...
    }, [cache, location.pathname, setConversation, isAddedRequest]);
    const finalHandler = useCallback((data, submission) => {
        const { requestMessage, responseMessage, conversation, runMessages } = data;
        const { messages, conversation: submissionConvo, isRegenerate = false, isTemporary: _isTemporary = false, } = submission;
        ...
        try {
            ...
            if (data.earlyAbort) {
                ...
                const isExistingConvo = currentConvoId && currentConvoId !== Constants.NEW_CONVO && !isInitialNewConvo;
                if (isExistingConvo) {
                    ...
                    queryClient.setQueryData([QueryKeys.messages, currentConvoId], abortMessages);
            ...
            const hasNoResponse = responseMessage?.content?.[0]?.['text']?.value ===
            ...
            if (!conversation.conversationId && hasNoResponse) {
                ...
                if (isNewConvo && submissionConvo.conversationId) {
                    cache.removeConversation(submissionConvo.conversationId);
                }
            ...
            if (finalMessages.length > 0) {
                const currentMsgMap = new Map(currentMessages
            ...
            else if (isAssistantsEndpoint(submissionConvo.endpoint) &&
    ...
    const errorHandler = useCallback(({ data, submission }) => {
        ...
        const conversationId = userMessage.conversationId ?? submission.conversation?.conversationId ?? '';
        const setErrorMessages = (convoId, errorMessage) => {
            const finalMessages = [...messages, userMessage, errorMessage];
            setMessages(finalMessages);
            queryClient.setQueryData([QueryKeys.messages, convoId], finalMessages);
        };
    ...
    const abortConversation = useCallback(async (conversationId = '', submission, messages) => {
