...
import { Constants, LocalStorageKeys, QueryKeys, StepEvents, request, } from 'librechat-data-provider';
const mockSSEInstances = [];
jest.mock('sse.js', () => ({
...
const mockUseSetRecoilStateMock = jest.fn((atom) => {
...
function mockUseSetRecoilState(atom) {
...
jest.mock('~/data-provider', () => ({
    ...
    streamStatusQueryKey: (conversationId) => ['streamStatus', conversationId],
}));
...
const advanceRetryTimer = async (ms) => {
    await act(async () => {
...
describe('useResumableSSE - 404 error path', () => {
    ...
    const seedDraft = (conversationId) => {
    ...
    it('clears both TEXT and FILES drafts for new-convo when conversationId is absent', async () => {
        ...
        expect(localStorage.getItem(`${LocalStorageKeys.FILES_DRAFT}${Constants.NEW_CONVO}`)).toBeNull();
        unmount();
    });
    it('invalidates the stream conversation id on 404 for a new conversation', async () => {
        ...
        const allConversationWrites = mockSetQueryData.mock.calls.filter(([queryKey]) => Array.isArray(queryKey) && queryKey[0] === QueryKeys.allConversations);
        expect(allConversationWrites).toHaveLength(2);
        const removeUpdater = allConversationWrites[1][1];
    ...
    it('seeds sidebar and message caches for a new conversation once the stream id is known', async () => {
        ...
        expect(mockSetQueryData).toHaveBeenCalledWith([QueryKeys.conversation, 'stream-123'], expect.any(Function));
        expect(mockSetQueryData).toHaveBeenCalledWith([QueryKeys.messages, 'stream-123'], expect.arrayContaining([
        ...
        expect(mockSetQueryData).toHaveBeenCalledWith([QueryKeys.messages, Constants.NEW_CONVO], expect.arrayContaining([
            expect.objectContaining({ messageId: 'msg-1', conversationId: 'stream-123' }),
        ]));
    ...
    it('hydrates the submission conversation id before created handlers run', async () => {
        ...
        expect(mockCreatedHandler).toHaveBeenCalledWith(expect.any(Object), expect.objectContaining({
    ...
    it('queues run step events until created hydrates the submission', async () => {
        ...
        expect(mockStepHandler).toHaveBeenCalledWith(runStepEvent, expect.objectContaining({
    ...
    it('renders OAuth run step events before created while retaining replay after hydration', async () => {
        ...
        expect(mockStepHandler).toHaveBeenNthCalledWith(1, runStepEvent, expect.objectContaining({
            userMessage: expect.objectContaining({
                messageId: 'msg-1',
                conversationId: 'stream-123',
            }),
        }));
        expect(mockStepHandler).toHaveBeenNthCalledWith(2, runStepDeltaEvent, expect.objectContaining({
            userMessage: expect.objectContaining({
                messageId: 'msg-1',
                conversationId: 'stream-123',
            }),
        ...
        expect(mockStepHandler).toHaveBeenNthCalledWith(3, runStepEvent, expect.objectContaining({
            userMessage: expect.objectContaining({
                messageId: 'msg-1',
                conversationId: 'stream-123',
            }),
        }));
        expect(mockStepHandler).toHaveBeenNthCalledWith(4, runStepDeltaEvent, expect.objectContaining({
            userMessage: expect.objectContaining({
                messageId: 'msg-1',
                conversationId: 'stream-123',
            }),
    ...
    it('replays pre-created OAuth completion against the hydrated response id', async () => {
        ...
        expect(mockStepHandler).toHaveBeenNthCalledWith(3, completedEvent, expect.objectContaining({
        ...
        expect(mockCreatedHandler).toHaveBeenCalledWith(createdEvent, expect.objectContaining({
            userMessage: expect.objectContaining({
                messageId: 'server-user',
                parentMessageId: previousResponse.messageId,
            }),
        ...
        for (let callIndex = 4; callIndex <= 6; callIndex++) {
            expect(mockStepHandler).toHaveBeenNthCalledWith(callIndex, expect.any(Object), expect.objectContaining({
                userMessage: expect.objectContaining({
                    messageId: 'server-user',
                    parentMessageId: previousResponse.messageId,
                }),
    ...
    it('continues retrying chat start while the server reports startup readiness pending', async () => {
        jest.useFakeTimers();
        for (let i = 0; i < 9; i++) {
            request.post.mockRejectedValueOnce(serverNotReadyError('1'));
        }
        request.post.mockResolvedValueOnce({ streamId: 'stream-ready' });
        const submission = buildSubmission();
        const chatHelpers = buildChatHelpers();
        const { unmount } = renderHook(() => useResumableSSE(submission, chatHelpers));
        await flushMicrotasks();
    ...
    it('replays OAuth run step delta events from resume state sync', async () => {
        ...
        expect(mockStepHandler).toHaveBeenNthCalledWith(1, { event: StepEvents.ON_RUN_STEP, data: runStep }, expect.objectContaining({ userMessage: expect.objectContaining({ messageId: 'msg-1' }) }));
        expect(mockStepHandler).toHaveBeenNthCalledWith(2, replayEvent, expect.objectContaining({ userMessage: expect.objectContaining({ messageId: 'msg-1' }) }));
        unmount();
    });
    it('anchors preliminary OAuth replay events to the response message from resume state sync', async () => {
        ...
        expect(mockStepHandler).toHaveBeenNthCalledWith(1, { event: StepEvents.ON_RUN_STEP, data: runStep }, expect.objectContaining({
        ...
        expect(mockStepHandler).toHaveBeenNthCalledWith(2, replayEvent, expect.objectContaining({
            userMessage: expect.objectContaining({ messageId: 'follow-up-user' }),
            initialResponse: expect.objectContaining({
                messageId: 'follow-up-response',
                parentMessageId: 'follow-up-user',
    ...
    it('merges resumed user and response messages into loaded conversation history', async () => {
        ...
        expect(lastMessages[2]).toEqual(expect.objectContaining({
            messageId: 'follow-up-user',
            parentMessageId: 'original-response',
            text: 'Follow-up prompt',
        }));
        expect(lastMessages[3]).toEqual(expect.objectContaining({
            messageId: 'follow-up-response',
            parentMessageId: 'follow-up-user',
    ...
    it('uses the resumed submission for final events after sync', async () => {
        ...
        expect(mockFinalHandler).toHaveBeenCalledWith(finalPayload, expect.objectContaining({
            userMessage: expect.objectContaining({ messageId: 'follow-up-user' }),
            initialResponse: expect.objectContaining({
                messageId: 'follow-up-response',
                parentMessageId: 'follow-up-user',
    ...
    it('uses the resumed submission for final events after reconnecting from sync', async () => {
        ...
        expect(mockFinalHandler).toHaveBeenLastCalledWith(finalPayload, expect.objectContaining({
            userMessage: expect.objectContaining({ messageId: 'follow-up-user' }),
            initialResponse: expect.objectContaining({
                messageId: 'follow-up-response',
                parentMessageId: 'follow-up-user',
    ...
    it.each([undefined, 500, 503])('does not call errorHandler for responseCode %s (reconnect path)', async (responseCode) => {
        const submission = buildSubmission();
        const chatHelpers = buildChatHelpers();
        const { unmount } = renderHook(() => useResumableSSE(submission, chatHelpers));
        await act(async () => {
