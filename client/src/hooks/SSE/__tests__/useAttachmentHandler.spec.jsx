...
function setup() {
    ...
    return {
        handle: (data) => act(() => result.current({ data, submission })),
        get list() {
...
describe('useAttachmentHandler upsert-by-file_id', () => {
    ...
    it('keeps multiple distinct file_ids as separate entries', () => {
        ...
        expect(ctx.list.map((a) => a.file_id).sort()).toEqual([
            'fid-A',
            'fid-B',
        ]);
    });
    ...
    describe('filePreview cache eviction (cross-turn filename reuse)', () => {
        ...
        it('removes the same query key for the deferred preview update event too', () => {
            ...
            ctx.handle(makeAttachment({ status: 'pending' }));
            ctx.handle(makeAttachment({ status: 'ready', text: '<table></table>', textFormat: 'html' }));
            const previewEvictions = ctx.removeSpy.mock.calls.filter((c) => Array.isArray(c[0]) && c[0][0] === 'filePreview' && c[0][1] === 'fid-1');
        ...
        it('does not evict when the attachment carries no file_id', () => {
            ...
            ctx.handle(noFileId);
            const previewEvictions = ctx.removeSpy.mock.calls.filter((c) => Array.isArray(c[0]) && c[0][0] === 'filePreview');
        ...
        it('does NOT use invalidateQueries (would leave stale cache readable per round-3 review)', () => {
            ...
            const usedRemove = removeSpy.mock.calls.some((c) => Array.isArray(c[0]) && c[0][0] === 'filePreview');
            const usedInvalidate = invalidateSpy.mock.calls.some((c) => Array.isArray(c[0]) && c[0][0] === 'filePreview');
