...
jest.mock('~/data-provider', () => ({
    useStreamStatus: (conversationId, enabled) => mockUseStreamStatus(conversationId, enabled),
}));
...
function buildConversation(conversationId = CONVERSATION_ID) {
    return {
        conversationId,
        endpoint: 'agents',
    };
}
function buildUserMessage(conversationId = CONVERSATION_ID, messageId = USER_MESSAGE_ID) {
    return {
        text: 'Hello',
        sender: 'User',
        messageId,
...
function buildSubmission(conversationId) {
    return {
        messages: [],
        isTemporary: false,
...
function renderUseResumeOnLoad({ messages = [], getMessages: getMessagesOverride, submission = null, conversationId = CONVERSATION_ID, messagesLoaded = true, onSubmission, siblingIndexParentId, onSiblingIndex, }) {
    ...
    const wrapper = ({ children }) => (<RecoilRoot initializeState={initializeState}>
      <SubmissionProbe />
      <SiblingIndexProbe />
      {children}
    </RecoilRoot>);
...
describe('useResumeOnLoad', () => {
    ...
    it('restores model spec icon metadata on the resumed assistant placeholder', async () => {
        ...
        expect(observedSubmissions[observedSubmissions.length - 1]?.initialResponse).toEqual(expect.objectContaining({
            messageId: RESPONSE_MESSAGE_ID,
            sender: 'Spec Agent',
            iconURL: 'https://example.com/spec-icon.png',
            model: 'gpt-4.1',
