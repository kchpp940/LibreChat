...
import { buildCreatedInitialResponse, getExistingConversationAbortMessages, isInitialNewConversationSubmission, mergeRegenerateFinalMessages, } from '~/hooks/SSE/useEventHandlers';
describe('buildCreatedInitialResponse', () => {
    ...
    it('uses the created user message id for new turns', () => {
        expect(buildCreatedInitialResponse({
            initialResponse,
            userMessage,
            isRegenerate: false,
...
describe('isInitialNewConversationSubmission', () => {
    it('treats a root user message as an optimistic new chat', () => {
        expect(isInitialNewConversationSubmission({
            userMessage: {
                messageId: 'user-1',
...
describe('mergeRegenerateFinalMessages', () => {
    const userMessage = (messageId, parentMessageId = Constants.NO_PARENT) => ({
        messageId,
        parentMessageId,
        conversationId: 'conversation-1',
        isCreatedByUser: true,
    ...
    const assistantMessage = (messageId, parentMessageId) => ({
        messageId,
        parentMessageId,
        conversationId: 'conversation-1',
        isCreatedByUser: false,
    ...
    it('keeps the original branch siblings when a non-tail regenerate finalizes', () => {
        ...
        expect(mergeRegenerateFinalMessages({
    ...
    it('replaces the streamed preliminary response when it is present', () => {
        ...
        }).map((message) => message.messageId)).toEqual([rootUser.messageId, finalResponse.messageId]);
    });
});
describe('getExistingConversationAbortMessages', () => {
    ...
    it('keeps the existing non-regenerate abort rollback behavior', () => {
        ...
        expect(getExistingConversationAbortMessages({
            ...
            currentMessages,
        }).map(({ messageId }) => messageId)).toEqual(['user-1']);
    });
});
