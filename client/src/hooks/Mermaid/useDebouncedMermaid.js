 ...
 */
const isLikelyStreaming = (content) => {
 ...
 */
const looksComplete = (content) => {
    const lines = content.trim().split('\n');
    if (lines.length < 2) {
        return false;
    }
    // Has complete node connections (flowchart/graph)
    const hasConnections = /[A-Za-z]\w*(\[.*?\]|\(.*?\)|\{.*?\})?(\s*--+>?\s*|\s*-+\.\s*|\s*==+>?\s*)[A-Za-z]\w*/.test(content);
...
export const useDebouncedMermaid = ({ content, id, theme, delay = 500, minLength = 15, key = 0, }) => {
