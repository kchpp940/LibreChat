...
export default function useMentions({ assistantMap, includeAssistants, }) {
    ...
    const listMap = useAssistantListMap((res) => res.data.map(({ id, name, description }) => ({
        id,
        name,
        description,
    })));
    ...
    const agentQueryEnabled = hasAgentAccess &&
        interfaceConfig.modelSelect === true &&
        (includedEndpoints.size === 0 || includedEndpoints.has(EModelEndpoint.agents));
    const { data: agentsList = null, isLoading: isLoadingAgents } = useListAgentsQuery({ requiredPermission: PermissionBits.VIEW }, {
