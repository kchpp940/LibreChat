...
export const useAutoSave = ({ isSubmitting, conversationId: _conversationId, textAreaRef, setFiles, files, }) => {
    ...
    const restoreFiles = useCallback((id) => {
        const filesDraft = JSON.parse((localStorage.getItem(`${LocalStorageKeys.FILES_DRAFT}${id}`) ?? '') || '[]');
    ...
    const restoreText = useCallback((id) => {
        setValue('text', getDraft(id) ?? '');
    }, [setValue]);
    const saveText = useCallback((id) => {
    ...
    useEffect(() => {
        ...
        const handleInputFast = debounce((value) => setDraft({ id: conversationId, value }), 25);
        /** Use longer debounce for clearing empty values (850ms) to prevent accidental draft loss */
        const handleInputSlow = debounce((value) => setDraft({ id: conversationId, value }), 850);
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (prevConversationIdRef.current === Constants.PENDING_CONVO &&
                conversationId !== Constants.PENDING_CONVO &&
                conversationId.length > 3) {
                const pendingDraft = localStorage.getItem(`${LocalStorageKeys.TEXT_DRAFT}${Constants.PENDING_CONVO}`);
                ...
                const pendingFileDraft = localStorage.getItem(`${LocalStorageKeys.FILES_DRAFT}${Constants.PENDING_CONVO}`);
                if (pendingFileDraft) {
                    localStorage.setItem(`${LocalStorageKeys.FILES_DRAFT}${conversationId}`, pendingFileDraft);
                    localStorage.removeItem(`${LocalStorageKeys.FILES_DRAFT}${Constants.PENDING_CONVO}`);
                    const filesDraft = JSON.parse(pendingFileDraft || '[]');
                    if (filesDraft.length > 0) {
    ...
    useEffect(() => {
        ...
        if (fileIds.length === 0) {
            ...
            localStorage.setItem(`${LocalStorageKeys.FILES_DRAFT}${conversationId}`, JSON.stringify(fileIds));
        }
