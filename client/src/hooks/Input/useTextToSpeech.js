...
const useTextToSpeech = (props) => {
    ...
    const { generateSpeechLocal, cancelSpeechLocal, voices: voicesLocal, } = useTextToSpeechBrowser({ setIsSpeaking });
    const { generateSpeechExternal, cancelSpeech: cancelSpeechExternal, isLoading: isLoadingExternal, voices: voicesExternal, } = useTextToSpeechExternal({
        setIsSpeaking,
        audioRef,
        messageId,
        isLast,
    ...
    useEffect(() => {
        ...
        if (voices.length && typeof firstVoice === 'object') {
            const lastSelectedVoice = voices.find((v) => typeof v === 'object' ? v.value === voice : v === voice);
            if (lastSelectedVoice != null) {
                const currentVoice = typeof lastSelectedVoice === 'object' ? lastSelectedVoice.value : lastSelectedVoice;
    ...
    const handleMouseDown = () => {
        ...
        timerRef.current = window.setTimeout(() => {
            if (isMouseDownRef.current) {
                ...
                const parsedMessage = typeof messageContent === 'string' ? messageContent : parseTextParts(messageContent);
