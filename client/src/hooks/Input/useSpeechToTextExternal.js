...
const useSpeechToTextExternal = (setText, onTranscriptionComplete) => {
    ...
    const [audioMimeType, setAudioMimeType] = useState(() => getBestSupportedMimeType());
    const [minDecibels] = useRecoilState(store.decibelValue);
    const [autoSendText] = useRecoilState(store.autoSendText);
    const [languageSTT] = useRecoilState(store.languageSTT);
    const [speechToText] = useRecoilState(store.speechToText);
    ...
    const monitorSilence = (stream, stopRecording) => {
