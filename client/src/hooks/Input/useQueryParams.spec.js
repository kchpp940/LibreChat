...
describe('useQueryParams', () => {
    ...
    beforeEach(() => {
        ...
        dataProvider.useGetAgentByIdQuery.mockReturnValue({
            data: null,
            isLoading: false,
            error: null,
        });
        ...
        useRecoilValue.mockImplementation((atom) => {
            if (atom === store.modularChat)
                return false;
            if (atom === store.availableTools)
                return [];
            return null;
        });
        ...
        useChatFormContext.mockReturnValue(mockMethods);
        const mockSubmitMessage = jest.fn();
        useSubmitMessage.mockReturnValue({
            submitMessage: mockSubmitMessage,
        });
        const mockGetDefaultConversation = jest.fn().mockReturnValue({});
        useDefaultConvo.mockReturnValue(mockGetDefaultConversation);
        // Mock useAuthContext
    ...
    const setUrlParams = (params) => {
        ...
        useSearchParams.mockReturnValue([searchParams, jest.fn()]);
    };
    ...
    it('should process query parameters on initial render', () => {
        ...
        useChatFormContext.mockReturnValue({
        ...
        // Assert
        expect(mockSetValue).toHaveBeenCalledWith('text', 'hello world', expect.objectContaining({ shouldValidate: true }));
        const mockSetSearchParams = useSearchParams.mock.results[0].value[1];
