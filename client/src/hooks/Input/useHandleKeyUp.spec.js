...
const mockEndpoint = { current: 'openAI' };
...
jest.mock('~/store', () => ({
    showPromptsPopoverFamily: (idx) => `showPromptsPopoverFamily-${idx}`,
    showMentionPopoverFamily: (idx) => `showMentionPopoverFamily-${idx}`,
    showPlusPopoverFamily: (idx) => `showPlusPopoverFamily-${idx}`,
    showSkillsPopoverFamily: (idx) => `showSkillsPopoverFamily-${idx}`,
    effectiveEndpointByIndex: (idx) => `effectiveEndpointByIndex-${idx}`,
    atCommand: 'atCommand',
    plusCommand: 'plusCommand',
    slashCommand: 'slashCommand',
    dollarCommand: 'dollarCommand',
}));
jest.mock('~/hooks/Roles/useHasAccess', () => jest.fn(({ permissionType }) => {
...
jest.mock('~/hooks/Agents/useGetAgentsConfig', () => jest.fn(() => ({ agentsConfig: { capabilities: [] } })));
jest.mock('~/hooks/Agents/useAgentCapabilities', () => jest.fn(() => ({
...
const makeTextAreaRef = (value = '', selectionStart) => {
    const ref = {
        current: {
            value,
...
const makeKeyEvent = (key) => ({ key, preventDefault: jest.fn() });
const renderUseHandleKeyUp = (textAreaRef, overrides) => {
    const { result } = renderHook(() => useHandleKeyUp({
...
describe('useHandleKeyUp', () => {
    ...
    describe('non-command text', () => {
        it('does NOT trigger when text does not start with a command char', () => {
            ...
            const { handleKeyUp, setShowPromptsPopover, setShowMentionPopover, setShowPlusPopover } = renderUseHandleKeyUp(ref);
            act(() => handleKeyUp(makeKeyEvent('o')));
            expect(setShowPromptsPopover).not.toHaveBeenCalled();
            expect(setShowMentionPopover).not.toHaveBeenCalled();
            expect(setShowPlusPopover).not.toHaveBeenCalled();
