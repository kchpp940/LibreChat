...
import { clearModelForNonEphemeralAgent, removeUnavailableTools, getModelSpecIconURL, getConvoSwitchLogic, logger, } from '~/utils';
import { useDefaultConvo } from '~/hooks';
import store from '~/store';
export default function useSelectMention({ presets, modelSpecs, assistantsMap, returnHandlers, endpointsConfig, getConversation, newConversation, }) {
    ...
    const onSelectSpec = useCallback((spec) => {
        if (!spec) {
            return;
        }
        ...
        const { template, shouldSwitch, isNewModular, newEndpointType, isCurrentModular, isExistingConversation, } = getConvoSwitchLogic({
            newEndpoint,
            modularChat,
            conversation,
            endpointsConfig,
    ...
    const onSelectEndpoint = useCallback((_newEndpoint, kwargs = {}) => {
        const newEndpoint = _newEndpoint ?? '';
        if (!newEndpoint) {
            return;
        }
        const conversation = getConversation();
        const { shouldSwitch, isNewModular, isCurrentModular, isExistingConversation, newEndpointType, template, } = getConvoSwitchLogic({
            newEndpoint,
            modularChat,
            conversation,
            endpointsConfig,
        ...
        if (isExistingConversation && isCurrentModular && isNewModular && shouldSwitch) {
            ...
            /* We don't reset the latest message, only when changing settings mid-converstion */
            logger.info('conversation', 'Switching conversation to new endpoint/model (modular)', currentConvo);
            newConversation({
                template: currentConvo,
                preset: currentConvo,
                keepAddedConvos: true,
    ...
    const onSelectPreset = useCallback((_newPreset) => {
        if (!_newPreset) {
            return;
        }
        const conversation = getConversation();
    ...
    const onSelectMention = useCallback((option) => {
