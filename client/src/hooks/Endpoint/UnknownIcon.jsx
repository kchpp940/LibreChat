...
const knownEndpointComponents = new Set([KnownEndpoints.moonshot, KnownEndpoints.xai]);
export function getKnownEndpointAsset(endpoint) {
    if (!endpoint) {
        return '';
    }
    return knownEndpointAssets[endpoint.toLowerCase()] ?? '';
}
export function hasKnownEndpointIcon(endpoint) {
    if (!endpoint) {
        return false;
    }
    const currentEndpoint = endpoint.toLowerCase();
    return (getKnownEndpointAsset(currentEndpoint) !== '' || knownEndpointComponents.has(currentEndpoint));
}
...
const getKnownClass = ({ currentEndpoint, context = '', className, }) => {
...
function UnknownIcon({ className = '', endpoint: _endpoint, iconURL = '', context, }) {
    ...
    return (<img className={getKnownClass({
            currentEndpoint,
            context: context,
            className,
        })} src={assetPath} alt={`${currentEndpoint} Icon`}/>);
}
