...
import { useUpdatePresetMutation, useDeletePresetMutation, useGetPresetsQuery, } from '~/data-provider';
...
export default function usePresets(index = 0) {
    ...
    const setPresets = useCallback((presets) => {
        queryClient.setQueryData([QueryKeys.presets], presets);
    }, [queryClient]);
    ...
    const importPreset = (jsonPreset) => {
        createPresetMutation.mutate({ ...jsonPreset }, {
            onSuccess: () => {
                showToast({
    ...
    const onSelectPreset = (_newPreset) => {
        if (!_newPreset) {
            return;
        }
        ...
        const { shouldSwitch, isNewModular, newEndpointType, isCurrentModular, isExistingConversation, } = getConvoSwitchLogic({
    ...
    const onSetDefaultPreset = (preset, remove = false) => {
