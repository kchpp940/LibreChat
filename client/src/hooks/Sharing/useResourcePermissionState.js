...
import { useGetResourcePermissionsQuery, useUpdateResourcePermissionsMutation, } from 'librechat-data-provider/react-query';
 ...
 */
export const useResourcePermissionState = (resourceType, resourceDbId, isModalOpen = false) => {
    const config = getResourceConfig(resourceType);
    // Only enable the query if we have a valid resourceDbId
    const isValidResourceId = !!resourceDbId && resourceDbId.trim() !== '';
    const { data: permissionsData, isLoading: isLoadingPermissions, error: permissionsError, } = useGetResourcePermissionsQuery(resourceType, resourceDbId || '', {
    ...
    const currentShares = permissionsData?.principals?.map((principal) => ({
        type: principal.type,
        id: principal.id,
        name: principal.name,
    ...
    const [publicRole, setPublicRole] = useState(config?.defaultViewerRoleId);
