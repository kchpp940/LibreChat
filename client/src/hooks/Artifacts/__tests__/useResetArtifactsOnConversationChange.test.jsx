...
const buildConversation = (conversationId) => ({ conversationId });
const Harness = ({ handleRef }) => {
    ...
    // closure.
    const readArtifacts = useRecoilCallback(({ snapshot }) => () => snapshot.getLoadable(store.artifactsState).getValue(), []);
    const readCurrentId = useRecoilCallback(({ snapshot }) => () => snapshot.getLoadable(store.currentArtifactId).getValue(), []);
