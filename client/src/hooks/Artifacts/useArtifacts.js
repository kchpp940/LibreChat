...
export default function useArtifacts() {
    ...
    const { isSubmitting, latestMessageId, latestMessageText, conversationId } = useArtifactsContext();
    const artifacts = useRecoilValue(store.artifactsState);
    const resetArtifacts = useResetRecoilState(store.artifactsState);
    const resetCurrentArtifactId = useResetRecoilState(store.currentArtifactId);
    const [currentArtifactId, setCurrentArtifactId] = useRecoilState(store.currentArtifactId);
    const { orderedArtifactIds, latestAutoOpenArtifactId } = useMemo(() => {
        const ids = Object.keys(artifacts ?? {}).sort((a, b) => (artifacts?.[a]?.lastUpdateTime ?? 0) - (artifacts?.[b]?.lastUpdateTime ?? 0));
    ...
    const hasEnclosedArtifactRef = useRef(false);
    const hasAutoSwitchedToCodeRef = useRef(false);
    const lastRunMessageIdRef = useRef(null);
    ...
    useEffect(() => {
        ...
        if (currentId != null && orderedArtifactIds.includes(currentId))
            return;
