...
import { getKey, getProps, getTemplate, getArtifactFilename, languageForFilename, wrapAsFencedCodeBlock, TOOL_ARTIFACT_TYPES, } from '~/utils/artifacts';
import { getMarkdownFiles } from '~/utils/markdown';
import { getMermaidFiles } from '~/utils/mermaid';
export default function useArtifactProps({ artifact }) {
    ...
    const [fileKey, files] = useMemo(() => {
        ...
        if (type === TOOL_ARTIFACT_TYPES.DOCX ||
    ...
    const template = useMemo(() => getTemplate(artifact.type ?? '', artifact.language), [artifact.type, artifact.language]);
