 ...
 */
let mockActiveJobIds = [];
let mockTiming = 'immediate';
let mockQueriesResults = [];
let mockCapturedQueries = [];
...
jest.mock('~/utils', () => ({
    ...
    updateConvoInAllQueries: (...args) => mockUpdateConvoInAllQueries(...args),
}));
...
import { useTitleGeneration, genTitleQueryKey, queueTitleGeneration, markTitleGenerationProcessed, } from '../queries';
...
const isEligible = (id) => eligibleKeys().includes(JSON.stringify(genTitleQueryKey(id)));
beforeEach(() => {
    mockActiveJobIds = [];
...
describe('useTitleGeneration — result handling', () => {
    it('applies the fetched title to the conversation caches on success', () => {
        ...
        rerender();
        expect(mockSetQueryData).toHaveBeenCalledWith(['conversation', 'conv-ok'], expect.any(Function));
        expect(mockUpdateConvoInAllQueries).toHaveBeenCalled();
        const call = mockSetQueryData.mock.calls.find(([key]) => JSON.stringify(key) === JSON.stringify(['conversation', 'conv-ok']));
        const updater = call?.[1];
        expect(updater({ title: 'New Chat' })).toEqual(expect.objectContaining({ title: 'Quantum Chat' }));
    });
