...
import { getStableMessages, shouldPreserveMessagesOnNotFound, useGetMessagesByConvoId, } from '../queries';
...
function createWrapper(queryClient, initialEntry) {
    return function Wrapper({ children }) {
        return createElement(QueryClientProvider, { client: queryClient }, createElement(MemoryRouter, { initialEntries: [initialEntry] }, children));
    };
}
afterEach(() => {
    jest.clearAllMocks();
...
describe('shouldPreserveMessagesOnNotFound', () => {
    it('keeps cache when a transient 404 races with a pending assistant tail', () => {
        ...
        expect(shouldPreserveMessagesOnNotFound({
            pathname: '/c/convo-id',
            currentMessages,
            isStreaming: true,
        })).toBe(true);
...
describe('useGetMessagesByConvoId', () => {
    it('keeps cache during submitting cleanup when active job cache still marks the stream active', async () => {
        ...
        const mockGetMessagesByConvoId = dataService.getMessagesByConvoId;
        mockGetMessagesByConvoId.mockResolvedValue(serverMessages);
        const { result, unmount } = renderHook(() => useGetMessagesByConvoId(conversationId, undefined, { isStreaming: false }), { wrapper: createWrapper(queryClient, `/c/${conversationId}`) });
        await act(async () => {
    ...
    it('keeps cache when a transient 404 races with a regenerated pending assistant tail', async () => {
        ...
        const { result, unmount } = renderHook(() => useGetMessagesByConvoId(conversationId, { enabled: false }, { isStreaming: true }), { wrapper: createWrapper(queryClient, `/c/${conversationId}`) });
        await act(async () => {
            await result.current.refetch();
        });
        await waitFor(() => {
        ...
        expect(dataService.getMessagesByConvoId).toHaveBeenCalledWith(conversationId);
        expect(queryClient.getQueryData([QueryKeys.messages, conversationId])).toBe(currentMessages);
        expect(logger.warn).toHaveBeenCalledWith('messages', expect.stringContaining('returned 404 while cache has a pending assistant tail'), currentMessages);
        unmount();
    });
});
