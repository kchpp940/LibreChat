...
export const isDirectDownloadSource = (source) => source === FileSources.s3 || source === FileSources.cloudfront;
...
export const useFileDownload = (userId, file_id, options = {}) => {
    const queryClient = useQueryClient();
    return useQuery([QueryKeys.fileDownload, file_id, options.source ?? '', options.direct ?? true], async () => {
        if (!userId || !file_id) {
...
const consecutivePreviewErrors = new Map();
export const fetchFilePreview = async (fileId) => {
    try {
...
export const previewRefetchInterval = (data, query) => {
...
export const _resetPreviewErrorCounter = (fileId) => {
    if (fileId)
        consecutivePreviewErrors.delete(fileId);
    else
        consecutivePreviewErrors.clear();
};
/**
 ...
 */
export const useFilePreview = (file_id, config) => {
    return useQuery([QueryKeys.filePreview, file_id], () => fetchFilePreview(file_id ?? ''), {
        refetchOnWindowFocus: false,
        refetchOnReconnect: false,
