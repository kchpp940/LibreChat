...
jest.mock('librechat-data-provider', () => {
    ...
    return {
        ...
        dataService: {
            ...actual.dataService,
            getFilePreview: (...args) => mockGetFilePreview(...args),
        },
    };
});
import { PREVIEW_MAX_CONSECUTIVE_ERRORS, _resetPreviewErrorCounter, fetchFilePreview, isDirectDownloadSource, previewRefetchInterval, revokeDownloadURL, } from '../queries';
const q = (fileId) => ({ queryKey: ['filePreview', fileId] });
const FID = 'fid-test';
beforeEach(() => {
...
describe('previewRefetchInterval', () => {
    ...
    it('stops on terminal failed', () => {
        const data = { file_id: FID, status: 'failed', previewError: 'oops' };
        expect(previewRefetchInterval(data, q(FID))).toBe(false);
    });
