...
jest.mock('@hyperdx/browser', () => ({
    ...
    default: {
        ...
        setGlobalAttributes: (...args) => mockSetGlobalAttributes(...args),
    },
}));
...
describe('useRum', () => {
    ...
    it('initializes HyperDX public-token RUM with privacy defaults and safe attributes', async () => {
        ...
        expect(mockSetGlobalAttributes).not.toHaveBeenCalledWith(expect.objectContaining({ email: 'user@example.com' }));
    });
    ...
    it('initializes proxy RUM with the LibreChat bearer token for same-origin ingest', async () => {
        const fetchMock = jest.fn((_input, _init) => Promise.resolve({ ok: true, status: 200 }));
