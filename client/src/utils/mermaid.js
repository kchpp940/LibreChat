...
const getButtonStyles = (isDarkMode) => isDarkMode ? darkButtonStyles : lightButtonStyles;
 ...
 */
export const fixSubgraphTitleContrast = (svgElement) => {
    ...
    for (const cluster of clusters) {
        ...
        const textElements = cluster.querySelectorAll(':scope > g.cluster-label text, :scope > text, :scope > g > text');
...
const buildMermaidComponent = (mermaidTheme, bgColor, btnStyles) => dedent(`import React, { useEffect, useRef, useState, useCallback } from "react";
import {
  TransformWrapper,
  TransformComponent,
...
const wrapMermaidDiagram = (content) => {
...
export const getMermaidFiles = (content, isDarkMode = true) => {
...
export const sanitizeMermaidSvg = (svg) => new XMLSerializer().serializeToString(DOMPurify().sanitize(svg, {
