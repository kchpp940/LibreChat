...
export function getKey(type, language) {
    return `${type}${(language?.length ?? 0) > 0 ? `-${language}` : ''}`;
}
export function getArtifactFilename(type, language) {
    const key = getKey(type, language);
    return artifactFilename[key] ?? artifactFilename.default;
}
export function getTemplate(type, language) {
    const key = getKey(type, language);
    return artifactTemplate[key] ?? artifactTemplate.default;
}
 ...
 */
const extensionFromBasename = (base) => {
 ...
 */
const bareNameFromBasename = (base) => {
    if (base.includes('.')) {
        return '';
    }
...
const TOOL_ARTIFACT_TYPE_VALUES = new Set(Object.values(TOOL_ARTIFACT_TYPES));
export function isToolArtifactType(type) {
    return typeof type === 'string' && TOOL_ARTIFACT_TYPE_VALUES.has(type);
}
const lookupOwn = (record, key) => Object.prototype.hasOwnProperty.call(record, key) ? record[key] : undefined;
/**
 ...
 */
const PREVIEW_ONLY_ARTIFACT_TYPES = new Set([
    TOOL_ARTIFACT_TYPES.DOCX,
    TOOL_ARTIFACT_TYPES.SPREADSHEET,
    TOOL_ARTIFACT_TYPES.PRESENTATION,
]);
export function isPreviewOnlyArtifact(type) {
    if (type == null) {
        return false;
    }
    return PREVIEW_ONLY_ARTIFACT_TYPES.has(type);
}
/**
 ...
 */
export function isCodeOnlyArtifact(type) {
 ...
 */
const CODE_EXTENSION_TO_LANGUAGE = {
    // Web / scripting
    js: 'javascript',
    mjs: 'javascript',
    cjs: 'javascript',
 ...
 * on the `<code>` element.
 */
export function languageForFilename(filename, mime) {
 ...
 */
export function longestLeadingBacktickRun(source) {
    let max = 0;
 ...
 * markdown-shaped.
 */
export function wrapAsFencedCodeBlock(source, lang) {
...
const EXTENSION_TO_TOOL_ARTIFACT_TYPE = {
...
for (const ext of Object.keys(CODE_EXTENSION_TO_LANGUAGE)) {
    if (lookupOwn(EXTENSION_TO_TOOL_ARTIFACT_TYPE, ext) != null)
        continue;
    EXTENSION_TO_TOOL_ARTIFACT_TYPE[ext] = TOOL_ARTIFACT_TYPES.CODE;
}
const MIME_TO_TOOL_ARTIFACT_TYPE = {
    ...
    'application/vnd.openxmlformats-officedocument.wordprocessingml.document': TOOL_ARTIFACT_TYPES.DOCX,
    'application/vnd.oasis.opendocument.text': TOOL_ARTIFACT_TYPES.PLAIN_TEXT,
    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet': TOOL_ARTIFACT_TYPES.SPREADSHEET,
    ...
    'application/vnd.openxmlformats-officedocument.presentationml.presentation': TOOL_ARTIFACT_TYPES.PRESENTATION,
 ...
 */
const OFFICE_HTML_BUCKETS = new Set([
    TOOL_ARTIFACT_TYPES.DOCX,
    TOOL_ARTIFACT_TYPES.SPREADSHEET,
    TOOL_ARTIFACT_TYPES.PRESENTATION,
]);
export function detectArtifactTypeFromFile(attachment) {
    ...
    const byMime = lookupOwn(MIME_TO_TOOL_ARTIFACT_TYPE, normalizedMime) ??
 ...
 */
export const toolArtifactKey = (file) => `tool-artifact-${file.file_id ?? file.filename ?? file.filepath ?? 'unknown'}`;
/**
...
attachment, options) {
     ...
     * hook reads directly. */
    const language = type === TOOL_ARTIFACT_TYPES.CODE
