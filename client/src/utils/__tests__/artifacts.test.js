import { buildSandpackOptions, detectArtifactTypeFromFile, fileToArtifact, isCodeOnlyArtifact, isPreviewOnlyArtifact, languageForFilename, TOOL_ARTIFACT_TYPES, } from '../artifacts';
...
describe('buildSandpackOptions', () => {
    ...
    it('uses staticBundlerURL when template is static and config is provided', () => {
        const config = { staticBundlerURL: 'https://static.example.com' };
    ...
    it('uses bundlerURL when template is react-ts and config is provided', () => {
        const config = { bundlerURL: 'https://bundler.example.com' };
...
describe('detectArtifactTypeFromFile', () => {
    it.each([
        ...
        const textFormat = isOfficeBucket ? 'html' : undefined;
        expect(detectArtifactTypeFromFile({ filename, type: '', text: 'content', textFormat })).toBe(expected);
    });
    it.each([
        ...
        ['report.docx', TOOL_ARTIFACT_TYPES.DOCX, 'html'],
        ['data.csv', TOOL_ARTIFACT_TYPES.SPREADSHEET, 'html'],
        ['workbook.xlsx', TOOL_ARTIFACT_TYPES.SPREADSHEET, 'html'],
        ['slides.pptx', TOOL_ARTIFACT_TYPES.PRESENTATION, 'html'],
    ])('returns null when %s has no text (renderer needs real content)', (filename, _expected, textFormat) => {
        expect(detectArtifactTypeFromFile({ filename, type: '', text: '', textFormat })).toBeNull();
        expect(detectArtifactTypeFromFile({ filename, type: '', text: undefined, textFormat })).toBeNull();
    });
    it.each([
        ['readme.txt', TOOL_ARTIFACT_TYPES.PLAIN_TEXT],
        ['notes.md', TOOL_ARTIFACT_TYPES.MARKDOWN],
        ['notes.odt', TOOL_ARTIFACT_TYPES.PLAIN_TEXT],
    ])('still routes %s through the panel without text (deferred-extraction case for plain-text/markdown)', (filename, expected) => {
    ...
    it.each([
        ...
        expect(detectArtifactTypeFromFile({
            filename: 'noext',
            type: mime,
            text: '<html>x</html>',
            textFormat: 'html',
    ...
    ])('%s with no text and no textFormat returns null (preserves legacy download path)', (filename, type) => {
    ...
    it('falls back to MIME when the extension is unknown', () => {
        expect(detectArtifactTypeFromFile({ filename: 'noext', type: 'text/html', text: 'x' })).toBe(TOOL_ARTIFACT_TYPES.HTML);
        expect(detectArtifactTypeFromFile({ filename: 'noext', type: 'text/markdown', text: 'x' })).toBe(TOOL_ARTIFACT_TYPES.MARKDOWN);
    });
    it.each([
    ...
    it('returns null for unsupported types', () => {
        ...
        expect(detectArtifactTypeFromFile({ filename: 'doc.pdf', type: 'application/pdf', text: 'x' })).toBeNull();
        expect(detectArtifactTypeFromFile({ filename: 'photo.jpg', type: 'image/jpeg', text: 'binary' })).toBeNull();
    });
    it('does not route bare text/plain MIME without a recognized extension', () => {
        ...
        expect(detectArtifactTypeFromFile({ filename: 'unknown', type: 'text/plain', text: 'x' })).toBeNull();
        expect(detectArtifactTypeFromFile({ filename: '.env', type: 'text/plain', text: 'KEY=value' })).toBeNull();
    });
    describe('CODE bucket (programming-language source files)', () => {
        ...
        it.each([
            ...
            expect(detectArtifactTypeFromFile({ filename, type, text: 'x = 1' })).toBe(TOOL_ARTIFACT_TYPES.CODE);
        });
        ...
        it('keeps jsx/tsx on the React (sandpack) bucket, not CODE', () => {
            ...
            expect(detectArtifactTypeFromFile({ filename: 'App.jsx', type: '', text: 'x' })).toBe(TOOL_ARTIFACT_TYPES.REACT);
            expect(detectArtifactTypeFromFile({ filename: 'App.tsx', type: '', text: 'x' })).toBe(TOOL_ARTIFACT_TYPES.REACT);
        });
        it('does NOT route data formats to CODE (JSON / YAML / TOML / XML)', () => {
            ...
            expect(detectArtifactTypeFromFile({ filename: 'data.json', type: 'application/json', text: '{}' })).toBeNull();
            expect(detectArtifactTypeFromFile({
        ...
        it('does NOT route config dotfiles to CODE (.env / .ini)', () => {
            expect(detectArtifactTypeFromFile({ filename: 'app.env', type: 'text/plain', text: 'KEY=val' })).toBeNull();
            expect(detectArtifactTypeFromFile({
                filename: 'config.ini',
                type: 'text/plain',
                text: '[section]',
        ...
        it('allows empty text for CODE files (an empty Python file is still a Python file)', () => {
            expect(detectArtifactTypeFromFile({ filename: 'empty.py', type: 'text/x-python', text: '' })).toBe(TOOL_ARTIFACT_TYPES.CODE);
        });
        ...
        it.each([
            ...
            expect(detectArtifactTypeFromFile({ filename, type: '', text: 'FROM alpine' })).toBe(TOOL_ARTIFACT_TYPES.CODE);
        });
        it('still recognizes nested-path Dockerfile (path-preserving sanitizer output)', () => {
            ...
            expect(detectArtifactTypeFromFile({ filename: 'proj/Dockerfile', type: '', text: 'FROM alpine' })).toBe(TOOL_ARTIFACT_TYPES.CODE);
        });
        it('does not bare-name match files that DO have an extension (no double-match)', () => {
            ...
            expect(detectArtifactTypeFromFile({ filename: 'dockerfile.dev', type: '', text: 'x' })).toBeNull();
        });
        ...
        it.each(['constructor', '__proto__'])('does not classify inherited Object property "%s" from filename or MIME lookups', (key) => {
            expect(detectArtifactTypeFromFile({ filename: key, type: '', text: '<script>x</script>' })).toBeNull();
            expect(detectArtifactTypeFromFile({
        ...
        it('routes nested-path Dockerfile under dotted directory to CODE', () => {
            expect(detectArtifactTypeFromFile({ filename: 'pkg.v1/Dockerfile', type: '', text: 'FROM x' })).toBe(TOOL_ARTIFACT_TYPES.CODE);
        });
        it('still routes file extensions correctly under dotted directory', () => {
            expect(detectArtifactTypeFromFile({ filename: 'pkg.v1/main.go', type: '', text: 'package main' })).toBe(TOOL_ARTIFACT_TYPES.CODE);
            expect(detectArtifactTypeFromFile({
...
describe('fileToArtifact', () => {
    ...
    it('returns null for unsupported types so callers can fall through', () => {
        ...
        expect(fileToArtifact({ ...baseFile, filename: 'doc.pdf', type: 'application/pdf' })).toBeNull();
    });
    ...
    it('preserves an empty string as legitimate content (does not fall through to placeholder)', () => {
        ...
        const artifact = fileToArtifact({ ...baseFile, filename: 'empty.md', type: 'text/markdown', text: '' }, { placeholder: '_should not appear_' });
        expect(artifact.content).toBe('');
    });
    ...
    it('skips re-classification when preClassifiedType is provided', () => {
        ...
        const artifact = fileToArtifact({ ...baseFile, filename: 'index.html', type: 'text/html', text: 'x' }, { preClassifiedType: TOOL_ARTIFACT_TYPES.PLAIN_TEXT });
        expect(artifact.type).toBe(TOOL_ARTIFACT_TYPES.PLAIN_TEXT);
    });
    it('rejects a runtime bogus preClassifiedType before constructing an artifact', () => {
        const artifact = fileToArtifact({ ...baseFile, filename: 'constructor', type: '', text: '<script>x</script>' }, { preClassifiedType: Object });
        expect(artifact).toBeNull();
    });
    it.each([[TOOL_ARTIFACT_TYPES.HTML], [TOOL_ARTIFACT_TYPES.REACT], [TOOL_ARTIFACT_TYPES.MERMAID]])('returns null when preClassifiedType=%s is paired with empty text (defense in depth)', (preClassifiedType) => {
        ...
        const artifact = fileToArtifact({ ...baseFile, filename: 'whatever', type: '', text: '' }, { preClassifiedType });
        expect(artifact).toBeNull();
    });
    ...
    it('falls back to filename when file_id is missing', () => {
        const artifact = fileToArtifact({ ...baseFile, file_id: undefined });
...
describe('isPreviewOnlyArtifact', () => {
    ...
    it.each([[null], [undefined], [''], ['application/pdf'], ['text/plain'], ['some/random-type']])('returns false for non-artifact type %s', (type) => {
...
describe('isCodeOnlyArtifact', () => {
    ...
    it.each([[null], [undefined], [''], ['application/pdf'], ['text/plain'], ['some/random-type']])('returns false for non-code-only type %s', (type) => {
