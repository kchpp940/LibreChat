...
const createModelSpec = (name, overrides = {}) => ({
    name,
    label: name,
    preset: {
...
const createStartupConfig = (list, { prioritize = true, modelSelect = true, addedEndpoints, } = {}) => ({
    interface: {
        modelSelect,
    },
    modelSpecs: {
...
const fullEndpointsConfig = {
    [EModelEndpoint.openAI]: { order: 0 },
    [EModelEndpoint.agents]: { order: 1 },
};
const agentsOnlyEndpointsConfig = {
    [EModelEndpoint.agents]: { order: 0 },
};
const writeLastModel = (endpoint, model) => {
...
const persistAppliedSpec = (spec) => {
    localStorage.setItem(LocalStorageKeys.LAST_SPEC, spec.name);
    writeLastModel(spec.preset.endpoint, spec.preset.model);
    localStorage.setItem(`${LocalStorageKeys.LAST_CONVO_SETUP}_0`, JSON.stringify({
...
const persistAgentSelection = (agentId) => {
...
const persistEphemeralSelection = (endpoint, model) => {
    writeLastModel(endpoint, model);
    localStorage.setItem(`${LocalStorageKeys.LAST_CONVO_SETUP}_0`, JSON.stringify({ endpoint, model, spec: null }));
};
describe('getDefaultModelSpec', () => {
    ...
    it('does not apply the soft default when a prior model selection exists', () => {
        const softSpec = createModelSpec('soft-spec', { softDefault: true });
        localStorage.setItem(LocalStorageKeys.LAST_MODEL, JSON.stringify({ [EModelEndpoint.openAI]: 'gpt-4o' }));
    ...
    describe('soft default auto-application residue', () => {
        ...
        it('stays soft after its own application is persisted (prioritized config)', () => {
            ...
            const result = getDefaultModelSpec(createStartupConfig([otherSpec, softSpec]), fullEndpointsConfig);
            expect(result).toEqual({ softDefault: softSpec });
        });
        it('stays soft after its own application is persisted (modelSelect config)', () => {
            persistAppliedSpec(softSpec);
            const result = getDefaultModelSpec(createStartupConfig([otherSpec, softSpec], { prioritize: false }), fullEndpointsConfig);
            expect(result).toEqual({ softDefault: softSpec });
        });
        ...
        it('yields to a stored agent even when it matches the soft default agent spec', () => {
            ...
            const result = getDefaultModelSpec(createStartupConfig([softAgentSpec], { prioritize: false }), fullEndpointsConfig);
            expect(result).toBeUndefined();
        });
        it('yields to a stored model selection matching the soft default preset', () => {
            localStorage.setItem(LocalStorageKeys.LAST_MODEL, JSON.stringify({ [softSpec.preset.endpoint]: softSpec.preset.model }));
            const result = getDefaultModelSpec(createStartupConfig([otherSpec, softSpec], { prioritize: false }), fullEndpointsConfig);
            expect(result).toBeUndefined();
        });
    ...
    describe('no ephemeral endpoint → model options (edge case)', () => {
        ...
        it('applies the soft default despite a stored agent when modelSelect is disabled', () => {
            persistAgentSelection('agent_abc');
            const result = getDefaultModelSpec(createStartupConfig([otherSpec, softSpec], { modelSelect: false }), fullEndpointsConfig);
            expect(result).toEqual({ softDefault: softSpec });
        });
        it('applies the soft default despite a stored agent when addedEndpoints only includes agents', () => {
            persistAgentSelection('agent_abc');
            const result = getDefaultModelSpec(createStartupConfig([otherSpec, softSpec], {
                prioritize: false,
        ...
        it('applies the soft default despite a stored agent when agents is the only endpoint', () => {
            persistAgentSelection('agent_abc');
            const result = getDefaultModelSpec(createStartupConfig([otherSpec, softSpec], { prioritize: false }), agentsOnlyEndpointsConfig);
            expect(result).toEqual({ softDefault: softSpec });
        });
        ...
        it('keeps the selection gate while the endpoints config has not loaded', () => {
            persistAgentSelection('agent_abc');
            const result = getDefaultModelSpec(createStartupConfig([otherSpec, softSpec], { prioritize: false }), undefined);
            expect(result).toBeUndefined();
        });
