import { fixSubgraphTitleContrast, sanitizeMermaidSvg, artifactFlowchartConfig, inlineFlowchartConfig, getMermaidFiles, } from '~/utils/mermaid';
const makeSvg = (clusters) => {
    const parser = new DOMParser();
    const doc = parser.parseFromString(`<svg xmlns="http://www.w3.org/2000/svg">${clusters}</svg>`, 'image/svg+xml');
...
describe('mermaid config', () => {
    ...
    describe('fixSubgraphTitleContrast', () => {
        it('darkens title text on light subgraph backgrounds (fill attribute)', () => {
            const svg = makeSvg('<g class="cluster"><rect fill="#FFF9C4"/><g class="cluster-label"><text fill="#E0E0E0">Title</text></g></g>');
        ...
        it('darkens title text on light subgraph backgrounds (inline style fill)', () => {
            const svg = makeSvg('<g class="cluster"><rect style="fill: #FFF9C4; stroke: #F9A825"/><g class="cluster-label"><text>Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
            expect(svg.querySelector('text').getAttribute('style')).toContain('fill: #1a1a1a');
        });
        it('lightens title text on dark subgraph backgrounds', () => {
            const svg = makeSvg('<g class="cluster"><rect fill="#1f2020"/><g class="cluster-label"><text fill="#222222">Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
        ...
        it('leaves title text alone when contrast is already good', () => {
            const svg = makeSvg('<g class="cluster"><rect fill="#FFF9C4"/><g class="cluster-label"><text fill="#333333">Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
            expect(svg.querySelector('text').getAttribute('style')).toBeNull();
        });
        it('skips clusters without a rect', () => {
            const svg = makeSvg('<g class="cluster"><g class="cluster-label"><text fill="#E0E0E0">Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
            expect(svg.querySelector('text').getAttribute('style')).toBeNull();
        });
        it('skips clusters with non-hex fills', () => {
            const svg = makeSvg('<g class="cluster"><rect fill="rgb(255,249,196)"/><g class="cluster-label"><text fill="#E0E0E0">Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
            expect(svg.querySelector('text').getAttribute('style')).toBeNull();
        });
        it('sets dark fill when text has no explicit fill on light backgrounds', () => {
            const svg = makeSvg('<g class="cluster"><rect style="fill:#FFF9C4"/><g class="cluster-label"><text>Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
            expect(svg.querySelector('text').getAttribute('style')).toContain('fill: #1a1a1a');
        });
        it('preserves existing text style when appending fill override', () => {
            const svg = makeSvg('<g class="cluster"><rect fill="#FFF9C4"/><g class="cluster-label"><text style="font-size: 14px" fill="#E0E0E0">Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
        ...
        it('handles 3-char hex shorthand fills', () => {
            const svg = makeSvg('<g class="cluster"><rect fill="#FFC"/><g class="cluster-label"><text fill="#EEE">Title</text></g></g>');
            fixSubgraphTitleContrast(svg);
            expect(svg.querySelector('text').getAttribute('style')).toContain('fill: #1a1a1a');
        });
    ...
    describe('sanitizeMermaidSvg', () => {
        ...
        it('preserves foreignObject and its HTML children', () => {
            const svg = wrap('<foreignObject width="100" height="50"><div xmlns="http://www.w3.org/1999/xhtml"><p>Hello</p></div></foreignObject>');
            const result = sanitizeMermaidSvg(svg);
            expect(result).toContain('<foreignObject');
            expect(result).toContain('<p>Hello</p>');
        });
        it('serializes <br> as self-closing <br /> for XML compatibility', () => {
            const svg = wrap('<foreignObject width="100" height="50"><div xmlns="http://www.w3.org/1999/xhtml"><p>Line1<br/>Line2</p></div></foreignObject>');
            const result = sanitizeMermaidSvg(svg);
        ...
        it('strips script tags inside foreignObject HTML', () => {
            const svg = wrap('<foreignObject width="100" height="50"><div xmlns="http://www.w3.org/1999/xhtml"><script>alert("xss")</script><p>Safe</p></div></foreignObject>');
            const result = sanitizeMermaidSvg(svg);
            expect(result).not.toContain('<script');
            expect(result).toContain('Safe');
        });
        it('strips javascript: URLs in foreignObject HTML', () => {
            const svg = wrap('<foreignObject width="100" height="50"><div xmlns="http://www.w3.org/1999/xhtml"><a href="javascript:alert(1)">click</a></div></foreignObject>');
            const result = sanitizeMermaidSvg(svg);
        ...
        it('preserves styled span and class names inside foreignObject', () => {
            const svg = wrap('<foreignObject width="150" height="48"><div xmlns="http://www.w3.org/1999/xhtml" style="display: table-cell;"><span class="nodeLabel"><p>Node text<br/>second line</p></span></div></foreignObject>');
            const result = sanitizeMermaidSvg(svg);
