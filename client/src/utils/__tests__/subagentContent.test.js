...
describe('aggregateSubagentContent', () => {
    ...
    it('creates a TOOL_CALL part for each unique tool_call id on run_step', () => {
        ...
        expect(parts[0].tool_call.name).toBe('calculator');
        expect(parts[1].tool_call.name).toBe('web_search');
        expect(parts[0].tool_call.progress).toBe(0.1);
    });
    ...
    it('interleaves tool calls between text parts in order', () => {
        ...
        expect(parts[0].type).toBe(ContentTypes.TEXT);
        expect(parts[1].type).toBe(ContentTypes.TOOL_CALL);
        expect(parts[2].type).toBe(ContentTypes.TEXT);
        expect(parts[2].text).toBe('The answer is 4.');
    });
    ...
    it('preserves chronological order when reasoning and text arrive interleaved (ordering regression)', () => {
        ...
        expect(parts).toHaveLength(3);
        expect(parts[0].type).toBe(ContentTypes.THINK);
        expect(parts[0].think).toBe('Let me think.');
        expect(parts[1].type).toBe(ContentTypes.TEXT);
        expect(parts[1].text).toBe('Sure!');
        expect(parts[2].type).toBe(ContentTypes.TOOL_CALL);
    });
    it('handles repeated reasoning → text → reasoning flows across a turn', () => {
        ...
        expect(parts[0].think).toBe('Pre-thinking.');
        expect(parts[1].text).toBe('First draft.');
        expect(parts[2].think).toBe('Post-thinking.');
        expect(parts[3].text).toBe('Final.');
    });
...
describe('buildSubagentTickerLines', () => {
    it('returns empty array when no meaningful events are present', () => {
        expect(buildSubagentTickerLines([
            makeEvent({ phase: 'start' }),
            makeEvent({ phase: 'run_step_delta' }),
            makeEvent({ phase: 'stop' }),
        ])).toEqual([]);
    ...
    it('truncates long tool output to a 48-char head snippet', () => {
        ...
        const snippet = lines[0].outputSnippet ?? '';
    ...
    it('closes the opposite cursor on text↔reasoning phase switches (chronological order)', () => {
        ...
        expect(lines[0].body).toBe('Plan step.');
        expect(lines[1].body).toBe('Answer part 1.');
        expect(lines[2].body).toBe('Refined plan.');
        expect(lines[3].body).toBe('Answer part 2.');
    });
