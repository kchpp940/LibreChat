...
describe('getToolDisplayLabel', () => {
    ...
    it('returns the friendly translation key for a known native tool', () => {
        ...
        expect(getToolDisplayLabel('web_search', identityLocalize)).toBe(TOOL_FRIENDLY_NAME_KEYS.web_search);
    });
    it('returns the code translation key for bash PTC tool calls', () => {
        expect(getToolDisplayLabel(Constants.BASH_PROGRAMMATIC_TOOL_CALLING, identityLocalize)).toBe(TOOL_FRIENDLY_NAME_KEYS[Constants.BASH_PROGRAMMATIC_TOOL_CALLING]);
    });
    it('returns the code translation key for bash_tool calls', () => {
        expect(getToolDisplayLabel('bash_tool', identityLocalize)).toBe(TOOL_FRIENDLY_NAME_KEYS.bash_tool);
    });
