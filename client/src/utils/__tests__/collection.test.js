import { addData, findPage, deleteData, updateFields, updateFieldsInPlace, normalizeData, getRecordByProperty, } from '../collection';
function makeInfiniteData(pages) {
    return {
        pages,
        pageParams: pages.map((_, i) => i),
    };
}
function makeItem(id, name, value) {
...
describe('updateFieldsInPlace', () => {
    it('updates matching item fields without changing its position', () => {
        ...
        const result = updateFieldsInPlace(data, { id: 'b', name: 'BetaUpdated' }, 'items', 'id');
    ...
    it('does NOT move the updated item to page 0 (unlike updateFields)', () => {
        ...
        const result = updateFieldsInPlace(data, { id: 'c', name: 'GammaUpdated' }, 'items', 'id');
    ...
    it('does NOT set updatedAt on the updated item', () => {
        const data = makeInfiniteData([{ items: [makeItem('a', 'Alpha')] }]);
        const result = updateFieldsInPlace(data, { id: 'a', name: 'AlphaChanged' }, 'items', 'id');
    ...
    it('returns the data unchanged when the item is not found', () => {
        const data = makeInfiniteData([{ items: [makeItem('a', 'Alpha')] }]);
        const result = updateFieldsInPlace(data, { id: 'z', name: 'Missing' }, 'items', 'id');
        expect(result.pages[0].items).toHaveLength(1);
        expect(result.pages[0].items[0]).toMatchObject({ id: 'a', name: 'Alpha' });
    });
    it('handles items across multiple pages, updating the correct page', () => {
        ...
        const result = updateFieldsInPlace(data, { id: 'b', name: 'BetaNew' }, 'items', 'id');
        expect(result.pages[0].items[0]).toMatchObject({ id: 'a', name: 'Alpha' });
    ...
    it('handles an empty pages array without throwing', () => {
        const data = makeInfiniteData([]);
        expect(() => updateFieldsInPlace(data, { id: 'a', name: 'Alpha' }, 'items', 'id')).not.toThrow();
    });
    ...
    it('updates the first item in a page correctly', () => {
        ...
        const result = updateFieldsInPlace(data, { id: 'first', name: 'FirstUpdated' }, 'items', 'id');
    ...
    it('updates the last item in a page correctly', () => {
        ...
        const result = updateFieldsInPlace(data, { id: 'last', name: 'LastUpdated' }, 'items', 'id');
    ...
    it('preserves pageParams on the returned data', () => {
        ...
        const result = updateFieldsInPlace(data, { id: 'a', name: 'AlphaNew' }, 'items', 'id');
...
describe('addData', () => {
    it('unshifts new item onto page 0 when item does not already exist', () => {
        ...
        const result = addData(data, 'items', newItem, (page) => page.items.findIndex((i) => i.id === newItem.id));
    ...
    it('calls updateData instead when item already exists', () => {
        ...
        const result = addData(data, 'items', { id: 'a', name: 'AlphaUpdated' }, (page) => page.items.findIndex((i) => i.id === 'a'));
...
describe('deleteData', () => {
    it('removes the matching item from its page', () => {
        const data = makeInfiniteData([{ items: [makeItem('a', 'Alpha'), makeItem('b', 'Beta')] }]);
        const result = deleteData(data, 'items', (page) => page.items.findIndex((i) => i.id === 'a'));
        expect(result.pages[0].items).toHaveLength(1);
        expect(result.pages[0].items[0]).toMatchObject({ id: 'b', name: 'Beta' });
    });
    it('leaves data unchanged when item is not found', () => {
        const data = makeInfiniteData([{ items: [makeItem('a', 'Alpha')] }]);
        const result = deleteData(data, 'items', (page) => page.items.findIndex((i) => i.id === 'z'));
        expect(result.pages[0].items).toHaveLength(1);
    });
});
// ---------------------------------------------------------------------------
