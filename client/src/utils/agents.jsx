 ...
 */
export const getAgentAvatarUrl = (agent) => {
    if (!agent?.avatar) {
        return null;
    }
...
const LazyAgentAvatar = ({ url, alt, imgClass, }) => {
    const [isLoaded, setIsLoaded] = useState(false);
    useEffect(() => {
        setIsLoaded(false);
    }, [url]);
    return (<>
      <img src={url} alt={alt} className={imgClass} loading="lazy" onLoad={() => setIsLoaded(true)} onError={() => setIsLoaded(false)} style={{
 ...
 */
export const renderAgentAvatar = (agent, options = {}) => {
    ...
    if (avatarUrl) {
        return (<div className={`relative flex items-center justify-center ${sizeClasses[size]} ${className}`}>
        <LazyAgentAvatar url={avatarUrl} alt={`${agent?.name || 'Agent'} avatar`} imgClass={`${sizeClasses[size]} rounded-full object-cover shadow-lg ${borderClasses}`}/>
      </div>);
    }
    return (<div className={`relative flex items-center justify-center ${sizeClasses[size]} ${className}`}>
 ...
 */
export const getContactDisplayName = (agent) => {
    if (!agent)
        return null;
