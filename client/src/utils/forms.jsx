...
import { FileSources, alternateName, EModelEndpoint, EToolResources, LocalStorageKeys, defaultAgentFormValues, } from 'librechat-data-provider';
/**
 ...
 **/
export const createDropdownSetter = (setValue) => {
    return (value) => {
        if (!value) {
            setValue('');
            return;
 ...
 **/
export const createProviderOption = (provider) => ({
...
export const processAgentOption = ({ agent: _agent, fileMap, }) => {
    ...
    const agent = {
        ...
        context_files: context_files.length > 0 ? [] : undefined,
    ...
    const handleFile = ({ file_id, tool_resource, list, }) => {
        const file = fileMap[file_id];
        const source = tool_resource === EToolResources.file_search
    ...
    if (agent.context_files && context_files.length > 0) {
        context_files.forEach((file_id) => handleFile({
            file_id,
    ...
    if (agent.knowledge_files && _agent?.tool_resources?.file_search?.file_ids) {
        _agent.tool_resources.file_search.file_ids.forEach((file_id) => handleFile({
            file_id,
    ...
    if (agent.code_files && _agent?.tool_resources?.execute_code?.file_ids) {
        _agent.tool_resources.execute_code.file_ids.forEach((file_id) => handleFile({ file_id, list: agent.code_files, tool_resource: EToolResources.execute_code }));
    }
    return agent;
};
