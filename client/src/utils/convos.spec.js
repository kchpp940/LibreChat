...
import { dateKeys, storeEndpointSettings, addConversationToInfinitePages, updateInfiniteConvoPage, findConversationInInfinite, removeConvoFromInfinitePages, groupConversationsByDate, updateConvoFieldsInfinite, addConvoToAllQueries, upsertConvoInAllQueries, updateConvoInAllQueries, removeConvoFromAllQueries, addConversationToAllConversationsQueries, } from './convos';
...
describe('Conversation Utilities', () => {
    describe('groupConversationsByDate', () => {
        ...
        it('sorts conversations by month correctly', () => {
            ...
            const sorted = [...conversationsIn2023].sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
        ...
        it('correctly groups and sorts conversations for every month of the year', () => {
            ...
            const totalGroupedConversations = grouped.reduce((total, [_, convos]) => total + convos.length, 0);
    ...
    describe('normalizeConversationData', () => {
        ...
        it('returns empty data when there is no data', () => {
            const normalizedData = normalizeData({ pages: [{ conversations: [], pageNumber: 1, pageSize: 5, pages: 1 }], pageParams: [] }, 'conversations', 5);
            expect(normalizedData.pages[0].conversations).toHaveLength(0);
        });
        it('does not normalize data when not needed', () => {
            const normalizedData = normalizeData({ pages: [{ conversations: ['1'], pageNumber: 1, pageSize: 5, pages: 1 }], pageParams: [] }, 'conversations', 5);
    ...
    describe('InfiniteData helpers', () => {
        const makeConversation = (id, updatedAt) => ({
            conversationId: id,
            updatedAt: updatedAt || new Date().toISOString(),
        });
        const makePage = (conversations, nextCursor = null) => ({
            conversations,
            nextCursor,
        });
        ...
        describe('updateInfiniteConvoPage', () => {
            it('updates a conversation in InfiniteData', () => {
                ...
                const updater = (c) => ({ ...c, updatedAt: '2024-01-01T00:00:00Z' });
        ...
        describe('updateConvoFieldsInfinite', () => {
            it('updates fields and bumps to front if keepPosition is false', () => {
                ...
                const updated = updateConvoFieldsInfinite(data, { conversationId: '2', title: 'new' }, false);
            ...
            it('updates fields and keeps position if keepPosition is true', () => {
                ...
                const updated = updateConvoFieldsInfinite(data, { conversationId: '2', title: 'stay' }, true);
            ...
            it('returns original data if conversation not found', () => {
                ...
                const updated = updateConvoFieldsInfinite(data, { conversationId: 'notfound', title: 'x' }, false);
                expect(updated).toEqual(data);
            });
            it('returns original data if data is undefined', () => {
                expect(updateConvoFieldsInfinite(undefined, { conversationId: '1', title: 'x' }, false)).toBeUndefined();
            });
        });
        describe('storeEndpointSettings', () => {
            ...
            it('stores model for endpoint', () => {
                ...
                storeEndpointSettings(conversation);
            ...
            it('does nothing if endpoint is missing', () => {
                storeEndpointSettings({ conversationId: '1', model: 'x' });
                expect(localStorage.getItem('lastModel')).toBeNull();
            });
        });
        describe('QueryClient helpers', () => {
            ...
            it('upsertConvoInAllQueries moves an existing conversation to the top once', () => {
                const convoC = { conversationId: 'c', updatedAt: '2024-01-03T12:00:00Z' };
                queryClient.setQueryData(['allConversations'], {
            ...
            it('updateConvoInAllQueries with moveToTop moves convo to front and updates updatedAt', () => {
                ...
                expect(new Date(data.pages[0].conversations[0].updatedAt).getTime()).toBeGreaterThanOrEqual(new Date(before).getTime());
            ...
            it('updateConvoInAllQueries with moveToTop from second page', () => {
                const convoC = { conversationId: 'c', updatedAt: '2024-01-03T12:00:00Z' };
                const convoD = { conversationId: 'd', updatedAt: '2024-01-04T12:00:00Z' };
                queryClient.setQueryData(['allConversations'], {
                    pages: [
            ...
            it('updateConvoInAllQueries with moveToTop returns original data if convo not found', () => {
                const dataBefore = queryClient.getQueryData(['allConversations']);
                updateConvoInAllQueries(queryClient, 'nonexistent', (c) => ({ ...c, model: 'gpt-4' }), true);
                const dataAfter = queryClient.getQueryData(['allConversations']);
            ...
            it('addConversationToAllConversationsQueries works with multiple pages', () => {
                ...
                const mainData = queryClient.getQueryData(['allConversations']);
                const otherData = queryClient.getQueryData([
                    'allConversations',
                    'other',
                ]);
