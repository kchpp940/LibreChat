import { TextPaths, FilePaths, CodePaths, AudioPaths, VideoPaths, SheetPaths, } from '@librechat/client';
import { megabyte, QueryKeys, inferMimeType, excelMimeTypes, EToolResources, fileConfig as defaultFileConfig, } from 'librechat-data-provider';
 ...
 */
export function formatDate(dateString, isSmallScreen = false) {
    if (!dateString) {
        return '';
    }
 ...
 */
export function addFileToCache(queryClient, newfile) {
    const currentFiles = queryClient.getQueryData([QueryKeys.files]);
    if (!currentFiles) {
...
export const validateFiles = ({ files, fileList, setError, endpointFileConfig, toolResource, fileConfig, }) => {
    const { fileLimit, fileSizeLimit, totalSizeLimit, supportedMimeTypes, disabled } = endpointFileConfig;
    ...
    const combinedFilesInfo = [
        ...existingFiles.map((file) => `${file.file?.name ?? file.filename}-${file.size}-${file.type?.split('/')[0] ?? 'file'}`),
...
export function sortPagesByRelevance(pages, pageRelevance) {
