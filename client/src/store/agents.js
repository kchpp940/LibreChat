...
export function useUpdateEphemeralAgent() {
    const updateEphemeralAgent = useRecoilCallback(({ set }) => (convoId, agent) => {
...
export function useApplyNewAgentTemplate() {
    const applyTemplate = useRecoilCallback(({ snapshot, set }) => async (targetId, _sourceId = Constants.NEW_CONVO, ephemeralAgentState) => {
        ...
        try {
            ...
            const agentTemplate = ephemeralAgentState ?? (await snapshot.getPromise(ephemeralAgentByConvoId(sourceId)));
            ...
            if (agentTemplate) {
                ...
                logger.warn('agents', `Agent template from "${sourceId}" is null or unset. Setting agent for "${targetId}" to null.`);
            ...
            logger.error('agents', `Error applying agent template from "${sourceId}" to "${targetId}":`, error);
            set(ephemeralAgentByConvoId(targetId), null);
        }
    }, []);
...
export function useGetEphemeralAgent() {
    const getEphemeralAgent = useRecoilCallback(({ snapshot }) => (conversationId) => {
