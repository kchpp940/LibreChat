export function createInitialState(runIndex) {
    const now = Date.now();
    return {
        status: 'idle',
        runIndex,
...
const isRunningStatus = (status) => status === 'submitting' || status === 'streaming' || status === 'reconnecting' || status === 'resumed';
function markUpdated(state) {
    return { ...state, updatedAt: Date.now() };
}
export function chatStreamReducer(state, action) {
    switch (action.type) {
        case 'SUBMIT_START': {
...
export function deriveSelectors(state) {
