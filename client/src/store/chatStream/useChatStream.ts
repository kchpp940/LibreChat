...
import type { StreamError, ChatStreamState } from './types';
import {
  useChatStreamValue,
  useChatStreamDispatch,
  useChatStreamSelectors,
  useChatStreamIsRunning,
  useChatStreamShowStop,
  useChatStreamAbortScroll,
  useChatStreamSetAbortScroll,
} from './atoms';

export function useChatStream(runIndex: string | number = 0) {
  ...
  const submitStart = useCallback(
    (submission: TSubmission, conversation: TConversation | null) => {
  ...
  const streamCreated = useCallback(
    (runId: string, userMessage: TMessage, responseMessage: TMessage) => {
  ...
  const streamReconnect = useCallback(
    (attempt: number) => {
  ...
  const streamResumed = useCallback(
    (streamId: string) => {
  ...
  const updateResponseMessage = useCallback(
  ...
  const setStopButton = useCallback(
    (value: boolean) => {
  ...
  const addCompletedId = useCallback(
    (messageId: string) => {
