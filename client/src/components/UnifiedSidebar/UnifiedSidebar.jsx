 ...
 */
function SidebarChatProvider({ children }) {
    const chatHelpers = useChatHelpers(0);
    const sidebarFormMethods = useForm({ defaultValues: { text: '' } });
    return (<ChatFormProvider {...sidebarFormMethods}>
...
function UnifiedSidebar() {
    ...
    const resizeHandlers = useRef(null);
    ...
    const handleResizeKeyboard = useCallback((direction) => {
        setSidebarWidth((w) => {
    ...
    if (isSmallScreen) {
        return (<>
        <div className={cn('fixed left-0 top-0 z-[110] flex h-full bg-surface-primary-alt', expanded ? 'translate-x-0' : '-translate-x-full')} style={{
        ...
        <div className={cn('fixed inset-0 z-[109] bg-black/50', expanded ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0')} style={{ transition: `opacity ${TRANSITION_MS}ms ${EASING}` }} role="presentation">
          <button className="h-full w-full" onClick={handleCollapse} aria-label={localize('com_nav_close_sidebar')} tabIndex={expanded ? 0 : -1}/>
        </div>
      </>);
    }
    return (<SidebarChatProvider>
      <ActivePanelProvider>
        <aside className="relative flex h-full flex-shrink-0 overflow-hidden" style={{
          ...
          <Sidebar links={links} expanded={expanded} onCollapse={handleCollapse} onExpand={handleExpand} onResizeStart={handleResizeStart} onResizeKeyboard={handleResizeKeyboard}/>
        </aside>
      </ActivePanelProvider>
    </SidebarChatProvider>);
}
