...
const MeasuredRow = memo(({ cache, rowKey, parent, index, style, children }) => (<CellMeasurer cache={cache} columnIndex={0} key={rowKey} parent={parent} rowIndex={index}>
      {({ registerChild }) => (<div ref={registerChild} style={style}>
          {children}
        </div>)}
    </CellMeasurer>));
...
const LoadingRow = memo(() => {
    ...
    return (<div className="flex items-center justify-center gap-2 py-4 text-sm text-text-secondary">
      <Spinner className="text-text-primary"/>
      <span>{localize('com_ui_loading')}</span>
    </div>);
});
LoadingRow.displayName = 'ProjectWorkspaceLoadingRow';
const ConversationRow = memo(({ conversation, isGenerating }) => {
    ...
    return (<button type="button" className="flex w-full items-center gap-3 border-b border-border-light py-3 text-left outline-none transition-colors hover:bg-surface-hover focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring-primary" onClick={() => navigateToConvo(conversation)}>
        ...
        {isGenerating && (<Spinner className="h-4 w-4 shrink-0 text-text-primary" aria-label={localize('com_ui_generating')}/>)}
      </button>);
}, areConversationRenderPropsEqual);
ConversationRow.displayName = 'ProjectWorkspaceConversationRow';
const ProjectChatList = ({ conversations, isLoading, isFetchingNextPage, hasNextPage, sortBy, emptyLabel, loadMore, }) => {
    ...
    const rowRenderer = useCallback(({ index, key, parent, style }) => {
        ...
        return (<MeasuredRow key={key} {...rowProps}>
          <ConversationRow conversation={item.convo} isGenerating={activeJobIds.has(item.convo.conversationId ?? '')}/>
        </MeasuredRow>);
    ...
    return (<div className={cn('min-h-[280px] flex-1 overflow-hidden rounded-lg border border-border-light')}>
      <AutoSizer>
        {({ width, height }) => (<List ref={listRef} width={width} height={height} rowCount={flattenedItems.length} rowHeight={getRowHeight} rowRenderer={rowRenderer} deferredMeasurementCache={cache} overscanRowCount={8} onRowsRendered={handleRowsRendered} className="outline-none" style={{ outline: 'none' }}/>)}
      </AutoSizer>
    </div>);
};
