 ...
 */
export default function MCPServerStatusIcon({ serverName, serverStatus, onConfigClick, isInitializing, canCancel, onCancel, hasCustomUserVars = false, compact = false, }) {
    ...
    if (isInitializing) {
        return (<LoadingStatusIcon serverName={serverName} onConfigClick={onConfigClick} onCancel={onCancel} canCancel={canCancel}/>);
    }
    if (!serverStatus) {
        return null;
    }
...
function CompactStatusDot({ serverStatus, isInitializing }) {
    if (isInitializing) {
        return (<div className="flex size-3.5 items-center justify-center rounded-full border-2 border-surface-secondary bg-blue-500">
    ...
    return (<div className={cn('size-3 rounded-full border-2 border-surface-secondary', colorClass)}/>);
}
function LoadingStatusIcon({ serverName, onCancel, canCancel }) {
    if (canCancel) {
        return (<button type="button" onClick={onCancel} className="group flex size-6 items-center justify-center rounded p-1 hover:bg-red-100 dark:hover:bg-red-900/20" aria-label={localize('com_ui_cancel')} title={localize('com_ui_cancel')}>
    ...
    return (<div className="flex size-6 items-center justify-center rounded p-1">
      <Spinner className="size-4 text-text-primary" aria-label={localize('com_nav_mcp_status_connecting', { 0: serverName })}/>
    </div>);
}
function ConnectingSpinner({ serverName }) {
    return (<div className="flex size-6 items-center justify-center rounded p-1">
      <Spinner className="size-4 text-text-primary" aria-label={localize('com_nav_mcp_status_connecting', { 0: serverName })}/>
    </div>);
}
/** Connect button - shown for disconnected/error states. Uses PlugZap icon. */
function ConnectButton({ serverName, onConfigClick }) {
    return (<button type="button" onClick={onConfigClick} className="flex size-6 items-center justify-center rounded p-1 hover:bg-surface-secondary" aria-label={localize('com_nav_mcp_connect_server', { 0: serverName })}>
...
function ConfigureButton({ serverName, onConfigClick }) {
    return (<button type="button" onClick={onConfigClick} className="flex size-6 items-center justify-center rounded p-1 hover:bg-surface-secondary" aria-label={localize('com_nav_mcp_configure_server', { 0: serverName })}>
