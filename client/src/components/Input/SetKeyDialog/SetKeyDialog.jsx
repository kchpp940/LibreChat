...
import { useRevokeUserKeyMutation, useRevokeAllUserKeysMutation, } from 'librechat-data-provider/react-query';
import { Label, Button, Spinner, OGDialog, Dropdown, OGDialogTitle, OGDialogHeader, OGDialogFooter, OGDialogContent, useToastContext, OGDialogTrigger, } from '@librechat/client';
...
const RevokeKeysButton = ({ endpoint, disabled, setDialogOpen, }) => {
    ...
    return (<div className="flex items-center justify-between">
      <OGDialog open={open} onOpenChange={setOpen}>
        <OGDialogTrigger asChild>
          <Button variant="destructive" className="flex items-center justify-center rounded-lg transition-colors duration-200" onClick={() => setOpen(true)} disabled={disabled}>
        ...
        <OGDialogContent className="max-w-[450px]">
          ...
          <OGDialogFooter>
            ...
            <Button variant="destructive" onClick={onClick} disabled={isLoading} className="bg-destructive text-white transition-all duration-200 hover:bg-destructive/80">
...
const SetKeyDialog = ({ open, onOpenChange, endpoint, endpointType, userProvideURL, userProvideAccessKeyId, userProvideSecretAccessKey, userProvideSessionToken, userProvideBearerToken, }) => {
    ...
    const handleExpirationChange = (label) => {
        ...
        if (formSet.has(endpoint) || formSet.has(endpointType ?? '')) {
            ...
            methods.handleSubmit((data) => {
                ...
                if (isBedrock) {
                    ...
                    const hasUserProvidedAccessKeyAuth = !!userProvideAccessKeyId || !!userProvideSecretAccessKey || !!userProvideSessionToken;
                    const missingFields = [
                ...
                const { apiKey, baseURL, bedrockAccessKeyId, bedrockSecretAccessKey, bedrockSessionToken, bedrockBearerToken, ...azureOptions } = data;
      ...
      <OGDialogContent className="w-11/12 max-w-2xl">
        ...
        <div className="grid w-full items-center gap-2 py-4">
          <small className="text-red-600">
            ...
            : `${localize('com_endpoint_config_key_encryption')} ${new Date(expiryTime ?? 0).toLocaleString()}`}
          </small>
          <Dropdown label="Expires " value={expiresAtLabel} onChange={handleExpirationChange} options={expirationOptions.map((option) => option.label)} sizeClasses="w-[185px]" portal={false}/>
          <div className="mt-2"/>
          <FormProvider {...methods}>
            <EndpointComponent userKey={userKey} endpoint={endpoint} setUserKey={setUserKey} userProvideURL={userProvideURL} userProvideAccessKeyId={userProvideAccessKeyId} userProvideSecretAccessKey={userProvideSecretAccessKey} userProvideSessionToken={userProvideSessionToken} userProvideBearerToken={userProvideBearerToken}/>
          </FormProvider>
          <HelpText endpoint={endpoint}/>
        </div>
        <OGDialogFooter>
          <RevokeKeysButton endpoint={endpoint} disabled={!(expiryTime ?? '')} setDialogOpen={onOpenChange}/>
