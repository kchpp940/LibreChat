...
import { QueryKeys, Constants, EModelEndpoint, PermissionBits, LocalStorageKeys, } from 'librechat-data-provider';
 ...
 */
const AgentDetailContent = ({ agent }) => {
     ...
     */
    const formatContact = () => {
        if (!agent?.support_contact)
            return null;
        const { name, email } = agent.support_contact;
        if (name && email) {
            return (<a href={`mailto:${email}`} className="text-primary hover:underline">
          {name}
        </a>);
        }
        if (email) {
    ...
    return (<OGDialogContent className="max-h-[90vh] w-11/12 max-w-lg overflow-y-auto">
      {/* Agent avatar */}
      ...
      {/* Contact info */}
      {agent?.support_contact && formatContact() && (<div className="mt-1 text-center text-sm text-text-secondary">
      ...
      <div className="mb-4 mt-6 flex justify-center gap-2">
        <Button variant="outline" size="icon" onClick={handleFavoriteClick} title={isFavorite ? localize('com_ui_unpin') : localize('com_ui_pin')} aria-label={isFavorite ? localize('com_ui_unpin') : localize('com_ui_pin')}>
          {isFavorite ? <PinOff className="h-4 w-4"/> : <Pin className="h-4 w-4"/>}
        </Button>
        <Button variant="outline" size="icon" onClick={handleCopyLink} title={localize('com_agents_copy_link')} aria-label={localize('com_agents_copy_link')}>
          <Link className="h-4 w-4" aria-hidden="true"/>
        </Button>
        <Button variant="submit" className="w-full max-w-xs" onClick={handleStartChat} disabled={!agent}>
