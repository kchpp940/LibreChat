...
jest.mock('~/utils/agents', () => ({
    renderAgentAvatar: jest.fn((agent, options) => (<div data-testid="agent-avatar" data-size={options?.size}/>)),
}));
jest.mock('~/Providers', () => ({
...
const mockLocalize = jest.fn((key, values) => {
...
describe('AgentDetail', () => {
    beforeEach(() => {
        ...
        useLocalize.mockReturnValue(mockLocalize);
        useDefaultConvo.mockReturnValue(() => ({
    ...
    describe('Rendering', () => {
        ...
        it('should render contact information when available', () => {
            ...
            expect(screen.getByRole('link', { name: 'Support Team' })).toHaveAttribute('href', 'mailto:support@test.com');
        });
        it('should not render contact information when not available', () => {
            const agentWithoutContact = { ...mockAgent };
            delete agentWithoutContact.support_contact;
        ...
        it('should render loading state when agent is null', () => {
            renderWithProviders(<AgentDetail {...defaultProps} agent={null}/>);
    ...
    describe('Interactions', () => {
        ...
        it('should copy link and show success toast when Copy Link is clicked', async () => {
            ...
            await waitFor(() => {
                expect(mockWriteText).toHaveBeenCalledWith(`${window.location.origin}/c/new?agent_id=test-agent-id`);
            });
            await waitFor(() => {
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle very long description with proper text wrapping', () => {
            const agentWithLongDescription = {
                ...mockAgent,
                description: 'This is a very long description that should wrap properly and be displayed in multiple lines when the content exceeds the available width of the container.',
            };
