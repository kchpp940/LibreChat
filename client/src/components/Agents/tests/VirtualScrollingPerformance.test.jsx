...
jest.mock('react-virtualized', () => {
    ...
    const mockRowRendererRef = {
        current: null,
    };
    return {
        AutoSizer: ({ children, disableHeight, }) => {
            if (disableHeight) {
                return children({ width: 1200 });
            }
            return children({ width: 1200, height: 800 });
        },
        List: ReactActual.forwardRef(({ rowRenderer, rowCount, width, style, 'aria-rowcount': ariaRowCount, 'data-testid': dataTestId, 'data-total-rows': dataTotalRows, }, ref) => {
            ...
            return (<div data-testid={dataTestId || 'virtual-list'} data-total-rows={dataTotalRows || rowCount} aria-rowcount={ariaRowCount} style={style}>
            {Array.from({ length: visibleRows }, (_, index) => rowRenderer({
                    index,
                    key: `row-${index}`,
                    style: { height: 184 },
        ...
        WindowScroller: ({ children, scrollElement: _scrollElement, }) => {
            return children({
                height: 800,
                isScrolling: false,
                registerChild: (_ref) => { },
...
const createMockInfiniteQuery = (agentCount) => ({
    data: {
...
jest.mock('../AgentCard', () => {
    return function MockAgentCard({ agent }) {
        return (<div data-testid={`agent-card-${agent.id}`} style={{ height: '160px' }}>
        <h3>{agent.name}</h3>
...
describe('Virtual Scrolling Performance', () => {
    ...
    const renderComponent = (agentCount) => {
        const mockQuery = createMockInfiniteQuery(agentCount);
        const useMarketplaceAgentsInfiniteQuery = jest.requireMock('~/data-provider/Agents').useMarketplaceAgentsInfiniteQuery;
    ...
    it('memory usage remains stable with large datasets', () => {
        ...
        console.log(`Memory usage: 100 agents=${memory100}, 1000 agents=${memory1000}, 5000 agents=${memory5000}`);
    });
});
