...
jest.mock('react-virtualized', () => {
    ...
    return {
        ...
        List: ReactActual.forwardRef(({ rowRenderer, rowCount, width, style, 'aria-rowcount': ariaRowCount, 'data-testid': dataTestId, 'data-total-rows': dataTotalRows, }, ref) => {
            ...
            return (<div data-testid={dataTestId || 'virtual-list'} aria-rowcount={ariaRowCount} data-total-rows={dataTotalRows} style={style}>
            {Array.from({ length: Math.min(rowCount, 5) }, (_, index) => rowRenderer({
                    index,
                    key: `row-${index}`,
                    style: {},
...
jest.mock('../AgentCard', () => {
    return function MockAgentCard({ agent, onSelect, }) {
        return (<div data-testid={`agent-card-${agent.id}`} onClick={() => onSelect(agent)}>
        <h3>{agent.name}</h3>
