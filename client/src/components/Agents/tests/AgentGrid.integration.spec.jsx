...
jest.mock('~/hooks/useLocalize', () => () => (key, options) => {
...
jest.mock('../AgentCard', () => ({
    __esModule: true,
    default: ({ agent, onSelect }) => (<div data-testid={`agent-card-${agent.id}`} onClick={() => onSelect?.(agent)}>
      <h3>{agent.name}</h3>
...
const createMockResponse = (agentIds, hasMore, afterCursor) => ({
    object: 'list',
...
const setupViewport = (scrollHeight, clientHeight) => {
    const listeners = {};
    return {
        scrollHeight,
        clientHeight,
...
const createMockInfiniteQuery = (pages, options) => ({
    data: {
        pages,
...
describe('AgentGrid Integration with useGetMarketplaceAgentsQuery', () => {
    ...
    describe('Query Integration', () => {
        it('should call useGetMarketplaceAgentsQuery with correct parameters for category search', () => {
            render(<AgentGrid category="finance" searchQuery="test query" onSelectAgent={mockOnSelectAgent}/>);
    ...
    describe('Infinite Scroll Functionality', () => {
        ...
        describe('Auto-fetch to fill viewport', () => {
            it('should NOT auto-fetch when viewport is filled (5 agents, has_more=false)', async () => {
                ...
                mockUseMarketplaceAgentsInfiniteQuery.mockReturnValue(createMockInfiniteQuery([mockResponse], { fetchNextPage }));
                const scrollElement = setupViewport(500, 1000); // Content smaller than viewport
                const scrollElementRef = { current: scrollElement };
                const Wrapper = createWrapper();
                render(<Wrapper>
            <AgentGrid category="all" searchQuery="" onSelectAgent={mockOnSelectAgent} scrollElementRef={scrollElementRef}/>
          </Wrapper>);
            ...
            it('should auto-fetch when viewport not filled (7 agents, big viewport)', async () => {
                ...
                mockUseMarketplaceAgentsInfiniteQuery.mockImplementation(() => createMockInfiniteQuery(currentPages, {
            ...
            it('should NOT auto-fetch when viewport is filled (7 agents, small viewport)', async () => {
                const firstPage = createMockResponse(['1', '2', '3', '4', '5', '6'], true, 'cursor-6');
                const fetchNextPage = jest.fn();
                mockUseMarketplaceAgentsInfiniteQuery.mockReturnValue(createMockInfiniteQuery([firstPage], { fetchNextPage, hasNextPage: true }));
