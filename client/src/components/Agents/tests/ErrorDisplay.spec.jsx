...
const mockLocalize = jest.fn((key, options) => {
    const translations = {
...
describe('ErrorDisplay', () => {
    ...
    describe('Backend error responses', () => {
        it('displays user-friendly message from backend response', () => {
            ...
            expect(screen.getByText('Unable to load agents. Please try refreshing the page.')).toBeInTheDocument();
            expect(screen.getByText('💡 Try refreshing the page or check your network connection')).toBeInTheDocument();
        });
        it('handles search context with backend response', () => {
            ...
            expect(screen.getByText('Search is temporarily unavailable. Please try again.')).toBeInTheDocument();
        });
    });
    ...
    describe('HTTP status codes', () => {
        ...
        it('handles 400 bad request errors', () => {
            ...
            expect(screen.getByText('💡 Enter a search term to find agents by name or description')).toBeInTheDocument();
        });
    ...
    describe('Fallback error handling', () => {
        it('handles unknown errors gracefully', () => {
            ...
            render(<ErrorDisplay error={error}/>);
            expect(screen.getByText('Something went wrong')).toBeInTheDocument();
            expect(screen.getByText('We encountered an issue while loading the content.')).toBeInTheDocument();
            expect(screen.getByText('💡 Please try refreshing the page or try again later.')).toBeInTheDocument();
        });
