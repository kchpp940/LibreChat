...
jest.mock('~/Providers', () => ({
    ChatContext: {
        Provider: ({ children, value }) => (<div data-testid="chat-context-provider" data-value={JSON.stringify(value)}>
        {children}
      </div>),
    },
...
const mockedUseChatContext = useChatContext;
...
describe('MarketplaceProvider', () => {
    beforeEach(() => {
        ...
        useChatHelpers.mockReturnValue({
            conversation: {
    ...
    it('provides stable context value (memoization)', () => {
        const { rerender } = render(<MarketplaceProvider>
        <TestConsumer />
      </MarketplaceProvider>);
