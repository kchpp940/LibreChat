 ...
 */
const VirtualizedAgentGrid = ({ category, searchQuery, onSelectAgent, scrollElement, }) => {
    ...
    // Use infinite query
    const { data, isLoading, error, isFetching, fetchNextPage, hasNextPage, refetch, isFetchingNextPage, } = useMarketplaceAgentsInfiniteQuery(queryParams);
    ...
    const getCardsPerRow = useCallback((width) => {
        return width >= 768 ? CARDS_PER_ROW_DESKTOP : CARDS_PER_ROW_MOBILE;
    }, []);
    const getRowCount = useCallback((agentCount, cardsPerRow) => {
        return Math.ceil(agentCount / cardsPerRow);
    }, []);
    const getRowItems = useCallback((rowIndex, cardsPerRow) => {
    ...
    const rowRenderer = useCallback(({ index, key, style, parent }) => {
        ...
        return (<div key={key} style={style}>
          <div className={cn('grid gap-6 px-0', cardsPerRow === 1 ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2')} role="row" aria-rowindex={index + 1}>
            {rowAgents.map((agent, cardIndex) => {
                const globalIndex = index * cardsPerRow + cardIndex;
                return (<div key={`${agent.id}-${globalIndex}`} role="gridcell">
                  <AgentCard agent={agent} onSelect={onSelectAgent}/>
                </div>);
            })}
          </div>

          {showLoading && (<div className="flex justify-center py-4" role="status" aria-live="polite" aria-label={localize('com_agents_loading')}>
    ...
    const loadingSpinner = (<div className="flex justify-center py-12">
      <Spinner className="h-8 w-8 text-primary"/>
    </div>);
    // Handle error state
    if (error) {
        return (<ErrorDisplay error={error || 'Unknown error occurred'} onRetry={() => refetch()} context={{ searchQuery, category }}/>);
    }
    ...
    if ((!currentAgents || currentAgents.length === 0) && !isLoading && !isFetching) {
        return (<div className="py-12 text-center text-text-secondary" role="status" aria-live="polite" aria-label={searchQuery
    ...
    return (<div className="space-y-6" role="tabpanel" id={`category-panel-${category}`} aria-labelledby={`category-tab-${category}`} aria-live="polite" aria-busy={isLoading && !hasData}>
      ...
      <div role="grid" aria-label={localize('com_agents_grid_announcement', {
            ...
            {({ height, isScrolling, registerChild, onChildScroll, scrollTop }) => (<AutoSizer disableHeight>
                {({ width }) => {
                    ...
                    return (<div ref={registerChild}>
                      <VirtualList ref={listRef} autoHeight height={height} isScrolling={isScrolling} onScroll={onChildScroll} overscanRowCount={OVERSCAN_ROW_COUNT} rowCount={rowCount} rowHeight={ROW_HEIGHT} rowRenderer={rowRenderer} scrollTop={scrollTop} width={width} style={{ outline: 'none' }} aria-rowcount={rowCount} data-testid="virtual-list" data-total-rows={rowCount}/>
                    </div>);
                }}
              </AutoSizer>)}
          </WindowScroller>) : (
        ...
        <div style={{ height: 600 }}>
            <AutoSizer>
              {({ width, height }) => {
                const cardsPerRow = getCardsPerRow(width);
                const rowCount = getRowCount(currentAgents.length, cardsPerRow);
                return (<VirtualList ref={listRef} height={height} overscanRowCount={OVERSCAN_ROW_COUNT} rowCount={rowCount} rowHeight={ROW_HEIGHT} rowRenderer={rowRenderer} width={width} style={{ outline: 'none' }} aria-rowcount={rowCount} data-testid="virtual-list" data-total-rows={rowCount}/>);
            }}
            </AutoSizer>
          </div>)}
      </div>
      ...
      {!hasNextPage && currentAgents && currentAgents.length > 0 && (<div className="mt-8 text-center">
