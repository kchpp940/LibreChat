 ...
 */
const AgentMarketplace = ({ className = '' }) => {
    ...
    return (<div className={`relative flex w-full grow overflow-hidden bg-presentation ${className}`}>
      <SidePanelGroup>
        <main className="flex h-full flex-col overflow-hidden" role="main">
          {/* Scrollable container */}
          <div ref={scrollContainerRef} className="scrollbar-gutter-stable relative flex h-full flex-col overflow-y-auto overflow-x-hidden">
            {/* Hero Section - scrolls away */}
            {!isSmallScreen && (<div className="container mx-auto max-w-4xl">
            ...
            <div className="sticky top-0 z-10 mt-4 bg-presentation pb-4 md:mt-0">
              <div className="container mx-auto max-w-4xl px-4">
                {isSmallScreen ? (<div className="mx-auto mb-3 flex max-w-2xl items-center justify-between gap-2">
                    <OpenSidebar />
                ...
                {/* Category tabs */}
                <CategoryTabs categories={categoriesQuery.data || []} activeTab={displayCategory} isLoading={categoriesQuery.isLoading} onChange={handleTabChange}/>
              </div>
            </div>
            ...
            <div className="container mx-auto max-w-4xl px-4 pb-8">
              ...
              <div className="relative overflow-hidden">
                ...
                <div className={cn(isTransitioning &&
            (animationDirection === 'right'
                ? 'motion-safe:animate-slide-out-left'
                : 'motion-safe:animate-slide-out-right'))} key={`pane-current-${displayCategory}`}>
                  {/* Category header - only show when not searching */}
                  {!searchQuery && (<div className="mb-6 mt-6">
                      {(() => {
                    ...
                    if (categoryData) {
                        return {
                            ...
                            description: categoryData.description?.startsWith('com_')
                                ? localize(categoryData.description)
                ...
                return (<div className="text-left">
                  ...
                  {/* Agent grid */}
                  <AgentGrid key={`grid-${displayCategory}`} category={displayCategory} searchQuery={searchQuery} onSelectAgent={handleAgentSelect} scrollElementRef={scrollContainerRef}/>
                </div>

                {/* Next content pane, only during transition */}
                {isTransitioning && nextCategory && (<div className={cn('absolute inset-0', animationDirection === 'right'
                ? 'motion-safe:animate-slide-in-right'
                : 'motion-safe:animate-slide-in-left')} key={`pane-next-${nextCategory}-${animationDirection}`}>
                    {/* Category header - only show when not searching */}
                    {!searchQuery && (<div className="mb-6 mt-6">
                        {(() => {
                    // Get category data for display
                    const getCategoryData = () => {
                        ...
                        // Find the category in the API data
                        const categoryData = categoriesQuery.data?.find((cat) => cat.value === nextCategory);
                        if (categoryData) {
                            return {
                                name: categoryData.label?.startsWith('com_')
                                    ? localize(categoryData.label)
                        ...
                        // Fallback for unknown categories
                        return {
                            name: (nextCategory || '').charAt(0).toUpperCase() +
                    ...
                    {/* Agent grid */}
                    <AgentGrid key={`grid-${nextCategory}`} category={nextCategory} searchQuery={searchQuery} onSelectAgent={handleAgentSelect} scrollElementRef={scrollContainerRef}/>
                  </div>)}
