...
import { Button, OGDialog, TooltipAnchor, OGDialogTrigger, OGDialogTemplate, useToastContext, } from '@librechat/client';
import { useDeleteSkillMutation } from '~/data-provider';
import { useLocalize } from '~/hooks';
function DeleteSkill({ skillId, skillName, disabled, onDelete }) {
    ...
    return (<OGDialog>
      <OGDialogTrigger asChild>
        <TooltipAnchor description={localize('com_ui_delete')} side="bottom" render={<Button variant="destructive" size="icon" className="size-9" aria-label={localize('com_ui_delete')} disabled={triggerDisabled} onClick={(e) => e.stopPropagation()}>
              <Trash2 className="size-5" aria-hidden="true"/>
            </Button>}/>
      </OGDialogTrigger>
      <OGDialogTemplate showCloseButton={false} title={localize('com_ui_delete')} className="max-w-[450px]" main={<p className="text-left text-sm text-text-primary">
