...
export default function FilterSkills({ searchTerm, onSearchChange, className = '', }) {
    ...
    return (<div className={cn('flex flex-col gap-2', className)}>
      <div role="search" className="flex items-center gap-2">
        <FilterInput inputId="skills-filter" label={localize('com_ui_filter_skills_name')} value={searchTerm} onChange={onSearchChange} containerClassName="flex-1"/>
        {hasCreateAccess && <CreateSkillMenu />}
      </div>
      {user?.role === SystemRoles.ADMIN && (<div className="flex w-full items-center justify-end">
          <AdminSettings />
        </div>)}
    </div>);
}
