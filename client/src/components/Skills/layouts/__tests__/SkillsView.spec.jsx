...
const mockUseHasAccess = jest.fn((..._args) => true);
const mockUseMediaQuery = jest.fn((_query) => false);
...
jest.mock('~/hooks', () => ({
    useLocalize: () => (key) => key,
    useHasAccess: (...args) => mockUseHasAccess(...args),
    useAuthContext: () => ({
        user: { role: 'admin' },
        roles: { admin: {} },
    }),
...
jest.mock('~/components/Skills/display/SkillFileViewer', () => () => (<div data-testid="skill-file-viewer"/>));
jest.mock('~/components/Skills/display/SkillDetail', () => () => (<div data-testid="skill-detail"/>));
jest.mock('~/components/Skills/display/SkillState', () => ({ title }) => (<div>{title}</div>));
describe('SkillsView', () => {
    beforeEach(() => {
