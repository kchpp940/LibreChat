...
export default function SkillsView() {
    ...
    if (!rolesLoaded) {
        return (<div className="flex h-full w-full items-center justify-center bg-presentation">
    ...
    if (!skillId) {
        return (<div className="flex h-full w-full flex-col bg-presentation">
        <MobileSidebarToggle />
        <div className="flex flex-1 flex-col items-center justify-center">
          <SkillState title={localize('com_ui_skill_no_selection')} description={localize('com_ui_skill_no_selection_desc')}/>
        </div>
      </div>);
    }
...
function CreateView() {
    return (<div className="flex h-full w-full flex-col overflow-y-auto bg-presentation">
      <MobileSidebarToggle />
      <CreateSkillForm />
    </div>);
}
/** Read-only detail view — the default when clicking a skill. */
function DetailView({ skillId }) {
    const localize = useLocalize();
    ...
    if (skillQuery.isError || !skillQuery.data) {
        return (<div className="flex h-full w-full flex-col bg-presentation">
        <MobileSidebarToggle />
        <SkillState variant="error" title={localize('com_ui_skill_not_found')} description={localize('com_ui_skill_not_found_description')}/>
      </div>);
    }
    return (<div className="flex h-full w-full flex-col bg-presentation">
      <MobileSidebarToggle />
      <SkillDetail skill={skillQuery.data} onEdit={() => navigate(`/skills/${skillId}/edit`)} onDelete={() => navigate('/skills', { replace: true })}/>
    </div>);
}
/** Edit form — reached via the Edit button or `/skills/:id/edit` URL. */
function EditView({ skillId }) {
    return (<div className="flex h-full w-full flex-col overflow-y-auto bg-presentation">
      <MobileSidebarToggle />
...
function MobileSidebarToggle() {
    ...
    return (<div className="flex shrink-0 items-center px-4 pt-3">
      <OpenSidebar />
    </div>);
}
