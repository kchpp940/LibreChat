...
export default function SkillFileEditor({ skillId, nodeId, fileName }) {
    ...
    const handleSave = useCallback(() => {
        ...
        updateContent.mutate({ skillId, nodeId, content: localContent }, {
            onSuccess: () => {
                setIsDirty(false);
            },
        });
    ...
    if (isLoading) {
        return (<div className="flex h-full items-center justify-center bg-presentation">
        <Spinner className="text-text-tertiary"/>
      </div>);
    }
    const lineCount = displayContent.split('\n').length;
    return (<div className="flex h-full flex-col bg-presentation" onKeyDown={handleKeyDown}>
      <div className="flex items-center gap-2 border-b border-border-light px-4 py-2">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <Circle className={cn('size-2 shrink-0 transition-[opacity,color] duration-200', isDirty ? 'fill-current text-yellow-500 opacity-100' : 'opacity-0')} aria-hidden="true"/>
        </div>
        <Button type="button" variant={isDirty ? 'submit' : 'outline'} size="sm" disabled={!isDirty || updateContent.isLoading} onClick={handleSave} aria-label={localize('com_ui_save')} className="h-7 gap-1.5 px-2.5 text-xs">
          <Save className="size-3"/>
          {localize('com_ui_save')}
        </Button>
      </div>
      <div className="relative flex-1 overflow-hidden">
        <textarea ref={textareaRef} value={displayContent} onChange={handleChange} spellCheck={false} className={cn('size-full resize-none bg-transparent px-4 py-3 font-mono text-[13px] leading-6 text-text-primary outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring-primary', 'selection:bg-blue-500/20')} aria-label={`${localize('com_ui_edit')} ${fileName}`}/>
      </div>
