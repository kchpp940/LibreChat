...
export default function InvocationModePicker({ value, onChange }) {
    const localize = useLocalize();
    const [isOpen, setIsOpen] = useState(false);
    const menuItems = useMemo(() => modes.map((mode) => ({
        id: mode,
    ...
    return (<div className="flex flex-col">
      ...
      <DropdownPopup menuId="invocation-mode-menu" isOpen={isOpen} setIsOpen={setIsOpen} trigger={<Ariakit.MenuButton aria-label={localize('com_ui_invocation_mode')} className="flex w-fit items-center justify-between gap-2 rounded-xl border border-border-medium bg-transparent px-3 py-2 text-sm text-text-primary transition-colors hover:bg-surface-tertiary">
            <span className="font-medium text-text-primary">
              {localize(invocationLabels[value])}
            </span>
            <ChevronDown className="size-4 text-text-secondary" aria-hidden="true"/>
          </Ariakit.MenuButton>} items={menuItems} className="w-[280px]" portal={true}/>
    </div>);
}
