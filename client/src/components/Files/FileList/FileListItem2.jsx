...
export default function FileListItem2({ file, deleteFile, attachedVectorStores, }) {
    ...
    return (<div onClick={() => {
      ...
      <div className="flex w-10/12 flex-col justify-around md:flex-row">
        ...
        <div className="flex w-fit flex-row flex-wrap text-gray-500 md:w-3/5">
          {attachedVectorStores.map((vectorStore, index) => {
            if (index === 4) {
                return (<span key={index} className="ml-2 mt-1 flex flex-row items-center rounded-full bg-[#f5f5f5] px-2 text-xs">
            ...
            return (<span key={index} className="ml-2 mt-1 content-center rounded-full bg-[#f2f8ec] px-2 text-xs text-[#91c561]">
                {vectorStore.name}
              </span>);
        })}
        </div>
      ...
      <div className="mr-0 flex w-2/12 flex-col items-center justify-evenly sm:mr-4 md:flex-row">
        ...
        <Button className="w-min bg-transparent text-[#666666] hover:bg-slate-200" onClick={() => deleteFile(file._id)}>
          <TrashIcon className=""/>
        </Button>
      </div>
    </div>);
