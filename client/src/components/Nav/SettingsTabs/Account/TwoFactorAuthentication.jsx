...
import { OGDialog, useToastContext, OGDialogContent, OGDialogHeader, OGDialogTitle, Progress, } from '@librechat/client';
import { useConfirmTwoFactorMutation, useDisableTwoFactorMutation, useEnableTwoFactorMutation, useVerifyTwoFactorMutation, } from '~/data-provider';
...
const TwoFactorAuthentication = () => {
    ...
    const handleDisableVerify = useCallback((token, useBackup) => {
    ...
    return (<OGDialog open={isDialogOpen} onOpenChange={(open) => {
      ...
      <DisableTwoFactorToggle enabled={!!user?.twoFactorEnabled} onChange={() => setDialogOpen(true)} disabled={isVerifying || isDisabling || isGenerating} buttonRef={buttonRef}/>

      <OGDialogContent className="w-11/12 max-w-lg p-6">
        <AnimatePresence mode="wait">
          <motion.div id="two-factor-authentication-dialog" key={phase} variants={phaseVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
            <OGDialogHeader>
            ...
            : localize('com_ui_2fa_setup')}
              ...
              {user?.twoFactorEnabled && phase !== 'disable' && (<div className="mt-4 space-y-3">
                  <Progress value={(steps.indexOf(phasesLabel[phase]) / (steps.length - 1)) * 100} className="h-2 rounded-full"/>
                  <div className="flex justify-between text-sm">
                    {steps.map((step, index) => (<motion.span key={step} animate={{
            ...
            <AnimatePresence mode="wait">
              {phase === 'setup' && (<SetupPhase isGenerating={isGenerating} onGenerate={handleGenerateQRCode} onNext={() => setPhase('qr')} onError={(error) => showToast({ message: error.message, status: 'error' })}/>)}

              {phase === 'qr' && (<QRPhase secret={secret} otpauthUrl={otpauthUrl} onNext={() => setPhase('verify')} onError={(error) => showToast({ message: error.message, status: 'error' })}/>)}

              {phase === 'verify' && (<VerifyPhase token={verificationToken} onTokenChange={setVerificationToken} isVerifying={isVerifying} onNext={handleVerify} onError={(error) => showToast({ message: error.message, status: 'error' })}/>)}

              {phase === 'backup' && (<BackupPhase backupCodes={backupCodes} onDownload={handleDownload} downloaded={downloaded} onNext={handleConfirm} onError={(error) => showToast({ message: error.message, status: 'error' })}/>)}

              {phase === 'disable' && (<DisablePhase onDisable={handleDisableVerify} isDisabling={isDisabling} onError={(error) => showToast({ message: error.message, status: 'error' })}/>)}
            </AnimatePresence>
          </motion.div>
        </AnimatePresence>
      </OGDialogContent>
