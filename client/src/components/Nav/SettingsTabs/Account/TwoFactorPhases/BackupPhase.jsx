...
export const BackupPhase = ({ backupCodes, onDownload, downloaded, onNext, }) => {
    ...
    return (<motion.div {...fadeAnimation} className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 rounded-xl bg-surface-secondary p-6">
        {backupCodes.map((code, index) => (<motion.div key={code} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="rounded-lg bg-surface-tertiary p-3">
