...
import { OGDialogTemplate, Label, Button, OGDialog, OGDialogTrigger, Spinner, useOnClickOutside, } from '@librechat/client';
import { useLocalize } from '~/hooks';
export const DeleteCache = ({ disabled = false }) => {
    ...
    return (<div className="flex items-center justify-between">
      ...
      <OGDialog open={open} onOpenChange={setOpen}>
        <OGDialogTrigger asChild>
          <Button variant="destructive" onClick={() => setOpen(true)} disabled={disabled || isCacheEmpty} aria-labelledby="delete-cache-label">
            {localize('com_ui_delete')}
          </Button>
        </OGDialogTrigger>
        <OGDialogTemplate showCloseButton={false} title={localize('com_nav_confirm_clear')} className="max-w-[450px]" main={<Label className="text-left text-sm font-medium">
