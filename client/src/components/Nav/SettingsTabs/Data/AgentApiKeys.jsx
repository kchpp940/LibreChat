...
import { useGetAgentApiKeysQuery, useCreateAgentApiKeyMutation, useDeleteAgentApiKeyMutation, } from 'librechat-data-provider/react-query';
import { Permissions, PermissionTypes } from 'librechat-data-provider';
import { Plus, Trash2, Key, ShieldEllipsis } from 'lucide-react';
import { Button, Input, Label, Spinner, SecretInput, OGDialog, OGDialogClose, OGDialogTitle, OGDialogHeader, OGDialogContent, OGDialogTrigger, useToastContext, } from '@librechat/client';
...
function CreateKeyDialog({ onKeyCreated }) {
    ...
    return (<OGDialog open={open} onOpenChange={setOpen}>
        ...
        <div className="space-y-4 py-4">
          {!newKey ? (<>
              <div className="space-y-2">
                ...
                <Input id="key-name" value={name} onChange={(e) => setName(e.target.value)} placeholder={localize('com_ui_api_key_name_placeholder')}/>
              </div>
              ...
              <div className="space-y-2">
                ...
                <SecretInput value={newKey} readOnly showCopy controlsOnHover onCopy={handleCopy} aria-label={localize('com_ui_your_api_key')} className="font-mono text-sm"/>
              </div>
...
function KeyItem({ id, name, keyPrefix, createdAt, lastUsedAt, }) {
    const localize = useLocalize();
    const { showToast } = useToastContext();
    ...
    return (<div className="flex items-center justify-between rounded-lg border border-border-light p-3">
      ...
      <div>
        {confirmDelete ? (<div className="flex gap-2">
            ...
            <Button variant="destructive" size="sm" onClick={handleDelete} disabled={deleteMutation.isLoading}>
              {deleteMutation.isLoading ? (<Spinner className="h-4 w-4"/>) : (localize('com_ui_delete'))}
            </Button>
          </div>) : (<Button variant="ghost" size="icon" onClick={() => setConfirmDelete(true)} title={localize('com_ui_delete')}>
    ...
    return (<div className="space-y-4">
      ...
      <div className="max-h-[400px] space-y-2 overflow-y-auto">
        ...
        {!isLoading &&
            data?.keys &&
            data.keys.length > 0 &&
            data.keys.map((key) => (<KeyItem key={key.id} id={key.id} name={key.name} keyPrefix={key.keyPrefix} createdAt={key.createdAt} lastUsedAt={key.lastUsedAt}/>))}
        {!isLoading && (!data?.keys || data.keys.length === 0) && (<div className="rounded-lg border-2 border-dashed border-border-light p-8 text-center">
...
function RemoteAgentsAdminSettings() {
    ...
    const trigger = (<Button variant="ghost" size="icon" className="h-8 w-8" aria-label={localize('com_ui_admin_settings')}>
      <ShieldEllipsis className="h-5 w-5" aria-hidden="true"/>
    </Button>);
    return (<AdminSettingsDialog permissionType={PermissionTypes.REMOTE_AGENTS} sectionKey="com_ui_remote_agents" permissions={remoteAgentsPermissions} menuId="remote-agents-role-dropdown" mutation={mutation} trigger={trigger}/>);
}
export function AgentApiKeys() {
    ...
    return (<div className="flex items-center justify-between">
      ...
      <OGDialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <OGDialogContent title={localize('com_ui_agent_api_keys')} className="w-11/12 max-w-2xl bg-background text-text-primary shadow-2xl">
          <OGDialogHeader>
