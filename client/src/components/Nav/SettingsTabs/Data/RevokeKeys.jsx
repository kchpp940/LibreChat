...
import { OGDialogTemplate, Button, Label, OGDialog, OGDialogTrigger, Spinner, } from '@librechat/client';
import { useLocalize } from '~/hooks';
export const RevokeKeys = ({ disabled = false, setDialogOpen, }) => {
    ...
    return (<div className="flex items-center justify-between">
      ...
      <OGDialog open={open} onOpenChange={setOpen}>
        <OGDialogTrigger asChild>
          <Button variant="destructive" onClick={() => setOpen(true)} disabled={disabled} aria-labelledby="revoke-info-label">
            {localize('com_ui_revoke')}
          </Button>
        </OGDialogTrigger>
        <OGDialogTemplate showCloseButton={false} title={localize('com_ui_revoke_keys')} className="max-w-[450px]" main={<Label className="text-left text-sm font-medium">
