...
function buildDiagnosticsBlob(version, buildInfo) {
    const lines = [
...
function About() {
    ...
    const diagnosticsBlob = useMemo(() => buildDiagnosticsBlob(version, buildInfo), [version, buildInfo]);
    useEffect(() => () => {
    ...
    return (<div className="flex flex-col gap-3 p-1 text-sm text-text-primary">
      <section aria-labelledby="about-version-heading" className="flex flex-col">
        ...
        <div className="rounded-lg border border-border-light bg-surface-secondary p-3">
          ...
          <Row label={localize('com_nav_about_commit')} value={buildInfo?.commitShort ?? UNKNOWN_PLACEHOLDER}/>
          <Row label={localize('com_nav_about_branch')} value={buildInfo?.branch ?? UNKNOWN_PLACEHOLDER}/>
          <Row label={localize('com_nav_about_build_date')} value={formatBuildDate(buildInfo?.buildDate)}/>
        </div>
      </section>
      ...
      <section aria-labelledby="about-diagnostics-heading" className="flex flex-col">
        ...
        <button type="button" onClick={onCopy} className="inline-flex items-center justify-center gap-2 self-start rounded-md border border-border-light bg-surface-secondary px-3 py-1.5 text-xs font-medium text-text-primary transition-colors hover:bg-surface-tertiary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-xheavy">
          {copied ? (<>
