...
function Commands() {
    ...
    return (<div className="space-y-4 p-1">
      ...
      <div className="flex flex-col gap-3 text-sm text-text-primary">
        {commandSwitchConfigs.map((config) => (<div key={config.key} className="pb-3">
            <ToggleSwitch stateAtom={config.stateAtom} localizationKey={config.localizationKey} switchId={config.switchId} showSwitch={getShowSwitch(config.permissionType)}/>
          </div>))}
      </div>
    </div>);
}
