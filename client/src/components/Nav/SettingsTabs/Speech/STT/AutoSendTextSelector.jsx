...
export default function AutoSendTextSelector() {
    ...
    return (<div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <Switch id="autoSendTextToggle" checked={isEnabled} onCheckedChange={handleToggle} className="ml-4" data-testid="autoSendTextToggle" aria-labelledby={labelId} disabled={!speechToText}/>
      </div>
      {isEnabled && (<div className="mt-2 flex items-center justify-between">
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-between">
            <Slider value={[delayValue]} onValueChange={handleSliderChange} onDoubleClick={() => {
                setDelayValue(3);
                if (isEnabled) {
                    setAutoSendText(3);
                }
            }} min={0} max={60} step={1} className="ml-4 flex h-4 w-24" disabled={!speechToText || !isEnabled} aria-labelledby="auto-send-delay-label"/>
            <div className="w-2"/>
            <InputNumber value={`${delayValue} s`} disabled={!speechToText || !isEnabled} onChange={handleInputChange} min={0} max={60} aria-labelledby="auto-send-delay-label" className={cn(defaultTextProps, cn(optionText, 'reset-rc-number-input reset-rc-number-input-text-right h-auto w-12 border-0 group-hover/temp:border-gray-200'))}/>
          </div>
        </div>)}
    </div>);
}
