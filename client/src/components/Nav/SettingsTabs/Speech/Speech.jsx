...
import { CloudBrowserVoicesSwitch, AutomaticPlaybackSwitch, TextToSpeechSwitch, EngineTTSDropdown, CacheTTSSwitch, VoiceDropdown, PlaybackRate, } from './TTS';
import { AutoTranscribeAudioSwitch, LanguageSTTDropdown, SpeechToTextSwitch, AutoSendTextSelector, EngineSTTDropdown, DecibelSelector, } from './STT';
...
function Speech() {
    ...
    const [cloudBrowserVoices, setCloudBrowserVoices] = useRecoilState(store.cloudBrowserVoices);
    ...
    const updateSetting = useCallback((key, newValue) => {
        const settings = {
    ...
    return (<Tabs.Root defaultValue={'simple'} orientation="horizontal" value={advancedMode ? 'advanced' : 'simple'}>
      <div className="sticky -top-1 z-50 mb-4 bg-white dark:bg-gray-700">
        <Tabs.List className="flex justify-center bg-background">
          <Tabs.Trigger onClick={() => setAdvancedMode(false)} className={cn('group m-1 flex items-center justify-center gap-2 bg-transparent px-4 py-2 text-sm text-text-secondary transition-all duration-200 ease-in-out radix-state-active:bg-secondary radix-state-active:text-foreground radix-state-active:shadow-lg', isSmallScreen ? 'flex-row rounded-lg' : 'rounded-xl', 'w-full')} value="simple" style={{ userSelect: 'none' }}>
            <Lightbulb aria-hidden="true"/>
            {localize('com_ui_simple')}
          </Tabs.Trigger>
          <Tabs.Trigger onClick={() => setAdvancedMode(true)} className={cn('group m-1 flex items-center justify-center gap-2 bg-transparent px-4 py-2 text-sm text-text-secondary transition-all duration-200 ease-in-out radix-state-active:bg-secondary radix-state-active:text-foreground radix-state-active:shadow-lg', isSmallScreen ? 'flex-row rounded-lg' : 'rounded-xl', 'w-full')} value="advanced" style={{ userSelect: 'none' }}>
      ...
      <Tabs.Content value={'advanced'} tabIndex={-1}>
        <div className="flex flex-col gap-3 text-sm text-text-primary">
          ...
          {autoTranscribeAudio && (<div className="pb-2">
              <DecibelSelector />
            </div>)}
          <div className="pb-2">
          ...
          <EngineTTSDropdown external={ttsExternal}/>
          <VoiceDropdown />
          {engineTTS === 'browser' && (<div className="pb-2">
