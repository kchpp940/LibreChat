...
const FavoriteItemSkeleton = () => (<div className="flex w-full items-center rounded-lg px-3 py-2">
    <Skeleton className="mr-2 h-5 w-5 rounded-full"/>
    <Skeleton className="h-4 w-24"/>
  </div>);
const MarketplaceSkeleton = () => (<div className="flex w-full items-center rounded-lg px-3 py-2">
    <Skeleton className="mr-2 h-5 w-5"/>
    <Skeleton className="h-4 w-28"/>
  </div>);
const DraggableFavoriteItem = ({ id, index, moveItem, onDrop, children, }) => {
...
export default function FavoritesList({ isSmallScreen, toggleNav, }) {
    ...
    const modelSpecs = useMemo(() => startupConfig?.modelSpecs?.list ?? [], [startupConfig?.modelSpecs?.list]);
    ...
    const onSelectEndpoint = useCallback((...args) => {
        _onSelectEndpoint?.(...args);
        if (isSmallScreen && toggleNav) {
            toggleNav();
        }
    }, [_onSelectEndpoint, isSmallScreen, toggleNav]);
    const onSelectSpec = useCallback((...args) => {
        _onSelectSpec?.(...args);
        if (isSmallScreen && toggleNav) {
            toggleNav();
        }
    ...
    const handleRemoveFocus = useCallback(() => {
        ...
        const nextFavorite = listContainerRef.current?.querySelector('[data-testid="favorite-item"]');
        if (nextFavorite) {
            nextFavorite.focus();
            return;
        }
        const newChatButton = document.querySelector('[data-testid="nav-new-chat-button"]');
        newChatButton?.focus();
    }, []);
    const safeFavorites = useMemo(() => (Array.isArray(favorites) ? favorites : []), [favorites]);
    const allAgentIds = useMemo(() => safeFavorites.map((f) => f.agentId).filter(Boolean), [safeFavorites]);
    ...
    const isAgentsLoading = (allAgentIds.length > 0 && agentsMap === undefined) ||
    ...
    if (isFavoritesLoading) {
        return (<div className="mb-2 flex flex-col pb-2">
    ...
    return (<div className="mb-2 flex flex-col">
      <div ref={listContainerRef} className="mt-1 flex flex-col gap-1">
        ...
        {isAgentsLoading ? (<>
            ...
            {safeFavorites.map((_, index) => (<FavoriteItemSkeleton key={`skeleton-${index}`}/>))}
          </>) : (<>
            {/* Agent Marketplace button */}
            {showAgentMarketplace && (<div ref={marketplaceRef} role="button" tabIndex={0} aria-label={localize('com_agents_marketplace')} className="group relative flex w-full cursor-pointer items-center justify-between rounded-lg px-3 py-2 text-sm text-text-primary outline-none hover:bg-surface-active-alt focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-black dark:focus-visible:ring-white" onClick={handleAgentMarketplace} onKeyDown={(e) => {
            ...
            {safeFavorites.map((fav, index) => {
                if (fav.agentId) {
                    ...
                    return (<DraggableFavoriteItem key={fav.agentId} id={fav.agentId} index={index} moveItem={moveItem} onDrop={handleDrop}>
                    <FavoriteItem item={agent} type="agent" onSelectEndpoint={onSelectEndpoint} onRemoveFocus={handleRemoveFocus}/>
                    ...
                    return (<DraggableFavoriteItem key={`spec-${fav.spec}`} id={`spec-${fav.spec}`} index={index} moveItem={moveItem} onDrop={handleDrop}>
                    <FavoriteItem item={spec} type="spec" onSelectSpec={onSelectSpec} onRemoveFocus={handleRemoveFocus} endpointsConfig={endpointsConfig}/>
                  </DraggableFavoriteItem>);
                }
                else if (fav.model && fav.endpoint) {
                    return (<DraggableFavoriteItem key={`${fav.endpoint}-${fav.model}`} id={`${fav.endpoint}-${fav.model}`} index={index} moveItem={moveItem} onDrop={handleDrop}>
                    <FavoriteItem item={{ model: fav.model, endpoint: fav.endpoint }} type="model" onSelectEndpoint={onSelectEndpoint} onRemoveFocus={handleRemoveFocus}/>
                  </DraggableFavoriteItem>);
                }
                return null;
            })}
