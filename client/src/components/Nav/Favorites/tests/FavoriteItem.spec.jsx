...
jest.mock('@ariakit/react/menu', () => ({
    MenuButton: ({ children }) => (<button type="button">{children}</button>),
}));
const baseAgent = {
    id: 'agent-123',
    name: 'Research Agent',
...
const baseModel = { model: 'gpt-5', endpoint: 'openai' };
const baseSpec = {
    name: 'my-spec',
    label: 'My Model Spec',
...
describe('FavoriteItem', () => {
    ...
    describe('type="agent"', () => {
        ...
        it('has aria-label formatted as "<name> (com_ui_agent)"', () => {
            ...
            expect(screen.getByRole('button', { name: 'Research Agent (com_ui_agent)' })).toBeInTheDocument();
        });
    ...
    describe('type="spec"', () => {
        ...
        it('has aria-label formatted as "<label> (com_ui_model_spec)"', () => {
            render(<FavoriteItem type="spec" item={baseSpec}/>);
            expect(screen.getByRole('button', { name: 'My Model Spec (com_ui_model_spec)' })).toBeInTheDocument();
        });
