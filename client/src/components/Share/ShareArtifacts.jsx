...
import { useMediaQuery, ResizablePanel, ResizableHandleAlt, ResizablePanelGroup, } from '@librechat/client';
 ...
 */
export function ShareArtifactsContainer({ messages, conversationId, mainContent, }) {
    ...
    const artifactsContextValue = useMemo(() => {
        const latestMessage = Array.isArray(messages) && messages.length > 0 ? messages[messages.length - 1] : null;
        if (!latestMessage) {
            return null;
        }
    ...
    const hasAutoOpenableArtifact = Object.values(artifacts ?? {}).some((artifact) => artifact != null && !isCodeOnlyArtifact(artifact.type));
    const shouldRenderArtifacts = artifactsVisibility === true &&
    ...
    return (<ResizablePanelGroup orientation="horizontal" className="h-full w-full" onLayoutChanged={handleLayoutChanged}>
      <ResizablePanel defaultSize={`${100 - normalizedArtifactSize}`} minSize="35" id="share-content">
        {mainContent}
      </ResizablePanel>
      <ResizableHandleAlt withHandle className="bg-border-medium text-text-primary"/>
      <ResizablePanel defaultSize={`${normalizedArtifactSize}`} minSize="20" maxSize="60" id="share-artifacts">
 ...
 */
function ShareArtifactsPanel({ contextValue }) {
    return (<ArtifactsProvider value={contextValue}>
      <EditorProvider>
 ...
 */
function ShareArtifactsOverlay({ contextValue }) {
    return (<div className="fixed inset-y-0 right-0 z-40 flex w-full max-w-full sm:max-w-[420px]" role="complementary" aria-label="Artifacts panel">
