...
export const code = memo(({ inline, className, children }) => {
...
export const CodeMarkdown = memo(({ content = '', isSubmitting }) => {
    ...
    return (<div ref={scrollRef} className="max-h-full overflow-y-auto">
        <ReactMarkdown 
    /* @ts-expect-error — rehypePlugins type mismatch between react-markdown and unified PluggableList */
    rehypePlugins={rehypePlugins} components={{ code }}>
          {content}
        </ReactMarkdown>
      </div>);
});
