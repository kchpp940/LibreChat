...
const ArtifactButton = ({ artifact }) => {
    ...
    const debouncedSetVisibleRef = useRef(debounce((artifactToSet) => {
        logger.log('artifacts_visibility', 'Setting artifact to visible state from Artifact button', artifactToSet);
    ...
    return (<div className="group relative my-4 rounded-xl text-sm text-text-primary">
      {(() => {
            const handleClick = () => {
                if (isSelected) {
            ...
            const buttonClass = cn('relative overflow-hidden rounded-xl transition-all duration-300 hover:border-border-medium hover:bg-surface-hover hover:shadow-lg active:scale-[0.98]', {
            ...
            return (<button type="button" onClick={handleClick} className={buttonClass}>
