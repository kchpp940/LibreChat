...
let mockCapturedCache = null;
jest.mock('react-virtualized', () => {
    const actual = jest.requireActual('react-virtualized');
    return {
        ...actual,
        CellMeasurer: ({ children, }) => children({ registerChild: () => { } }),
        List: ({ rowRenderer, rowCount, deferredMeasurementCache, }) => {
            mockCapturedCache = deferredMeasurementCache;
            return (<div data-testid="virtual-list" data-row-count={rowCount}>
...
describe('Conversations – favorites CellMeasurerCache key invalidation', () => {
    ...
    const Wrapper = () => (<RecoilRoot>
      <Conversations conversations={[]} moveToTop={jest.fn()} toggleNav={jest.fn()} containerRef={containerRef} loadMoreConversations={jest.fn()} isLoading={false} isSearchLoading={false} isChatsExpanded={true} setIsChatsExpanded={jest.fn()}/>
    </RecoilRoot>);
