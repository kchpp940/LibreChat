...
import { useLocalize, useFavorites, useShowMarketplace, useNewConvo, useElementSize, } from '~/hooks';
...
const MeasuredRow = memo(({ cache, rowKey, parent, index, style, children }) => (<CellMeasurer cache={cache} columnIndex={0} key={rowKey} parent={parent} rowIndex={index}>
      {({ registerChild }) => (<div ref={registerChild} style={style} className="px-3">
          {children}
        </div>)}
    </CellMeasurer>));
...
const ChatsHeader = memo(({ isExpanded, onToggle }) => {
    ...
    return (<div className="flex h-8 w-full items-center gap-0.5 pr-2">
      <button onClick={onToggle} className="group flex min-w-0 flex-1 items-center gap-1 rounded-lg px-1 py-2 text-xs font-bold text-text-secondary outline-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-black dark:focus-visible:ring-white" type="button" aria-expanded={isExpanded}>
      ...
      <TooltipAnchor description={localize('com_ui_new_chat')} render={<button type="button" aria-label={localize('com_ui_new_chat')} className={headerIconButtonClassName} onClick={handleNewChat}>
            <NewChatIcon className="h-4 w-4"/>
          </button>}/>
    </div>);
});
ChatsHeader.displayName = 'ChatsHeader';
const DateLabel = memo(({ groupName, isFirst }) => {
    const localize = useLocalize();
    return (<h2 aria-label={localize('com_a11y_chats_date_section', {
            date: localize(groupName) || groupName,
        })} className={cn('pl-1 pt-1 text-text-secondary', isFirst === true ? 'mt-0' : 'mt-2')} style={{ fontSize: '0.7rem' }}>
      {localize(groupName) || groupName}
    </h2>);
});
DateLabel.displayName = 'DateLabel';
const Conversations = ({ conversations: rawConversations, moveToTop, toggleNav, containerRef, loadMoreConversations, isLoading, isSearchLoading, isChatsExpanded, setIsChatsExpanded, showFavorites = true, }) => {
    const localize = useLocalize();
    ...
    const { ref: listContainerRef, width: listWidth, height: listHeight, } = useElementSize();
    ...
    const shouldShowFavorites = showFavorites &&
        !search.query &&
    ...
    const filteredConversations = useMemo(() => rawConversations.filter(Boolean), [rawConversations]);
    ...
    const flattenedItems = useMemo(() => {
        ...
        if (isChatsExpanded) {
            groupedConversations.forEach(([groupName, convos]) => {
                ...
                items.push(...convos.map((convo) => ({ type: 'convo', convo })));
            });
            if (isLoading) {
    ...
    const rowRenderer = useCallback(({ index, key, parent, style }) => {
        ...
        if (item.type === 'loading') {
            return (<MeasuredRow key={key} {...rowProps}>
            <LoadingSpinner />
          </MeasuredRow>);
        }
        if (item.type === 'favorites') {
        ...
        if (item.type === 'convo') {
            ...
            return (<MeasuredRow key={key} {...rowProps}>
            <Convo conversation={item.convo} retainView={moveToTop} toggleNav={toggleNav} isGenerating={isGenerating}/>
          </MeasuredRow>);
        }
        return null;
    }, [cache, flattenedItems, moveToTop, toggleNav, isSmallScreen, activeJobIds]);
    const getRowHeight = useCallback(({ index }) => cache.getHeight(index, 0), [cache]);
    ...
    return (<div className="relative flex h-full min-h-0 flex-col pb-2 text-sm text-text-primary">
      <div className="px-3">
        <ChatsHeader isExpanded={isChatsExpanded} onToggle={() => setIsChatsExpanded(!isChatsExpanded)}/>
      </div>
      {isSearchLoading ? (<div className="flex flex-1 items-center justify-center">
          <Spinner className="text-text-primary"/>
          <span className="ml-2 text-text-primary">{localize('com_ui_loading')}</span>
        </div>) : (<div ref={listContainerRef} className="min-h-0 flex-1 overflow-hidden">
          <List ref={containerRef} width={listWidth} height={listHeight} deferredMeasurementCache={cache} rowCount={flattenedItems.length} rowHeight={getRowHeight} rowRenderer={rowRenderer} overscanRowCount={10} aria-readonly={false} className="outline-none" aria-label="Conversations" onRowsRendered={handleRowsRendered} tabIndex={-1} style={{ outline: 'none' }} containerRole="rowgroup"/>
        </div>)}
    </div>);
};
export { DateLabel };
