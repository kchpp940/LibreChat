 ...
 */
const parseFieldConfig = (variable) => {
...
export default function VariableForm({ group, onClose, }) {
    ...
    const { allVariables, uniqueVariables, variableIndexMap } = useMemo(() => extractVariableInfo(mainText), [mainText]);
    ...
    const generateHighlightedMarkdown = () => {
        ...
        allVariables.forEach((variable) => {
            ...
            const fieldIndex = variableIndexMap.get(variable);
    ...
    return (<div className="mx-auto p-1 md:container">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="mb-6 max-h-screen max-w-[80vw] overflow-auto rounded-md bg-surface-tertiary p-4 text-text-secondary dark:bg-surface-primary sm:max-w-full md:max-h-96">
          <ReactMarkdown 
    /** @ts-ignore */
    remarkPlugins={[supersub, remarkGfm, [remarkMath, { singleDollarTextMath: false }]]} rehypePlugins={[
            /** @ts-ignore */
            [rehypeKatex],
            /** @ts-ignore */
    ...
    /** @ts-ignore */
    components={{ code: codeNoExecution, p: PromptVariableGfm }} className="markdown prose dark:prose-invert light my-1 max-h-[50vh] max-w-full break-words dark:text-text-secondary">
            {generateHighlightedMarkdown()}
          </ReactMarkdown>
        </div>
        <div className="space-y-4">
          {fields.map((field, index) => (<div key={field.id} className="relative flex flex-col space-y-2">
              <Controller name={`fields.${index}.value`} control={control} render={({ field: { onChange, onBlur, value, ref } }) => {
                if (field.config.type === 'select') {
                    return (<InputCombobox options={field.config.options || []} placeholder={field.config.variable} className={cn(defaultTextProps, 'mb-1 rounded px-3 py-2 focus:bg-surface-tertiary')} value={value} onChange={onChange} onBlur={onBlur} aria-label={field.config.variable}/>);
                }
                return (<>
                      <TextareaAutosize ref={ref} value={value} onChange={onChange} onBlur={onBlur} id={`fields.${index}.value`} className={cn(defaultTextProps, 'peer rounded px-3 py-2 focus:bg-surface-tertiary')} placeholder="" maxRows={8} aria-label={field.config.variable}/>
                      <label htmlFor={`fields.${index}.value`} className="absolute left-3 top-0 text-sm text-text-secondary transition-all duration-200 peer-focus:-top-6 peer-focus:left-1 peer-focus:text-xs peer-focus:text-text-primary peer-[:not(:placeholder-shown)]:-top-6 peer-[:not(:placeholder-shown)]:left-1 peer-[:not(:placeholder-shown)]:text-xs">
