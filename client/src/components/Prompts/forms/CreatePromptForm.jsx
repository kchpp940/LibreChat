...
const CreatePromptForm = ({ defaultValues = defaultPrompt, onSuccess, }) => {
    ...
    const { watch, control, handleSubmit, formState: { isDirty, isSubmitting, errors, isValid }, } = methods;
    ...
    return (<FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)} className="w-full px-4 py-2">
        ...
        {isSmallScreen ? (<div className="mb-2 flex items-center justify-between gap-2">
            <OpenSidebar />
            <CategorySelector />
          </div>) : null}
        <div className="mb-1 flex flex-col items-center justify-between font-bold sm:text-xl md:mb-0 md:text-2xl">
          <div className="flex w-full flex-col items-center justify-between sm:flex-row">
            <Controller name="name" control={control} rules={{ required: localize('com_ui_prompt_name_required') }} render={({ field }) => (<div className="relative mb-1 flex w-full flex-col sm:w-auto md:mb-0">
                  <Input {...field} id="prompt-name" type="text" className="peer mr-2 w-full border border-border-medium p-2 text-2xl text-text-primary" placeholder=" " tabIndex={0} aria-label={localize('com_ui_prompt_name')} aria-required="true"/>
                  ...
                  <div className={cn('mt-1 w-56 text-sm text-red-500', errors.name ? 'visible h-auto' : 'invisible h-0')}>
        ...
        <div className="flex w-full flex-col gap-4 md:mt-[1.075rem]">
          <div className="flex flex-col">
            ...
            <div className="min-h-32 rounded-b-xl border border-t-0 border-border-medium p-3 sm:p-4">
              <Controller name="prompt" control={control} rules={{ required: localize('com_ui_prompt_text_required') }} render={({ field }) => (<div>
                    <TextareaAutosize {...field} className="w-full resize-none overflow-y-auto bg-transparent font-mono text-sm leading-relaxed text-text-primary placeholder:text-text-tertiary focus:outline-none focus-visible:ring-2 focus-visible:ring-ring-primary sm:text-base" minRows={4} maxRows={16} tabIndex={0} placeholder={localize('com_ui_prompt_input')} aria-label={localize('com_ui_prompt_input_field')} aria-required="true"/>
                    <div className={cn('mt-1 text-sm text-red-500', errors.prompt ? 'visible h-auto' : 'invisible h-0')}>
          ...
          <Description onValueChange={(value) => methods.setValue('oneliner', value)} tabIndex={0}/>
          <Command onValueChange={(value) => methods.setValue('command', value)} tabIndex={0}/>
          <div className="mt-4 flex justify-end">
            <Button aria-label={localize('com_ui_create_prompt')} className={cn('w-full sm:w-auto', (!isDirty || isSubmitting || !isValid) && 'opacity-50')} tabIndex={0} type="submit" aria-disabled={!isDirty || isSubmitting || !isValid || undefined} onClick={(e) => {
