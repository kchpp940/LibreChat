...
import { useGetPrompts, useGetPromptGroup, useAddPromptToGroup, useUpdatePromptGroup, useMakePromptProduction, } from '~/data-provider';
...
const VersionsPanel = React.memo(({ group, prompts, selectedPrompt, isLoadingPrompts, canEdit, selectionIndex, setSelectionIndex, }) => {
    ...
    return (<div className="flex h-full w-full flex-col overflow-hidden">
        {canEdit && (<div className="shrink-0 px-4 py-2">
            <Button variant="submit" size="sm" aria-label={localize('com_ui_make_production')} className={cn('w-full gap-1.5 transition-all duration-200', isProductionVersion &&
                'border border-green-500/30 bg-green-50 text-green-700 hover:bg-green-100 dark:bg-green-950/30 dark:text-green-400 dark:hover:bg-green-950/50')} onClick={() => {
                if (!selectedPrompt) {
                    return;
                }
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2">
          {isLoadingPrompts &&
            Array.from({ length: 6 }).map((_, index) => (<div key={index} className="my-2">
          ...
          {!isLoadingPrompts && prompts.length > 0 && (<>
              ...
              <PromptVersions group={group} prompts={prompts} selectionIndex={selectionIndex} setSelectionIndex={setSelectionIndex}/>
            </>)}
        </div>
      </div>);
});
VersionsPanel.displayName = 'VersionsPanel';
const HeaderActions = React.memo(({ group, canEdit, canDelete, selectedPromptId, onCategoryChange }) => {
    ...
    return (<div className="flex items-center gap-2">
        <CategorySelector currentCategory={groupCategory} onValueChange={canEdit ? onCategoryChange : undefined}/>
        {hasShareAccess && <SharePrompt group={group} disabled={isLoadingGroup}/>}
        {canDelete && (<DeletePrompt promptId={selectedPromptId} groupId={groupId} promptName={group?.name || ''} disabled={isLoadingGroup}/>)}
      </div>);
});
HeaderActions.displayName = 'HeaderActions';
const PromptForm = ({ promptId: promptIdProp }) => {
    ...
    const { data: prompts = [], isLoading: isLoadingPrompts } = useGetPrompts({ groupId: promptId }, { enabled: hasAccess && !!promptId });
    const { hasPermission, isLoading: permissionsLoading } = useResourcePermissions(ResourceType.PROMPTGROUP, group?._id || '');
    ...
    const selectedPrompt = useMemo(() => (prompts.length > 0 ? prompts[selectionIndex] : undefined), [prompts, selectionIndex]);
    ...
    const debouncedUpdateOneliner = useMemo(() => debounce((groupId, oneliner, mutate) => {
        mutate({ id: groupId, payload: { oneliner } });
    }, 950), []);
    const debouncedUpdateCommand = useMemo(() => debounce((groupId, command, mutate) => {
    ...
    const handleUpdateOneliner = useCallback((oneliner) => {
        if (!group || !group._id) {
            return;
        }
    ...
    const handleUpdateCommand = useCallback((command) => {
        if (!group || !group._id) {
            return;
        }
    ...
    if (!canEdit && !permissionsLoading && groupsQuery?.data) {
        ...
        if (fetchedPrompt || group) {
          ...
          {isSmallScreen && (<div className="flex shrink-0 items-center px-4 pt-3">
              <OpenSidebar />
            </div>)}
          ...
          <div className="h-full w-full" style={{
            ...
            <div className="flex h-full">
              <div className="flex-1 overflow-hidden px-4">
                {/* Mobile Actions Row */}
                {!isLoadingGroup && group && isSmallScreen && (<div className="mb-3 mt-2 flex items-center justify-between gap-2">
                    <OpenSidebar />
                    <HeaderActions group={group} canEdit={canEdit} canDelete={canDelete} selectedPromptId={selectedPromptId} onCategoryChange={handleCategoryChange}/>
                  </div>)}
                {/* Header: Title + Actions */}
                <div className="mb-3 mt-2 flex items-center justify-between gap-2">
                  {isLoadingGroup ? (<Skeleton className="h-9 w-48"/>) : (<>
                      <div className="flex min-w-0 flex-1 items-center gap-2">
                        <PromptName name={groupName} isLoading={updateGroupMutation.isLoading} isError={updateGroupMutation.isError} onSave={(value) => {
                ...
                updateGroupMutation.mutate({
                    ...
                    payload: { name: value },
                        ...
                        {editorMode === PromptsEditorMode.ADVANCED && (<Button ref={sidePanelTriggerRef} type="button" variant="outline" size="sm" className="shrink-0 lg:hidden" onClick={() => setShowSidePanel(true)} aria-label={localize('com_ui_versions')}>
                            <Menu className="size-4 sm:mr-1.5" aria-hidden="true"/>
                            <span className="hidden sm:inline">{localize('com_ui_versions')}</span>
                          </Button>)}
                      </div>
                      {!isSmallScreen && (<div className="shrink-0">
                          <HeaderActions group={group} canEdit={canEdit} canDelete={canDelete} selectedPromptId={selectedPromptId} onCategoryChange={handleCategoryChange}/>
                        </div>)}
                    </>)}
                </div>
                ...
                {/* Main Editor Content */}
                {isLoadingPrompts ? (<Skeleton className="h-96" aria-live="polite"/>) : (<div className="mb-2 flex h-full flex-col gap-3">
                    <PromptEditor name="prompt" isEditing={isEditing} setIsEditing={(value) => canEdit && setIsEditing(value)}/>
                    <PromptVariables promptText={promptText}/>
                    <Description initialValue={group.oneliner ?? ''} onValueChange={canEdit ? handleUpdateOneliner : undefined} disabled={!canEdit}/>
                    <Command initialValue={group.command ?? ''} onValueChange={canEdit ? handleUpdateCommand : undefined} disabled={!canEdit}/>
                  </div>)}
              </div>

              {/* Versions Sidebar - Advanced Mode Only */}
              {editorMode === PromptsEditorMode.ADVANCED && (<div className="hidden w-72 shrink-0 border-l border-border-medium lg:block xl:w-80">
                  <VersionsPanel group={group} prompts={prompts} selectionIndex={selectionIndex} selectedPrompt={selectedPrompt} isLoadingPrompts={isLoadingPrompts} canEdit={canEdit} setSelectionIndex={setSelectionIndex}/>
                </div>)}
            </div>
          </div>

          {/* Mobile Overlay */}
          <div aria-hidden={!showSidePanel} className={cn('fixed inset-0 z-[100] bg-black/20 lg:hidden', showSidePanel ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0')} style={{ transition: 'opacity 300ms cubic-bezier(0.2, 0, 0, 1)' }}>
            <button type="button" className="h-full w-full cursor-default" onClick={() => setShowSidePanel(false)} aria-label={localize('com_ui_close_menu')} tabIndex={showSidePanel ? 0 : -1}/>
          </div>

          {/* Mobile Versions Panel */}
          <div ref={sidePanelRef} className={cn('fixed right-0 top-0 z-[110] flex h-full flex-col border-l border-border-medium bg-surface-primary-alt shadow-xl lg:hidden', showSidePanel ? 'translate-x-0' : 'translate-x-full')} style={{
            width: 'min(85vw, 380px)',
            transition: 'transform 300ms cubic-bezier(0.2, 0, 0, 1)',
        }} role="dialog" aria-modal="true" aria-label={localize('com_ui_versions')} inert={!showSidePanel ? '' : undefined}>
            <div className="flex items-center justify-between px-4 py-2">
              <h2 className="text-sm font-semibold text-text-primary">
                {localize('com_ui_versions')}
              </h2>
              <Button type="button" variant="ghost" size="icon" onClick={() => setShowSidePanel(false)} aria-label={localize('com_ui_close')} className="size-8">
