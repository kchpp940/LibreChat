...
const DropdownVariableCard = ({ parsed }) => {
    const localize = useLocalize();
    return (<div className="bg-surface-secondary/50 rounded-lg border border-border-medium p-2.5 hover:bg-surface-secondary" role="listitem" aria-label={localize('com_ui_variable_with_options', {
            name: parsed.name,
      ...
      <div className="flex flex-wrap gap-1.5" role="list" aria-label={localize('com_ui_available_options')}>
        {parsed.options.map((option, index) => (<span key={index} className="rounded-md border border-border-medium bg-transparent px-2 py-0.5 text-xs text-text-secondary transition-colors hover:bg-surface-secondary" role="listitem">
            {option}
          </span>))}
      </div>
    </div>);
};
const SpecialVariableChip = ({ parsed }) => {
    const localize = useLocalize();
    const Icon = getSpecialVariableIcon(parsed.name);
    const labelKey = `com_ui_special_var_${parsed.name}`;
    const descKey = `com_ui_special_var_desc_${parsed.name}`;
    const displayLabel = localize(labelKey);
    const description = localize(descKey);
    return (<div className="group flex items-start gap-2 rounded-lg border border-border-medium bg-transparent p-2 hover:bg-surface-secondary" role="listitem" aria-label={displayLabel}>
...
const SimpleVariableChip = ({ parsed }) => (<span className="bg-surface-secondary/50 inline-flex items-center gap-1.5 rounded-lg border border-border-medium px-2.5 py-1.5 text-xs font-medium text-text-primary hover:bg-surface-tertiary" role="listitem">
    <Variable className="size-3 text-text-secondary" aria-hidden="true"/>
    <span className="max-w-32 truncate">{parsed.name}</span>
  </span>);
const PromptVariables = ({ promptText }) => {
    const localize = useLocalize();
    ...
    return (<div className="overflow-hidden rounded-xl border border-border-medium">
      ...
      <div className="flex flex-col gap-4 p-3" role="list" aria-label={localize('com_ui_prompt_variables_list')}>
        {specialVars.length > 0 && (<section aria-label={localize('com_ui_special_variables')}>
            <h5 className="mb-2 text-[11px] font-medium uppercase tracking-wide text-text-secondary">
              {localize('com_ui_special_variables')}
            </h5>
            <div className="grid gap-2 sm:grid-cols-2">
              {specialVars.map((parsed, index) => (<SpecialVariableChip key={`special-${index}`} parsed={parsed}/>))}
            </div>
          </section>)}

        {dropdownVariables.length > 0 && (<section aria-label={localize('com_ui_dropdown_variables')}>
            <h5 className="mb-2 text-[11px] font-medium uppercase tracking-wide text-text-secondary">
              {localize('com_ui_dropdown_variables')}
            </h5>
            <div className="flex flex-col gap-2">
              {dropdownVariables.map((parsed, index) => (<DropdownVariableCard key={`dropdown-${index}`} parsed={parsed}/>))}
            </div>
          </section>)}

        {simpleVariables.length > 0 && (<section aria-label={localize('com_ui_text_variables')}>
            <h5 className="mb-2 text-[11px] font-medium uppercase tracking-wide text-text-secondary">
              {localize('com_ui_text_variables')}
            </h5>
            <div className="flex flex-wrap gap-2">
              {simpleVariables.map((parsed, index) => (<SimpleVariableChip key={`simple-${index}`} parsed={parsed}/>))}
            </div>
          </section>)}
      </div>
    </div>);
