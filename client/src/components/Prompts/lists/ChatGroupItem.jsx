...
import { Label, Button, Spinner, OGDialog, TooltipAnchor, DropdownPopup, OGDialogTemplate, useToastContext, } from '@librechat/client';
...
function ChatGroupItem({ group, isChatRoute = true, }) {
    ...
    const snippet = typeof group.oneliner === 'string' && group.oneliner.length > 0
        ? group.oneliner
    ...
    return (<>
      <div className={cn('group/prompt relative mb-1.5 rounded-xl border border-border-light bg-transparent transition-colors hover:bg-surface-secondary', !isChatRoute && params.promptId === group._id && 'bg-surface-hover')}>
        {/* Clickable overlay for card */}
        <button type="button" className="absolute inset-0 z-0 rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-ring-primary" onClick={onCardClick} aria-label={ariaLabel}/>
        <div className="flex items-start gap-2.5 px-3 py-2.5">
          <CategoryIcon category={group.category ?? ''} className="mt-0.5 size-4 shrink-0" aria-hidden="true"/>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5">
              <span className="truncate text-sm font-semibold text-text-primary" title={group.name}>
                {group.name}
              </span>
              {isSharedPrompt && (<TooltipAnchor description={localize('com_ui_by_author', { 0: group.authorName })} side="top" render={<span tabIndex={0} role="img" aria-label={localize('com_ui_by_author', { 0: group.authorName })} className="flex shrink-0 items-center">
                      <User className="size-3.5 text-text-secondary" aria-hidden="true"/>
                    </span>}/>)}
              {groupIsGlobal && (<TooltipAnchor description={localize('com_ui_sr_global_prompt')} side="top" render={<span tabIndex={0} role="img" aria-label={localize('com_ui_sr_global_prompt')} className="flex shrink-0 items-center">
          ...
          <div className="relative z-10 shrink-0">
            <DropdownPopup portal={true} menuId={menuId} focusLoop={true} className="z-[125]" unmountOnHide={true} isOpen={menuOpen} setIsOpen={setMenuOpen} trigger={<Ariakit.MenuButton ref={menuButtonRef} aria-label={localize('com_nav_convo_menu_options')} className={cn('flex size-7 items-center justify-center rounded-md text-text-secondary transition-opacity hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring-primary', menuOpen
                ? 'opacity-100'
                : 'opacity-0 focus-visible:opacity-100 group-hover/prompt:opacity-100')} onClick={(e) => e.stopPropagation()}>
                  <Ellipsis className="size-4" aria-hidden="true"/>
                </Ariakit.MenuButton>} items={dropdownItems}/>
          </div>
        </div>
      </div>
      <PreviewPrompt group={group} open={isPreviewDialogOpen} onOpenChange={setPreviewDialogOpen} onCloseAutoFocus={() => {
            requestAnimationFrame(() => {
                menuButtonRef.current?.focus({ preventScroll: true });
            });
        }}/>
      {isChatRoute && (<VariableDialog open={isVariableDialogOpen} onClose={() => setVariableDialogOpen(false)} group={group}/>)}
      <OGDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <OGDialogTemplate title={localize('com_ui_delete_prompt')} className="w-11/12 max-w-md" main={<Label>{localize('com_ui_prompt_delete_confirm', { 0: group.name })}</Label>} selection={<Button onClick={handleDelete} variant="destructive" disabled={deleteGroup.isLoading}>
