...
export default function FilterPrompts({ className = '', dropdownClassName = '', }) {
    ...
    return (<div role="search" className={cn('flex items-center gap-2', className)}>
      <div aria-live="polite" aria-atomic="true" className="sr-only">
        {searchResultsAnnouncement}
      </div>
      <Dropdown value={categoryFilter || SystemCategories.ALL} onChange={onSelect} options={filterOptions} className={cn('shrink-0 rounded-lg bg-transparent [&>button]:size-9', dropdownClassName)} icon={<ListFilter className="h-4 w-4"/>} label="Filter: " ariaLabel={localize('com_ui_filter_prompts')} iconOnly/>
      <FilterInput inputId="prompts-filter" label={localize('com_ui_filter_prompts_name')} value={searchTerm} onChange={handleSearchChange} containerClassName="flex-1"/>
      <CreatePromptButton />
    </div>);
}
