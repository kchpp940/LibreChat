...
export default function VariablesDropdown({ fieldName = 'prompt', className = '', }) {
    const menuId = useId();
    ...
    const handleAddVariable = useCallback((key) => {
        const currentText = getValues(fieldName) || '';
        const spacer = currentText.length > 0 ? '\n\n' : '';
        const labelKey = `com_ui_special_var_${key}`;
    ...
    const items = useMemo(() => variableKeys.map((key) => {
        ...
        return {
            ...
            render: (<div className="flex w-full items-start gap-2.5 py-0.5">
              <div className={`flex size-7 shrink-0 items-center justify-center rounded-md ${iconClass}`}>
                {isUsed ? (<Check className="size-3.5" aria-hidden="true"/>) : (<Icon className="size-3.5" aria-hidden="true"/>)}
              </div>
    ...
    const usedCount = useMemo(() => variableKeys.filter((key) => usedVariables.has(key)).length, [usedVariables]);
    ...
    return (<div className={className}>
      <DropdownPopup portal={true} mountByState={true} unmountOnHide={true} preserveTabOrder={true} isOpen={isMenuOpen} setIsOpen={setIsMenuOpen} trigger={<Menu.MenuButton id="variables-menu-button" aria-label={localize('com_ui_add_special_variables')} className={`group flex h-8 items-center gap-1.5 rounded-lg bg-transparent px-2 text-sm ${buttonClass}`}>
            <Sparkles className="size-3.5 text-text-secondary" aria-hidden="true"/>
            <span className="hidden text-xs font-medium sm:inline">
              {localize('com_ui_special_variables')}
            </span>
            {usedCount > 0 && (<span className="flex size-4 items-center justify-center rounded-full bg-surface-tertiary text-[10px] font-medium text-text-secondary">
                {usedCount}
              </span>)}
            <ChevronDown className={`size-3 transition-transform duration-200 ${isMenuOpen ? 'rotate-180' : ''}`} aria-hidden="true"/>
          </Menu.MenuButton>} items={items} menuId={menuId} className="z-50 w-64" itemClassName="px-2 py-1"/>
    </div>);
}
