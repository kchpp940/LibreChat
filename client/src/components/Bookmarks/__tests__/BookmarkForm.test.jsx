...
jest.mock('@librechat/client', () => {
    ...
    return {
        Checkbox: ({ checked, onCheckedChange, value, ...props }) => ActualReact.createElement('input', {
            type: 'checkbox',
            checked,
        ...
        Label: ({ children, ...props }) => ActualReact.createElement('label', props, children),
        TextareaAutosize: ActualReact.forwardRef((props, ref) => ActualReact.createElement('textarea', { ref, ...props })),
        Input: ActualReact.forwardRef((props, ref) => ActualReact.createElement('input', { ref, ...props })),
        useToastContext: () => ({
...
describe('BookmarkForm - Bookmark Editing', () => {
    ...
    describe('Editing only the description (tag unchanged)', () => {
        it('should allow submitting when only the description is changed', async () => {
            ...
            render(<BookmarkForm bookmark={existingBookmark} mutation={createMockMutation()} setOpen={mockSetOpen} formRef={formRef}/>);
    ...
    describe('Renaming a tag to an existing tag (should show error)', () => {
        ...
        it('should show error toast when renaming to an existing tag name (via tags prop)', async () => {
            ...
            mockGetQueryData.mockReturnValue([existingBookmark]);
            render(<BookmarkForm tags={['Existing Tag', 'Another Tag']} bookmark={existingBookmark} mutation={createMockMutation()} setOpen={mockSetOpen} formRef={formRef}/>);
            const tagInput = screen.getByLabelText('Title');
            await act(async () => {
                fireEvent.change(tagInput, { target: { value: 'Existing Tag' } });
            });
    ...
    describe('Validation interaction between different data sources', () => {
        it('should check both tags prop and allTags query data for duplicates', async () => {
            ...
            render(<BookmarkForm tags={['Props Tag']} bookmark={existingBookmark} mutation={createMockMutation()} setOpen={mockSetOpen} formRef={formRef}/>);
            const tagInput = screen.getByLabelText('Title');
            await act(async () => {
        ...
        it('should not trigger mutation when mutation is loading', async () => {
            ...
            mockGetQueryData.mockReturnValue([existingBookmark]);
            render(<BookmarkForm bookmark={existingBookmark} mutation={createMockMutation(true)} setOpen={mockSetOpen} formRef={formRef}/>);
            const descriptionInput = screen.getByRole('textbox', { name: /description/i });
            await act(async () => {
                fireEvent.change(descriptionInput, { target: { value: 'Updated description' } });
            });
