...
const CodeBlock = ({ lang, blockIndex, codeChildren, classProp = '', allowExecution = true, plugin = null, error, }) => {
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(([entry]) => setIsCodeBarVisible(entry.isIntersecting), { root: null, threshold: 0 });
        observer.observe(el);
    ...
    return (<div ref={containerRef} className="relative w-full overflow-hidden rounded-xl border border-border-light text-xs" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} onFocus={handleFocus} onBlur={handleBlur}>
      <div ref={codeBarRef}>
        <CodeBar lang={lang} error={error} codeRef={codeRef} blockIndex={blockIndex} plugin={plugin === true} allowExecution={allowExecution}/>
      </div>
      <div className={cn(classProp, 'overflow-y-auto bg-surface-chat p-4 dark:bg-surface-primary-alt')}>
        <code ref={codeRef} className={cn(isNonCode ? '!whitespace-pre-wrap' : `hljs language-${language} !whitespace-pre`)}>
          {codeChildren}
        </code>
      </div>
      <FloatingCodeBar lang={lang} error={error} codeRef={codeRef} blockIndex={blockIndex} plugin={plugin === true} allowExecution={allowExecution} isVisible={showFloating}/>
          ...
          <div className="border-t border-border-light bg-surface-primary-alt p-4 text-xs dark:bg-transparent">
            ...
            <div className="flex flex-col-reverse text-text-primary">
              <pre className="shrink-0">
                <LogContent output={currentToolCall?.result ?? ''} attachments={currentToolCall?.attachments ?? []} renderImages={true}/>
              </pre>
            </div>
          </div>
          {toolCalls.length > 1 && (<ResultSwitcher currentIndex={currentIndex} totalCount={toolCalls.length} onPrevious={previous} onNext={next}/>)}
        </>)}
    </div>);
};
