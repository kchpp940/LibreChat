...
jest.mock('../../Chat/Messages/Content/UIResourceCarousel', () => ({
    __esModule: true,
    default: ({ uiResources }) => (<div data-testid="ui-resource-carousel" data-resource-count={uiResources.length}>
      {uiResources.map((resource, index) => (<div key={index} data-testid={`resource-${index}`} data-resource-uri={resource.uri}/>))}
    </div>),
}));
...
describe('MCPUIResourceCarousel', () => {
    ...
    describe('multiple resource fetching', () => {
        it('should fetch resources by resourceIds across conversation messages', () => {
            ...
            renderWithRecoil(<MCPUIResourceCarousel node={{ properties: { resourceIds: ['id-2', 'id-1'] } }}/>);
            const carousel = screen.getByTestId('ui-resource-carousel');
            expect(carousel).toHaveAttribute('data-resource-count', '2');
            expect(screen.getByTestId('resource-0')).toHaveAttribute('data-resource-uri', 'ui://test/resource-id2');
            expect(screen.getByTestId('resource-1')).toHaveAttribute('data-resource-uri', 'ui://test/resource-id1');
        });
    });
    describe('error handling', () => {
        it('should return null when no attachments', () => {
            ...
            const { container } = renderWithRecoil(<MCPUIResourceCarousel node={{ properties: { resourceIds: ['id1', 'id2'] } }}/>);
        ...
        it('should return null when resources not found', () => {
            ...
            const { container } = renderWithRecoil(<MCPUIResourceCarousel node={{ properties: { resourceIds: ['non-existent-id'] } }}/>);
            expect(container.firstChild).toBeNull();
        });
    ...
    describe('edge cases', () => {
        it('should handle empty resourceIds array', () => {
            ...
            const { container } = renderWithRecoil(<MCPUIResourceCarousel node={{ properties: { resourceIds: [] } }}/>);
            expect(container.firstChild).toBeNull();
        });
        it('should handle duplicate resource IDs', () => {
            ...
            renderWithRecoil(<MCPUIResourceCarousel node={{ properties: { resourceIds: ['id-a', 'id-a', 'id-b', 'id-b', 'id-a'] } }}/>);
            const carousel = screen.getByTestId('ui-resource-carousel');
        ...
        it('should handle null messages data', () => {
            currentTestMessages = [];
            const { container } = renderWithRecoil(<MCPUIResourceCarousel node={{ properties: { resourceIds: ['test-id'] } }}/>);
            expect(container.firstChild).toBeNull();
        });
