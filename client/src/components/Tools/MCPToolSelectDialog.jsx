...
function MCPToolSelectDialog({ isOpen, agentId, setIsOpen, mcpServerNames, }) {
    ...
    const [configuringServer, setConfiguringServer] = useState(null);
    const { maxPage, setMaxPage, currentPage, setCurrentPage, itemsPerPage, searchChanged, setSearchChanged, searchValue, setSearchValue, gridRef, handleSearch, handleChangePage, error, setError, errorMessage, setErrorMessage, } = usePluginDialogHelpers();
    ...
    const handleDirectAdd = async (serverName, authData) => {
        try {
    ...
    const addToolsToForm = async (serverName) => {
        const { data: updatedMCPData } = await refetchMCPTools();
        const currentTools = getValues('tools') || [];
        const toolsToAdd = [`${Constants.mcp_server}${Constants.mcp_delimiter}${serverName}`];
    ...
    const handleSaveCustomVars = async (serverName, authData) => {
        try {
            ...
            await handleDirectAdd(serverName, Object.keys(filteredAuthData).length > 0 ? filteredAuthData : undefined);
    ...
    const handleRevokeCustomVars = (serverName) => {
        setIsSavingCustomVars(true);
    ...
    const onAddTool = async (serverName) => {
        ...
        const hasCustomUserVars = serverConfig?.customUserVars && Object.keys(serverConfig.customUserVars).length > 0;
    ...
    const filteredServers = useMemo(() => {
        ...
        return mcpServers.filter((serverInfo) => serverInfo.serverName.toLowerCase().includes(searchValue.toLowerCase()));
    ...
    return (<Dialog open={isOpen} onClose={() => {
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="relative max-h-[90vh] w-full transform overflow-hidden overflow-y-auto rounded-lg bg-surface-secondary text-left shadow-xl transition-all max-sm:h-full sm:mx-7 sm:my-8 sm:max-w-2xl lg:max-w-5xl xl:max-w-7xl" style={{ minHeight: '610px' }}>
          <div className="flex items-center justify-between border-b-[1px] border-border-medium px-4 pb-4 pt-5 sm:p-6">
            ...
            setConfiguringServer(null);
            setIsInitializing(null);
        }} className="inline-block rounded-full text-text-secondary transition-colors hover:text-text-primary" aria-label="Close dialog" type="button">
                <X aria-hidden="true"/>
              </button>
            </div>
          </div>

          {error && (<div className="relative m-4 rounded border border-red-400 bg-red-100 px-4 py-3 text-red-700" role="alert">
              {localize('com_nav_plugin_auth_error')} {errorMessage}
            </div>)}

          {configuringServer && (<div className="p-4 sm:p-6 sm:pt-4">
              <div className="mb-4">
                <p className="text-sm text-text-secondary">
                  {localize('com_ui_mcp_configure_server_description', { 0: configuringServer })}
                </p>
              </div>
              <CustomUserVarsSection serverName={configuringServer} isSubmitting={isSavingCustomVars} fields={availableMCPServersMap?.[configuringServer]?.customUserVars || {}} onSave={(authData) => handleSaveCustomVars(configuringServer, authData)} onRevoke={() => handleRevokeCustomVars(configuringServer)}/>
            </div>)}

          <div className="p-4 sm:p-6 sm:pt-4">
            <div className="mt-4 flex flex-col gap-4">
              <div className="flex items-center justify-center space-x-4" onClick={() => setConfiguringServer(null)}>
                <Search className="h-6 w-6 text-text-tertiary"/>
                <input type="text" value={searchValue} onChange={handleSearch} placeholder={localize('com_nav_tool_search')} className="w-64 rounded border border-border-medium bg-transparent px-2 py-1 text-text-primary focus:outline-none"/>
              </div>

              <div ref={gridRef} className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" style={{ minHeight: '410px' }}>
                {filteredServers
            ...
            return (<MCPToolItem tool={tool} isInstalled={isInstalled} key={serverInfo.serverName} isConfiguring={isConfiguring} isInitializing={isServerInitializing} onAddTool={() => onAddTool(serverInfo.serverName)} onRemoveTool={() => removeTool(serverInfo.serverName)}/>);
        })}
              </div>
            </div>

            <div className="mt-2 flex flex-col items-center gap-2 sm:flex-row sm:justify-between">
              {maxPage > 0 ? (<PluginPagination currentPage={currentPage} maxPage={maxPage} onChangePage={handleChangePage}/>) : (<div style={{ height: '21px' }}></div>)}
            </div>
          </div>
        </DialogPanel>
      </div>
