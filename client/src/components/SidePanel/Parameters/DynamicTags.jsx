...
function DynamicTags({ label = '', settingKey, defaultValue = [], description = '', columnSpan, setOption, placeholder = '', readonly = false, showDefault = false, labelCode = false, descriptionCode = false, placeholderCode = false, descriptionSide = ESide.Left, conversation, minTags, maxTags, }) {
    ...
    const onTagRemove = useCallback((indexToRemove) => {
        if (!currentTags) {
            return;
        }
    ...
    return (<div className={`flex flex-col items-center justify-start gap-6 ${columnSpan != null ? `col-span-${columnSpan}` : 'col-span-full'}`}>
      <HoverCard openDelay={300}>
        <HoverCardTrigger className="grid w-full items-center gap-2">
          ...
          <div>
            <div className="mb-2 flex flex-wrap break-all rounded-lg border border-border-light bg-surface-secondary">
              {currentTags && currentTags.length > 0 && (<div className="flex w-full gap-1 p-1">
                  {currentTags.map((tag, index) => (<Tag key={`${tag}-${index}`} label={tag} onClick={onTagClick} onRemove={() => {
                    onTagRemove(index);
                    if (inputRef.current) {
                        inputRef.current.focus();
                    }
              ...
              <Input ref={inputRef} id={`${settingKey}-dynamic-input`} disabled={readonly} value={tagText} onKeyDown={(e) => {
            if (!currentTags) {
                return;
            }
        ...
        }} onChange={(e) => setTagText(e.target.value)} placeholder={placeholderCode
            ? (localize(placeholder) ?? placeholder)
            : placeholder} className={cn('flex h-9 max-h-9 border-none bg-surface-secondary px-3 py-2')}/>
            </div>
          </div>
        </HoverCardTrigger>
