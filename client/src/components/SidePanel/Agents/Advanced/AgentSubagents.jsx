...
import { Switch, HoverCard, CircleHelpIcon, HoverCardPortal, ControlCombobox, HoverCardContent, HoverCardTrigger, } from '@librechat/client';
import MessageIcon from '~/components/Share/MessageIcon';
import { useAgentsMapContext } from '~/Providers';
import { useLocalize } from '~/hooks';
import { ESide } from '~/common';
const AgentSubagents = ({ field, currentAgentId }) => {
    ...
    const setAllowSelf = useCallback((next) => {
        field.onChange({
            ...value,
            enabled: true,
            allowSelf: next,
    ...
    const setAgentIds = useCallback((ids) => {
        field.onChange({
            ...value,
            enabled: true,
            allowSelf: value.allowSelf ?? true,
    ...
    return (<HoverCard openDelay={50}>
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <Switch id={enableId} checked={enabled} onCheckedChange={setEnabled} aria-label={localize('com_ui_agent_subagents_enable')}/>
        </div>
      </div>

      {enabled && (<div className="mt-2 space-y-3">
          <div className="flex items-center justify-between gap-2 rounded-md border border-border-light bg-surface-primary p-3">
            ...
            <Switch id={selfId} checked={allowSelf} onCheckedChange={setAllowSelf} aria-label={localize('com_ui_agent_subagents_allow_self')}/>
          </div>
          ...
          <div className="space-y-1">
            ...
            {agentIds.map((agentId, idx) => {
                ...
                return (<div key={agentId} className="flex h-9 items-center gap-2 rounded-md border border-border-medium bg-surface-tertiary pl-2 pr-2">
                  ...
                  <button type="button" className="rounded-xl p-1 transition hover:bg-surface-hover" onClick={() => removeAgentAt(idx)} aria-label={localize('com_ui_agent_subagents_remove', {
