...
export default function VersionPanel() {
    ...
    const agentWithVersions = agent;
    const currentAgent = useMemo(() => {
        if (!agentWithVersions)
            return null;
        return {
    ...
    const versionIds = useMemo(() => {
        ...
        const activeVersionId = matchingVersions.length > 0 ? versions.findIndex((v) => v === matchingVersions[0]) : -1;
        return versions.map((version, displayIndex) => {
            const originalIndex = agentWithVersions?.versions?.findIndex((v) => v.updatedAt === version.updatedAt &&
                v.createdAt === version.createdAt &&
                v.name === version.name) ?? displayIndex;
            return {
                id: displayIndex,
                originalIndex,
                version,
    ...
    return (<div className="scrollbar-gutter-stable h-full min-h-[40vh] overflow-auto pb-12 text-sm">
      ...
      <div className="flex flex-col gap-4 px-2">
        <VersionContent selectedAgentId={selectedAgentId} isLoading={isLoading} error={error} versionContext={versionContext} onRestore={handleRestore}/>
      </div>
    </div>);
}
