export const isActiveVersion = (version, currentAgent, versions) => {
    if (!versions || versions.length === 0) {
        return false;
    }
    if (!currentAgent) {
        const versionIndex = versions.findIndex((v) => v.name === version.name &&
    ...
    const toolsMatch = () => {
        if (!version.tools && !currentAgent.tools)
            return true;
        if (!version.tools || !currentAgent.tools)
            return false;
        if (version.tools.length !== currentAgent.tools.length)
            return false;
    ...
    const capabilitiesMatch = () => {
        if (!version.capabilities && !currentAgent.capabilities)
            return true;
        if (!version.capabilities || !currentAgent.capabilities)
            return false;
        ...
        return sortedVersionCapabilities.every((capability, i) => capability === sortedCurrentCapabilities[i]);
    };
    return (matchesName &&
        matchesDescription &&
        matchesInstructions &&
