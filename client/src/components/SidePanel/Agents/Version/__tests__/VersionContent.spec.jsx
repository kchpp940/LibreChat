...
jest.mock('../VersionItem', () => ({
    __esModule: true,
    default: jest.fn(({ version, isActive, onRestore, index }) => (<div data-testid="version-item">
      <div>{version.name}</div>
      {!isActive && (<button data-testid={`restore-button-${index}`} onClick={() => onRestore(index)}>
          {mockRestore}
        </button>)}
    </div>)),
}));
...
describe('VersionContent', () => {
    ...
    test('restore functionality works correctly', () => {
        ...
        const { getByTestId, queryByTestId } = render(<VersionContent {...defaultProps} onRestore={onRestoreMock}/>);
    ...
    test('handles edge cases in data', () => {
        const { getAllByTestId, getByText, queryByTestId, queryByText, rerender } = render(<VersionContent {...defaultProps} versionContext={{ ...mockContext, versions: [] }}/>);
        expect(getAllByTestId('version-item')).toHaveLength(mockVersionIds.length);
        rerender(<VersionContent {...defaultProps} versionContext={{ ...mockContext, versionIds: [] }}/>);
        expect(getByText('No versions available')).toBeInTheDocument();
        rerender(<VersionContent {...defaultProps} selectedAgentId="" isLoading={true} error={new Error('Test')}/>);
