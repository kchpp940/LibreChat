...
export default function ApiKeyDialog({ isOpen, onSubmit, onRevoke, onOpenChange, authTypes, isToolAuthenticated, register, handleSubmit, triggerRef, triggerRefs, }) {
    ...
    const handleRerankerChange = (key) => {
        setSelectedReranker(key);
    };
    const handleScraperChange = (key) => {
        setSelectedScraper(key);
    };
    return (<OGDialog open={isOpen} onOpenChange={onOpenChange} triggerRef={triggerRef} triggerRefs={triggerRefs}>
      <OGDialogTemplate className="w-11/12 sm:w-[500px]" title="" main={<>
        ...
        }} buttons={isToolAuthenticated && (<Button onClick={onRevoke} className="bg-red-500 text-white hover:bg-red-600" aria-label={localize('com_ui_revoke')}>
              {localize('com_ui_revoke')}
            </Button>)} showCancelButton={true}/>
    </OGDialog>);
}
