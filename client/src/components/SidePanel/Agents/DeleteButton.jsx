...
import { Label, Button, OGDialog, TrashIcon, useToastContext, OGDialogTrigger, OGDialogTemplate, } from '@librechat/client';
import { logger, getDefaultAgentFormValues } from '~/utils';
import { useDeleteAgentMutation } from '~/data-provider';
import { isEphemeralAgent } from '~/common';
import { useLocalize } from '~/hooks';
import store from '~/store';
function DeleteButton({ agent_id, setCurrentAgentId, createMutation, }) {
    ...
    const deleteAgent = useDeleteAgentMutation({
        onSuccess: (_, vars, context) => {
            ...
            const firstAgent = updatedList[0];
            if (!firstAgent) {
    ...
    return (<OGDialog>
      <OGDialogTrigger asChild>
        <Button size="sm" variant="outline" aria-label={localize('com_ui_delete_agent')} title={localize('com_ui_delete_agent')} type="button">
          <div className="flex w-full items-center justify-center gap-2 text-red-500">
            <TrashIcon />
          </div>
        </Button>
      </OGDialogTrigger>
      <OGDialogTemplate title={localize('com_ui_delete_agent')} className="max-w-[450px]" main={<>
...
const MemoizedDeleteButton = memo(DeleteButton, (prevProps, nextProps) => prevProps.agent_id === nextProps.agent_id &&
