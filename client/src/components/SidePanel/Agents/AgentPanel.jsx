...
import { Tools, SystemRoles, ResourceType, EModelEndpoint, PermissionBits, PrecheckSeverity, isAssistantsEndpoint, } from 'librechat-data-provider';
import { useCreateAgentMutation, useUpdateAgentMutation, useGetAgentByIdQuery, useGetExpandedAgentByIdQuery, useUploadAgentAvatarMutation, usePrecheckAgentMutation, } from '~/data-provider';
...
/* Helpers */
function getUpdateToastMessage(noVersionChange, avatarActionState, name, localize) {
 ...
 */
export function composeAgentUpdatePayload(data, agent_id) {
    const { name, artifacts, description, instructions, model: _model, model_parameters, provider: _provider, agent_ids, edges, subagents, end_after_tools, hide_sequential_outputs, recursion_limit, category, support_contact, tool_options, skills, skills_enabled, avatar_action: avatarActionState, } = data;
    const shouldResetAvatar = avatarActionState === 'reset' && Boolean(agent_id) && !isEphemeralAgent(agent_id);
    const model = _model ?? '';
    const provider = (typeof _provider === 'string' ? _provider : _provider.value) ?? '';
    return {
        payload: {
            name,
            artifacts,
 ...
 */
export async function persistAvatarChanges({ agentId, avatarActionState, avatarFile, uploadAvatar, }) {
...
const isNestedDirtyField = (value) => typeof value === 'object' && value !== null;
const evaluateDirtyFields = (fields) => {
    let sawDirty = false;
 ...
 */
export const isAvatarUploadOnlyDirty = (dirtyFields) => {
    if (!dirtyFields) {
        return false;
    }
...
export default function AgentPanel() {
    ...
    const { activePanel, agentsConfig, setActivePanel, endpointsConfig, setCurrentAgentId, agent_id: current_agent_id, } = useAgentPanelContext();
    ...
    const { hasPermission, isLoading: permissionsLoading } = useResourcePermissions(ResourceType.AGENT, basicAgentQuery.data?._id || '');
    ...
    const { control, handleSubmit, reset, getValues, setValue, formState: { dirtyFields }, } = methods;
    const [isAvatarUploadInFlight, setIsAvatarUploadInFlight] = useState(false);
    const [precheckItems, setPrecheckItems] = useState([]);
    ...
    const handleAvatarUpload = useCallback(async (agentId) => {
    ...
    const allowedProviders = useMemo(() => new Set(agentsConfig?.allowedProviders), [agentsConfig?.allowedProviders]);
    const providers = useMemo(() => Object.keys(endpointsConfig ?? {})
        .filter((key) => !isAssistantsEndpoint(key) &&
        (allowedProviders.size > 0 ? allowedProviders.has(key) : true) &&
        key !== EModelEndpoint.agents)
        .map((provider) => createProviderOption(provider)), [endpointsConfig, allowedProviders]);
    /* Mutations */
    const update = useUpdateAgentMutation({
        ...
        onSuccess: async (data) => {
            const avatarActionState = getValues('avatar_action');
            const noVersionChange = previousVersionRef.current !== undefined && data.version === previousVersionRef.current;
            const toastMessage = getUpdateToastMessage(noVersionChange, avatarActionState, data.name, localize);
            if (toastMessage) {
    ...
    return (<FormProvider {...methods}>
      <form onSubmit={handleSubmit(onSubmit)} className="scrollbar-gutter-stable flex flex-1 flex-col px-3 pb-3 pt-2" aria-label="Agent configuration form">
        <div className="flex-1">
          <div className="flex w-full flex-wrap gap-2">
            <div className="w-full">
              <AgentSelect createMutation={create} agentQuery={agentQuery} setCurrentAgentId={setCurrentAgentId} selectedAgentId={agentQuery.isInitialLoading ? null : (current_agent_id ?? null)}/>
            </div>
            {agent_id && (<div className="flex w-full gap-2">
                <Button type="button" variant="outline" className="w-full justify-center" onClick={() => {
                reset(getDefaultAgentFormValues());
                setCurrentAgentId(undefined);
            }} disabled={agentQuery.isInitialLoading} aria-label={localize('com_ui_create_new_agent')}>
                  <Plus className="mr-1 h-4 w-4" aria-hidden="true"/>
                  {localize('com_ui_create_new_agent')}
                </Button>
                <Button variant="submit" disabled={isEphemeralAgent(agent_id) || agentQuery.isInitialLoading} onClick={(e) => {
                e.preventDefault();
                handleSelectAgent();
          ...
          {!canEditAgent && !agentQuery.isInitialLoading && (<div className="flex h-[30vh] w-full items-center justify-center">
          ...
          {canEditAgent && !agentQuery.isInitialLoading && activePanel === Panel.model && (<ModelPanel models={models} providers={providers} setActivePanel={setActivePanel}/>)}
          {canEditAgent && !agentQuery.isInitialLoading && activePanel === Panel.builder && (<AgentConfig />)}
          {canEditAgent && !agentQuery.isInitialLoading && activePanel === Panel.advanced && (<AdvancedPanel />)}
        </div>
        {canEditAgent && !agentQuery.isInitialLoading && (<AgentFooter createMutation={create} updateMutation={update} isAvatarUploading={isAvatarUploadInFlight || uploadAvatarMutation.isLoading} isPrechecking={precheckMutation.isLoading} activePanel={activePanel} setActivePanel={setActivePanel} setCurrentAgentId={setCurrentAgentId}/>)}
      </form>
      <PrecheckDialog isOpen={showPrecheckDialog} onOpenChange={setShowPrecheckDialog} items={precheckItems} onProceed={handlePrecheckProceed}/>
    </FormProvider>);
}
