...
function Avatar({ avatar }) {
    ...
    const handleFileChange = useCallback((event) => {
        ...
        reader.onloadend = () => {
            ...
            setValue('avatar_preview', reader.result ?? '', { shouldDirty: true });
    ...
    return (<>
      <div className="flex w-full items-center justify-center gap-4">
        <AvatarMenu trigger={<button type="button" className="f h-20 w-20 outline-none ring-offset-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label={localize('com_ui_upload_agent_avatar_label')}>
              {avatarPreview ? <AgentAvatarRender url={avatarPreview}/> : <NoImage />}
            </button>} handleFileChange={handleFileChange} onReset={handleReset} canReset={canReset}/>
      </div>
    </>);
}
const MemoizedAvatar = memo(Avatar, (prevProps, nextProps) => prevProps.avatar?.filepath === nextProps.avatar?.filepath);
