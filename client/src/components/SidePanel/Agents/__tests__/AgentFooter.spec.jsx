...
const createBaseMutation = (isLoading = false) => {
    if (isLoading) {
        return {
            mutate: jest.fn(),
...
jest.mock('~/data-provider', () => ({
    useUpdateAgentMutation: () => createBaseMutation(),
}));
...
jest.mock('../DeleteButton', () => ({
    __esModule: true,
    default: ({ agent_id }) => (<button data-testid="delete-button" data-agent-id={agent_id} title="Delete Agent"/>),
}));
jest.mock('../DuplicateAgent', () => ({
    __esModule: true,
    default: ({ agent_id }) => (<button data-testid="duplicate-button" data-agent-id={agent_id} title="Duplicate Agent"/>),
}));
jest.mock('@librechat/client', () => ({
    Spinner: () => <div data-testid="spinner"/>,
}));
jest.mock('~/components/Sharing', () => ({
    GenericGrantAccessDialog: ({ resourceDbId, resourceId, resourceName, resourceType, }) => (<div data-testid={`grant-access-dialog-${resourceType}`} data-resource-db-id={resourceDbId} data-resource-id={resourceId} data-resource-name={resourceName} data-resource-type={resourceType}/>),
}));
describe('AgentFooter', () => {
    ...
    const mockUpdateMutation = createBaseMutation();
    const defaultProps = {
    ...
    describe('Main Functionality', () => {
        ...
        test('handles loading states for createMutation', () => {
            const { unmount } = render(<AgentFooter {...defaultProps} createMutation={createBaseMutation(true)}/>);
