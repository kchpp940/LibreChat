...
export default function PrecheckDialog({ isOpen, onOpenChange, items, onProceed, }) {
    ...
    return (<OGDialog open={isOpen} onOpenChange={onOpenChange}>
      <OGDialogTemplate className="w-11/12 sm:w-[540px]" title={localize('com_agents_precheck_title')} main={<div className="max-h-[60vh] overflow-y-auto">
            {hasErrors && (<div className="mb-3 flex items-center gap-2 rounded-md bg-red-50 px-3 py-2 dark:bg-red-900/20">
                <XCircle className="h-5 w-5 flex-shrink-0 text-red-500"/>
                <span className="text-sm font-medium text-red-700 dark:text-red-400">
                  {localize('com_agents_precheck_errors_count', { count: errors.length })}
                </span>
              </div>)}
            {warnings.length > 0 && (<div className="mb-3 flex items-center gap-2 rounded-md bg-yellow-50 px-3 py-2 dark:bg-yellow-900/20">
            ...
            {[...grouped.entries()].map(([category, categoryItems]) => (<div key={category} className="mb-3">
                ...
                <div className="space-y-1.5">
                  {categoryItems.map((item, idx) => (<div key={`${item.category}-${idx}`} className="flex items-start gap-2 rounded-md border border-border-light px-3 py-2">
          ...
          </div>} selection={<div className="flex gap-2">
            <Button variant="outline" onClick={() => onOpenChange(false)}>
              {localize(hasErrors ? 'com_ui_close' : 'com_ui_cancel')}
            </Button>
            {!hasErrors && (<Button className="bg-green-500 hover:bg-green-600 text-white" onClick={() => {
                    onOpenChange(false);
                    onProceed();
                }}>
