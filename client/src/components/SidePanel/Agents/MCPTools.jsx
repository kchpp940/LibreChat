...
export default function MCPTools({ agentId, mcpServerNames, setShowMCPToolDialog, }) {
        ...
        <div className="mb-1">
          ...
          {mcpServerNames?.map((mcpServerName) => {
            ...
            if (!serverInfo?.isConfigured) {
                return (<UnconfiguredMCPTool key={`${mcpServerName}-${agentId}`} serverName={mcpServerName}/>);
            }
            if (!serverInfo) {
                return null;
            }
            if (serverInfo?.tools?.length && serverInfo.tools.length > 0) {
                return (<MCPTool key={`${serverInfo.serverName}-${agentId}`} serverInfo={serverInfo}/>);
            }
            return (<UninitializedMCPTool key={`${serverInfo.serverName}-${agentId}`} serverInfo={serverInfo}/>);
        })}
        </div>
        <div className="mt-2">
          <button type="button" onClick={() => setShowMCPToolDialog(true)} className="btn btn-neutral border-token-border-light relative h-9 w-full rounded-lg font-medium" aria-haspopup="dialog">
