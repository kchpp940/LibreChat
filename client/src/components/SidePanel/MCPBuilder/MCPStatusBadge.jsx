 ...
 * - Red: Error
 */
export default function MCPStatusBadge({ serverStatus, isInitializing = false, className, }) {
    const localize = useLocalize();
    const badgeBaseClass = cn('flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-medium', className);
    // Initializing/Connecting state - blue
    if (isInitializing) {
        return (<div role="status" aria-live="polite" className={cn(badgeBaseClass, 'bg-blue-50 text-blue-600 dark:bg-blue-950 dark:text-blue-400')}>
    ...
    if (connectionState === 'disconnected') {
        if (requiresOAuth) {
            ...
            return (<div role="status" className={cn(badgeBaseClass, 'bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-400')}>
        ...
        return (<div role="status" className={cn(badgeBaseClass, 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400')}>
        <span>{localize('com_nav_mcp_status_disconnected')}</span>
      </div>);
    }
    // Error state - red
    if (connectionState === 'error') {
        return (<div role="status" className={cn(badgeBaseClass, 'bg-red-50 text-red-600 dark:bg-red-950 dark:text-red-400')}>
        <span>{localize('com_nav_mcp_status_error')}</span>
      </div>);
    }
    // Connected state - green
    if (connectionState === 'connected') {
        return (<div role="status" className={cn(badgeBaseClass, 'bg-green-50 text-green-600 dark:bg-green-950 dark:text-green-400')}>
 ...
 * - Red: Error
 */
export function getStatusDotColor(serverStatus, isInitializing) {
    if (isInitializing) {
        return 'bg-blue-500';
    }
    if (!serverStatus) {
