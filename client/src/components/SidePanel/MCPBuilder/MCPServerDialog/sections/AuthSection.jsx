...
export default function AuthSection({ isEditMode, serverName }) {
    ...
    const apiKeySource = useWatch({
        name: 'auth.api_key_source',
    });
    const authorizationType = useWatch({
    ...
    return (<div className="space-y-3">
      ...
      <fieldset className="space-y-1.5">
        ...
        <Radio options={authTypeOptions} value={authType || AuthTypeEnum.None} onChange={(val) => setValue('auth.auth_type', val)} fullWidth disabled={isOboLockedReadOnly} aria-labelledby="auth-type-label"/>
      </fieldset>

      {/* API Key Fields */}
      {authType === AuthTypeEnum.ServiceHttp && (<div className="space-y-3 rounded-lg border border-border-light p-3">
          {/* User provides own key checkbox */}
          <div className="flex items-center gap-2">
            <Checkbox id="user_provides_key" checked={apiKeySource === 'user'} onCheckedChange={(checked) => setValue('auth.api_key_source', checked ? 'user' : 'admin')} aria-labelledby="user_provides_key_label"/>
            <label id="user_provides_key_label" htmlFor="user_provides_key" className="cursor-pointer text-sm">
              {localize('com_ui_user_provides_key')}
            </label>
          </div>

          {/* API Key input - only when admin provides */}
          {apiKeySource !== 'user' && (<div className="space-y-1.5">
              <Label htmlFor="api_key" className="text-sm font-medium">
                {localize('com_ui_api_key')}
              </Label>
              <SecretInput id="api_key" placeholder="sk-..." controlsOnHover {...register('auth.api_key')}/>
            </div>)}
          ...
          <fieldset className="space-y-1.5">
            ...
            <Radio options={headerFormatOptions} value={authorizationType || AuthorizationTypeEnum.Bearer} onChange={(val) => setValue('auth.api_key_authorization_type', val)} fullWidth aria-labelledby="header-format-label"/>
          </fieldset>

          {/* Custom header name */}
          {authorizationType === AuthorizationTypeEnum.Custom && (<div className="space-y-1.5">
              <Label htmlFor="custom_header" className="text-sm font-medium">
                {localize('com_ui_custom_header_name')}
              </Label>
              <Input id="custom_header" placeholder="X-Api-Key" {...register('auth.api_key_custom_header')}/>
            </div>)}
        </div>)}

      {/* OAuth Fields */}
      {authType === AuthTypeEnum.OAuth && (<div className="space-y-3 rounded-lg border border-border-light p-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <SecretInput id="oauth_client_id" autoComplete="new-password" data-lpignore="true" data-1p-ignore="true" controlsOnHover placeholder={isEditMode ? localize('com_ui_leave_blank_to_keep') : ''} aria-invalid={errors.auth?.oauth_client_id ? 'true' : 'false'} aria-describedby={errors.auth?.oauth_client_id ? 'oauth-client-id-error' : undefined} {...register('auth.oauth_client_id', { required: !isEditMode })} className={cn(errors.auth?.oauth_client_id && 'border-border-destructive')}/>
              {errors.auth?.oauth_client_id && (<p id="oauth-client-id-error" role="alert" className="text-xs text-text-destructive">
            ...
            <div className="space-y-1.5">
              ...
              <SecretInput id="oauth_client_secret" placeholder={isEditMode ? localize('com_ui_leave_blank_to_keep') : ''} controlsOnHover {...register('auth.oauth_client_secret')}/>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input id="oauth_authorization_url" placeholder="https://..." {...register('auth.oauth_authorization_url')}/>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="oauth_token_url" className="text-sm font-medium">
                {localize('com_ui_token_url')}
              </Label>
              <Input id="oauth_token_url" placeholder="https://..." {...register('auth.oauth_token_url')}/>
            </div>
          </div>
          ...
          {/* Redirect URI */}
          {isEditMode && redirectUri && (<div className="space-y-1.5">
              <Label htmlFor="auth-redirect-uri" className="text-sm font-medium">
                {localize('com_ui_redirect_uri')}
              </Label>
              <div className="flex items-center gap-2">
                <Input id="auth-redirect-uri" type="text" readOnly value={redirectUri} className="flex-1 text-xs text-text-secondary"/>
                <button type="button" onClick={() => {
                    if (isCopying)
                        return;
                    showToast({ message: localize('com_ui_copied_to_clipboard') });
                    copyLink(setIsCopying);
                }} className="flex size-10 shrink-0 items-center justify-center rounded-lg border border-border-light text-text-secondary transition-colors hover:bg-surface-hover hover:text-text-primary" aria-label={localize('com_ui_copy_link')}>
      ...
      {/* OBO Fields */}
      {authType === AuthTypeEnum.OBO && (<div className="space-y-3 rounded-lg border border-border-light p-3">
          <div className="space-y-1.5">
            ...
            <Input id="obo_scopes" placeholder="api://<client-id>/Mcp.Tools.ReadWrite" disabled={!canConfigureObo} aria-invalid={errors.auth?.obo_scopes ? 'true' : 'false'} aria-describedby={canConfigureObo ? 'obo-scopes-description' : 'obo-scopes-readonly-description'} {...register('auth.obo_scopes', {
            required: canConfigureObo && authType === AuthTypeEnum.OBO,
        })} className={cn(errors.auth?.obo_scopes && 'border-border-destructive')}/>
            {errors.auth?.obo_scopes && (<p role="alert" className="text-xs text-text-destructive">
                {localize('com_ui_field_required')}
              </p>)}
            {canConfigureObo ? (<p id="obo-scopes-description" className="text-xs text-text-secondary">
                {localize('com_ui_obo_scopes_description')}
              </p>) : (<p id="obo-scopes-readonly-description" className="text-xs text-text-secondary">
