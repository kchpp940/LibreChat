...
jest.mock('../PanelFileCell', () => ({ row }) => (<span>{row.original?.filename}</span>));
...
function makeExtendedFile(overrides = {}) {
    return {
        file_id: 'ext-1',
        size: 1024,
        progress: 1,
...
function clickFilenameCell() {
    ...
    const filenameCell = cells.find((cell) => cell.tagName === 'TD' && cell.textContent && !cell.textContent.includes('com_ui_'));
    if (!filenameCell) {
...
describe('PanelTable handleFileClick', () => {
    ...
    it('calls addFile when within file limits', () => {
        ...
        expect(mockAddFile).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('blocks attachment when fileLimit is reached', () => {
        ...
        mockFileMap = { [file.file_id]: file };
        mockFiles = new Map(Array.from({ length: 5 }, (_, i) => [
            `existing-${i}`,
    ...
    it('allows attachment when just under fileLimit', () => {
        ...
        mockFileMap = { [file.file_id]: file };
        mockFiles = new Map(Array.from({ length: 4 }, (_, i) => [
            `existing-${i}`,
            makeExtendedFile({ file_id: `existing-${i}` }),
        ]));
        renderTable([file]);
