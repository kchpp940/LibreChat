...
import { validateAndParseOpenAPISpec, openapiToFunction, AuthTypeEnum, } from 'librechat-data-provider';
...
const debouncedValidation = debounce((input, callback) => {
    const result = validateAndParseOpenAPISpec(input);
    callback(result);
}, 800);
export default function ActionsInput({ action, assistant_id, endpoint, version, setAction, }) {
    ...
    const updateAction = useUpdateAction({
        ...
        onError(error) {
            showToast({
                message: error?.message ?? localize('com_assistants_update_actions_error'),
                status: 'error',
            });
        },
    });
    ...
    return (<>
      <div className="">
        <div className="mb-1 flex flex-wrap items-center justify-between gap-4">
          <label htmlFor="example-schema" className="text-token-text-primary whitespace-nowrap font-medium">
        ...
        */}
        ...
        <div className="border-token-border-medium bg-token-surface-primary hover:border-token-border-hover mb-4 w-full overflow-hidden rounded-lg border ring-0">
          <div className="relative">
            <textarea id="schemaInput" value={inputValue} onChange={handleInputChange} spellCheck="false" placeholder={localize('com_ui_enter_openapi_schema')} className="text-token-text-primary block h-96 w-full bg-transparent p-2 font-mono text-xs outline-none focus:ring-1 focus:ring-border-light"/>
            {/* TODO: format input button */}
          </div>
          {validationResult && validationResult.message !== 'OpenAPI spec is valid.' && (<div className="border-token-border-light border-t p-2 text-red-500">
      ...
      <div className="relative my-1">
        ...
        <div className="border-token-border-medium bg-token-surface-primary hover:border-token-border-hover flex h-9 w-full rounded-lg border">
          <input type="text" placeholder="https://api.example-weather-app.com/privacy" className="flex-1 rounded-lg bg-transparent px-3 py-1.5 text-sm outline-none placeholder:text-text-secondary-alt focus:ring-1 focus:ring-border-light"/>
        </div>
      </div>
      <div className="flex items-center justify-end">
        <button disabled={!functions || !functions.length} onClick={saveAction} className="focus:shadow-outline mt-1 flex min-w-[100px] items-center justify-center rounded bg-green-500 px-4 py-2 font-semibold text-white hover:bg-green-400 focus:border-green-500 focus:outline-none focus:ring-0 disabled:bg-green-400" type="button">
          {submitContext()}
        </button>
      </div>
    </>);
