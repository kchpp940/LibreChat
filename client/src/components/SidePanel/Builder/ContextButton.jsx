import { Dialog, DialogTrigger, Label, DialogTemplate, useToastContext, TrashIcon, } from '@librechat/client';
import { useDeleteAssistantMutation } from '~/data-provider';
import { useLocalize, useSetIndexOptions } from '~/hooks';
import { cn, removeFocusOutlines } from '~/utils';
import { useChatContext } from '~/Providers';
export default function ContextButton({ activeModel, assistant_id, setCurrentAssistantId, createMutation, endpoint, }) {
    ...
    const deleteAssistant = useDeleteAssistantMutation({
        onSuccess: (_, vars, context) => {
            ...
            const firstAssistant = updatedList[0];
            if (!firstAssistant) {
            ...
            const currentAssistant = updatedList.find((assistant) => assistant.id === conversation?.assistant_id);
            if (currentAssistant) {
    ...
    return (<Dialog>
      <DialogTrigger asChild>
        <button className={cn('btn btn-neutral border-token-border-light relative h-9 rounded-lg font-medium', removeFocusOutlines)} type="button">
          <div className="flex w-full items-center justify-center gap-2 text-red-500">
            <TrashIcon />
          </div>
        </button>
      </DialogTrigger>
      <DialogTemplate title={localize('com_ui_delete_assistant')} className="max-w-[450px]" main={<>
            ...
            selectHandler: () => deleteAssistant.mutate({ assistant_id, model: activeModel, endpoint }),
