...
import { fileConfig as defaultFileConfig, QueryKeys, defaultOrderQuery, mergeFileConfig, } from 'librechat-data-provider';
...
function Avatar({ endpoint, version, assistant_id, metadata, createMutation, }) {
    ...
    const lastSeenCreatedId = useRef(null);
    ...
    const { mutate: uploadAvatar } = useUploadAssistantAvatarMutation({
        ...
        onSuccess: (data, vars) => {
            ...
            setInput(null);
            setPreviewUrl(data.metadata?.avatar);
            ...
            queryClient.setQueryData([QueryKeys.assistants, endpoint, defaultOrderQuery], {
                ...res,
                data: assistants,
            });
            setProgress(1);
    ...
    useEffect(() => {
        setPreviewUrl(metadata?.avatar ?? null);
    }, [metadata]);
    useEffect(() => {
         ...
         */
        const sharedUploadCondition = !!(createMutation.isSuccess &&
            input &&
            previewUrl &&
    ...
    return (<Popover.Root open={menuOpen} onOpenChange={setMenuOpen}>
      <div className="flex w-full items-center justify-center gap-4">
        <Popover.Trigger asChild>
          <button type="button" className="h-20 w-20" aria-label={localize('com_ui_upload_avatar_label')}>
