...
import { Tools, FileSources, Capabilities, EModelEndpoint, LocalStorageKeys, isImageVisionTool, defaultAssistantFormValues, } from 'librechat-data-provider';
...
export default function AssistantSelect({ reset, value, endpoint, documentsMap, selectedAssistant, setCurrentAssistantId, createMutation, allTools, }) {
    const localize = useLocalize();
    const fileMap = useFileMapContext();
    const lastSelectedAssistant = useRef(null);
    const [lastSelectedModels] = useLocalStorage(LocalStorageKeys.LAST_MODEL, {});
    const toolkits = useMemo(() => new Set(allTools?.filter((tool) => tool.toolkit === true).map((tool) => tool.pluginKey)), [allTools]);
    const query = useListAssistantsQuery(endpoint, undefined, {
        select: (res) => res.data.map((_assistant) => {
            const source = endpoint === EModelEndpoint.assistants ? FileSources.openai : FileSources.azure;
            const assistant = {
                ..._assistant,
                label: _assistant.name ?? '',
                value: _assistant.id,
                files: _assistant.file_ids ? [] : undefined,
                code_files: _assistant.tool_resources?.code_interpreter?.file_ids
                    ? []
                    : undefined,
            };
            const handleFile = (file_id, list) => {
            ...
            if (assistant.code_files && _assistant.tool_resources?.code_interpreter?.file_ids) {
                _assistant.tool_resources.code_interpreter.file_ids.forEach((file_id) => handleFile(file_id, assistant.code_files));
            }
