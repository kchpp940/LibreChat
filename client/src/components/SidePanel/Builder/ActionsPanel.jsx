...
import { OGDialog, OGDialogTrigger, Label, OGDialogTemplate, useToastContext, TrashIcon, } from '@librechat/client';
...
// activePanel,
action, endpoint, version, setAction, assistant_id, setActivePanel, }) {
    ...
    const deleteAction = useDeleteAction({
        ...
        onError(error) {
            showToast({
                message: error?.message ?? localize('com_assistants_delete_actions_error'),
                status: 'error',
            });
        },
    });
    ...
    useEffect(() => {
        if (action?.metadata?.auth) {
            reset({
                ...
                token_exchange_method: action.metadata.auth.token_exchange_method ?? TokenExchangeMethodEnum.DefaultPost,
            });
        }
    }, [action, reset]);
    return (<FormProvider {...methods}>
      <form className="h-full grow overflow-hidden">
            ...
            {!!action && (<OGDialog>
                <OGDialogTrigger asChild>
                  <div className="absolute right-0 top-6">
                    <button type="button" disabled={!(assistant_id ?? '') || !action.action_id} className="btn btn-neutral border-token-border-light relative h-9 rounded-lg font-medium">
                      <TrashIcon className="text-red-500"/>
                    </button>
                  </div>
                </OGDialogTrigger>
                <OGDialogTemplate showCloseButton={false} title={localize('com_ui_delete_action')} className="max-w-[450px]" main={<Label className="text-left text-sm font-medium">
        ...
        <a href="https://help.openai.com/en/articles/8554397-creating-a-gpt" target="_blank" rel="noreferrer" className="font-medium">Learn more.</a>
          ...
          <ActionsInput action={action} assistant_id={assistant_id} setAction={setAction} endpoint={endpoint} version={version}/>
        </div>
      </form>
    </FormProvider>);
}
