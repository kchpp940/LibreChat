...
import { GPTIcon, PaLMIcon, CodeyIcon, GeminiIcon, BedrockIcon, AssistantIcon, AnthropicIcon, AzureMinimalIcon, CustomMinimalIcon, } from '@librechat/client';
import UnknownIcon from '~/hooks/Endpoint/UnknownIcon';
import { cn } from '~/utils';
function getOpenAIColor(_model) {
...
function getGoogleIcon(model, size) {
...
function getGoogleModelName(model) {
    if (model?.toLowerCase().includes('code') === true) {
        return 'Codey';
    }
...
const MessageEndpointIcon = (props) => {
    const { error, iconURL = '', endpoint, size = 30, model = '', assistantName, agentName } = props;
    const assistantsIcon = {
        icon: iconURL ? (<div className="relative flex h-6 w-6 items-center justify-center">
        <div title={assistantName} style={{
                width: size,
                height: size,
            }} className={cn('overflow-hidden rounded-full', props.className ?? '')}>
          <img className="shadow-stroke h-full w-full object-cover" src={iconURL} alt={assistantName} style={{ height: '80', width: '80' }}/>
        </div>
      </div>) : (<div className="h-6 w-6">
    ...
    const agentsIcon = {
        icon: iconURL ? (<div className="relative flex h-6 w-6 items-center justify-center">
        <div title={agentName} style={{
                width: size,
                height: size,
            }} className={cn('overflow-hidden rounded-full', props.className ?? '')}>
          <img className="shadow-stroke h-full w-full object-cover" src={iconURL} alt={agentName} style={{ height: '80', width: '80' }}/>
        </div>
      </div>) : (<div className="h-6 w-6">
        <div className="shadow-stroke flex h-6 w-6 items-center justify-center overflow-hidden rounded-full">
    ...
    const endpointIcons = {
        ...
        default: {
          ...
          <div className="overflow-hidden rounded-full">
            <UnknownIcon iconURL={iconURL} endpoint={endpoint ?? ''} className="h-full w-full object-contain" context="message"/>
          </div>
        </div>),
            name: endpoint,
        },
    };
    let { icon, bg, name } = endpoint != null && endpoint && endpointIcons[endpoint]
        ? (endpointIcons[endpoint] ?? {})
        : endpointIcons.default;
    ...
    return (<div title={name ?? ''} style={{
            ...
            width: size,
            height: size,
        }} className={cn('relative flex h-9 w-9 items-center justify-center rounded-sm p-1 text-white', props.className ?? '')}>
      {icon}
      {error === true && (<span className="absolute right-0 top-[20px] -mr-2 flex h-3 w-3 items-center justify-center rounded-full border border-white bg-red-500 text-[10px] text-white">
          !
        </span>)}
    </div>);
};
