...
function resolveAvatar(userId, userAvatar, avatarSrc) {
    if (!userId) {
    ...
    const imgSrc = userAvatar || avatarSrc;
    const resolved = imgSrc && !failedUrls.has(imgSrc) ? { type: 'image', src: imgSrc } : { type: 'fallback' };
    avatarCache.set(userId, { avatar: userAvatar, avatarSrc, resolved });
    return resolved;
}
function markAvatarFailed(userId, src) {
    failedUrls.add(src);
...
const UserAvatar = memo(({ size, avatar, avatarSrc, userId, username, className }) => {
    ...
    return (<div title={username} style={{ width: size, height: size }} className={cn('relative flex items-center justify-center', className ?? '')}>
        {resolved.type === 'image' ? (<img className="rounded-full" src={resolved.src} alt="avatar" onError={() => setResolved(markAvatarFailed(userId, resolved.src))}/>) : (<div style={{
...
const Icon = memo((props) => {
    ...
    if (isCreatedByUser) {
        ...
        return (<UserAvatar size={size} avatarSrc={avatarSrc} username={username} userId={user?.id ?? ''} avatar={user?.avatar ?? ''} className={props.className}/>);
    }
