...
import { cn, defaultTextProps, removeFocusRings, mapAssistants, createDropdownSetter, } from '~/utils';
...
export default function Settings({ conversation, setOption, models, readonly }) {
    ...
    const defaultOption = useMemo(() => ({ label: localize('com_endpoint_use_active_assistant'), value: '' }), [localize]);
    const assistantListMap = useAssistantListMap((res) => mapAssistants(res.data));
    const { model, endpoint, assistant_id, endpointType, promptPrefix, instructions } = conversation ?? {};
    const currentList = useMemo(() => Object.values(assistantListMap[endpoint ?? ''] ?? {}), [assistantListMap, endpoint]);
    ...
    const modelOptions = useMemo(() => {
        return models.map((model) => ({
            label: model === activeAssistant?.model
                ? `${model} (${localize('com_endpoint_assistant_model')})`
                : model,
            value: model,
        }));
    }, [models, activeAssistant, localize]);
    const [assistantValue, setAssistantValue] = useState(activeAssistant != null
    ...
    return (<div className="grid grid-cols-6 gap-6">
      <div className="col-span-6 flex flex-col items-center justify-start gap-6 sm:col-span-3">
        <div className="grid w-full items-center gap-2">
          <SelectDropDown value={model ?? ''} title={localize('com_ui_model')} setValue={createDropdownSetter(setModel)} availableValues={modelOptions} disabled={readonly} className={cn(defaultTextProps, 'flex w-full resize-none', removeFocusRings)} containerClassName="flex w-full resize-none"/>
        </div>
      </div>
      <div className="col-span-6 flex flex-col items-center justify-start gap-6 px-3 sm:col-span-3">
        <HoverCard openDelay={300}>
          <HoverCardTrigger className="grid w-full items-center gap-2">
            <div className="grid w-full items-center gap-2">
              <SelectDropDown title={localize('com_endpoint_assistant')} value={assistantValue} setValue={createDropdownSetter(setAssistant)} availableValues={assistants} disabled={readonly} className={cn(defaultTextProps, 'flex w-full resize-none', removeFocusRings)} containerClassName="flex w-full resize-none"/>
            </div>
          </HoverCardTrigger>
      ...
      <div className="col-span-6 flex flex-col items-center justify-start gap-6">
        <div className="grid w-full items-center gap-2">
          ...
          <TextareaAutosize id="promptPrefix" disabled={readonly} value={promptPrefixValue ?? ''} onChange={onPromptPrefixChange} placeholder={localize('com_endpoint_prompt_prefix_assistants_placeholder')} className={cn(defaultTextProps, 'flex max-h-[240px] min-h-[80px] w-full resize-none px-3 py-2')}/>
        </div>
        <div className="grid w-full items-center gap-2">
          ...
          <TextareaAutosize id="instructions" disabled={readonly} value={instructionsValue ?? ''} onChange={onInstructionsChange} placeholder={localize('com_endpoint_instructions_assistants_placeholder')} className={cn(defaultTextProps, 'flex max-h-[240px] min-h-[80px] w-full resize-none px-3 py-2')}/>
        </div>
      </div>
    </div>);
}
