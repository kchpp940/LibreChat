...
export default function GoogleView({ conversation, models, isPreset = false }) {
    ...
    const { setOption, setExample, addExample, removeExample } = useSetIndexOptions(isPreset ? conversation : null);
    if (!conversation) {
        return null;
    }
    ...
    return showExamples && isChatModel && !isTextModel ? (<Examples examples={examples ?? [{ input: { content: '' }, output: { content: '' } }]} setExample={setExample} addExample={addExample} removeExample={removeExample}/>) : (<Settings conversation={conversation} setOption={setOption} models={models}/>);
}
