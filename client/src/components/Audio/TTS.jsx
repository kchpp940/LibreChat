...
export function BrowserTTS({ isLast, index, messageId, content, className, renderButton, }) {
    ...
    logger.log('MessageAudio: audioRef.current?.src, audioRef.current', audioRef.current?.src, audioRef.current);
    const handleClick = () => {
        if (audioRef.current) {
    ...
    return (<>
      ...
      <audio ref={audioRef} controls preload="none" controlsList="nodownload nofullscreen noremoteplayback" style={{
            position: 'absolute',
            overflow: 'hidden',
            display: 'none',
            height: '0px',
            width: '0px',
        }} src={audioRef.current?.src} onError={(error) => {
            logger.error('Error fetching audio:', error);
        }} id={`audio-${messageId}`} autoPlay/>
    </>);
}
export function ExternalTTS({ isLast, index, messageId, content, className, renderButton, }) {
    const localize = useLocalize();
    const playbackRate = useRecoilValue(store.playbackRate);
    ...
    return (<>
      {renderButton ? (renderButton({
            ...
            className,
                ...
                toggleSpeech();
            }} type="button" title={isSpeaking === true ? localize('com_ui_stop') : localize('com_ui_read_aloud')}>
          {renderIcon()}
        </button>)}
      <audio ref={audioRef} controls preload="none" controlsList="nodownload nofullscreen noremoteplayback" style={{
            position: 'absolute',
