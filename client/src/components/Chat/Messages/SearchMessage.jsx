...
const HitTypeBadge = ({ type, localize }) => {
    const badgeStyles = {
    ...
    return (<span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium', badgeStyles[type])}>
      {localize(labelMap[type])}
    </span>);
};
const SearchHitsSummary = ({ hits, localize }) => {
    if (!hits || hits.length === 0) {
        return null;
    }
    const uniqueTypes = [...new Set(hits.map((hit) => hit.type))];
    return (<div className="mt-2 space-y-2 rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/50">
      <div className="flex flex-wrap gap-1.5">
        {uniqueTypes.map((type) => (<HitTypeBadge key={type} type={type} localize={localize}/>))}
      </div>
      <div className="space-y-1.5">
        {hits.slice(0, 3).map((hit, index) => (<div key={index} className="text-sm">
        ...
        {hits.length > 3 && (<p className="text-xs text-gray-500 dark:text-gray-400">
            {localize('com_nav_search_more_hits', { count: hits.length - 3 })}
          </p>)}
      </div>
    </div>);
};
const MessageAvatar = ({ iconData }) => (<div className="relative flex flex-shrink-0 flex-col items-end">
...
const MessageBody = ({ message, messageLabel, fontSize, searchHits, localize }) => (<div className={cn('relative flex w-11/12 flex-col', message.isCreatedByUser ? '' : 'agent-turn')}>
...
export default function SearchMessage({ message, searchHits, }) {
    ...
    return (<div className="text-token-text-primary w-full bg-transparent">
      <div className="m-auto p-4 py-2 md:gap-6">
        <div className="final-completion group mx-auto flex flex-1 gap-3 md:max-w-3xl md:px-5 lg:max-w-[40rem] lg:px-1 xl:max-w-[48rem] xl:px-5">
          ...
          <MessageBody message={message} messageLabel={messageLabel} fontSize={fontSize} searchHits={searchHits} localize={localize}/>
        </div>
      </div>
    </div>);
}
