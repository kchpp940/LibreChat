...
function getToolMeta(part) {
    ...
    const isStandard = 'args' in toolCall && (!toolCall.type || toolCall.type === ToolCallTypes.TOOL_CALL);
    if (isStandard) {
        const tc = toolCall;
...
export default function ToolCallGroup({ parts, isSubmitting, isLast, renderPart, lastContentIdx, groupAttachments, initialExpansionState, onExpansionChange, }) {
    const localize = useLocalize();
    const mcpIconMap = useMCPIconMap();
    const rootRef = useRef(null);
    const cancelLayoutReconcileRef = useRef(null);
    const count = parts.length;
    const toolMetadata = useMemo(() => parts.map((p) => getToolMeta(p.part)), [parts]);
    const allCompleted = useMemo(() => toolMetadata.every((m) => m?.hasOutput === true), [toolMetadata]);
    ...
    const subagentCount = useMemo(() => toolNames.filter((n) => n === Constants.SUBAGENT).length, [toolNames]);
    ...
    const [isExpanded, setIsExpanded] = useState(initialState?.isExpanded ?? (autoExpand || !autoCollapse));
    ...
    const getSubagentLabel = () => subagentsDone
    ...
    const hasActiveToolCall = useMemo(() => isSubmitting && toolMetadata.some((m) => m && !m.hasOutput), [toolMetadata, isSubmitting]);
    useEffect(() => {
    ...
    return (<div className="mb-2 mt-1" ref={rootRef}>
      <button type="button" className="inline-flex w-full items-center gap-2 py-1 text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-heavy" onClick={handleToggle} aria-expanded={isExpanded} aria-label={groupLabel}>
        {allSubagents ? (
        ...
        <div className={cn('flex h-5 w-5 shrink-0 items-center justify-center text-text-secondary', !allCompleted && isSubmitting && 'animate-pulse text-primary')} aria-hidden="true">
            <Users size={14}/>
          </div>) : (<StackedToolIcons toolNames={iconToolNames} mcpIconMap={mcpIconMap} maxIcons={4} isAnimating={!allCompleted && isSubmitting}/>)}
        ...
        {toolNameSummary && !allSubagents && (<span className="text-xs font-normal text-text-secondary">— {toolNameSummary}</span>)}
        <ChevronDown className={cn('size-4 shrink-0 text-text-secondary transition-transform duration-200 ease-out', isExpanded && 'rotate-180')} aria-hidden="true"/>
      </button>
      <div style={expandStyle} onTransitionEnd={handleTransitionEnd} aria-hidden={!isExpanded}>
        {shouldRenderBody && (<div className="overflow-hidden" ref={expandRef}>
            <div className="py-0.5 pl-4">
              {parts.map(({ part, idx }) => renderPart(part, idx, isLast && idx === lastContentIdx, handleToolExpand))}
            </div>
          </div>)}
      </div>
      {groupAttachments && groupAttachments.length > 0 && (<AttachmentGroup attachments={groupAttachments}/>)}
    </div>);
}
