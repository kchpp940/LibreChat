 ...
 */
export function groupParallelContent(content) {
    if (!content) {
    ...
    content.forEach((part, idx) => {
        ...
        const partAgentId = part.agentId;
        if (groupId != null) {
 ...
 */
export const ParallelColumns = memo(function ParallelColumns({ columns, groupId, messageId, conversationId, isSubmitting, lastContentIdx, renderPart, }) {
    return (<div className={cn('flex w-full flex-col gap-3 md:flex-row', 'sibling-content-group')}>
      {columns.map(({ agentId, parts: columnParts }, colIdx) => {
            ...
            return (<div key={`column-${messageId}-${groupId}-${agentId || colIdx}`} className="min-w-0 flex-1 rounded-lg border border-border-light p-3">
            <SiblingHeader agentId={agentId} messageId={messageId} isSubmitting={isSubmitting} conversationId={conversationId}/>
            {showLoadingCursor ? (<Container>
                <EmptyText />
              </Container>) : (columnParts.map(({ part, idx }) => {
 ...
 */
export const ParallelContentRenderer = memo(function ParallelContentRenderer({ content, messageId, conversationId, attachments, searchResults, isSubmitting, renderPart, }) {
    const { parallelSections, sequentialParts } = useMemo(() => groupParallelContent(content), [content]);
    ...
    const { before, after } = useMemo(() => {
        ...
        const allParallelIndices = parallelSections.flatMap((s) => s.columns.flatMap((c) => c.parts.map((p) => p.idx)));
    ...
    return (<SearchContext.Provider value={{ searchResults }}>
      ...
      {parallelSections.map(({ groupId, columns }) => (<ParallelColumns key={`parallel-group-${messageId}-${groupId}`} columns={columns} groupId={groupId} messageId={messageId} renderPart={renderPart} isSubmitting={isSubmitting} conversationId={conversationId} lastContentIdx={lastContentIdx}/>))}
