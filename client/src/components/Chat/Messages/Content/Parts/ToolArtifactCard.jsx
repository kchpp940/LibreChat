 ...
 *     of context.
 */
const ToolArtifactCard = memo(({ attachment, artifact }) => {
    ...
    const consumeJustResolved = useRecoilCallback(({ snapshot, reset }) => (id) => {
     ...
     */
    const readInitialIsSubmitting = useRecoilCallback(({ snapshot }) => () => 
    ...
    const mountedDuringStreamRef = useRef(null);
    ...
    return (<div className="group relative my-2 inline-flex max-w-fit items-stretch gap-px overflow-hidden rounded-xl text-sm text-text-primary shadow-sm">
      <button type="button" onClick={handleOpen} aria-pressed={isSelected} className={cn('relative overflow-hidden rounded-l-xl transition-all duration-200 hover:bg-surface-hover active:scale-[0.99]', {
