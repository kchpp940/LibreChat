...
jest.mock('../Text', () => ({
    __esModule: true,
    default: ({ text }) => <div data-testid="text-part">{text}</div>,
}));
jest.mock('../Reasoning', () => ({
    __esModule: true,
    default: ({ reasoning }) => (<div data-testid="reasoning-part">{reasoning}</div>),
}));
jest.mock('~/components/Chat/Messages/Content/ToolCall', () => ({
    __esModule: true,
    default: ({ name, output }) => (<div data-testid="tool-call-part" data-name={name}>
      {output}
    </div>),
}));
jest.mock('~/components/Chat/Messages/Content/MarkdownLite', () => ({
    __esModule: true,
    default: ({ content }) => <div data-testid="prompt-markdown">{content}</div>,
}));
...
jest.mock('@librechat/client', () => ({
    __esModule: true,
    OGDialog: ({ children }) => <>{children}</>,
    OGDialogContent: ({ children }) => (<div data-testid="dialog-content">{children}</div>),
    OGDialogTitle: ({ children }) => (<div data-testid="dialog-title">{children}</div>),
    OGDialogDescription: ({ children }) => (<div data-testid="dialog-description">{children}</div>),
}));
...
jest.mock('~/components/Share/MessageIcon', () => ({
    __esModule: true,
    default: ({ agent }) => (<span data-testid="agent-icon">{agent?.name ?? ''}</span>),
}));
jest.mock('~/utils', () => ({
...
jest.mock('~/components/Chat/Messages/Content/Container', () => ({
    __esModule: true,
    default: ({ children }) => (<div data-testid="dialog-container">{children}</div>),
}));
jest.mock('~/components/Chat/Messages/Content/ToolCallGroup', () => ({
    __esModule: true,
    default: ({ parts, renderPart, lastContentIdx, }) => (<div data-testid="tool-call-group">
 ...
 *  outputs filled. */
function progressFromEvents(base) {
...
function renderWithState(args) {
    const setter = { current: null };
    const SeedHelper = () => {
        setter.current = useRecoilCallback(({ set }) => (next) => {
            set(subagentProgressByToolCallId(args.toolCallId), next);
        }, []);
        return null;
    };
    const rendered = render(<RecoilRoot>
      <SeedHelper />
      <SubagentCall toolCallId={args.toolCallId} initialProgress={args.initialProgress} isSubmitting={args.isSubmitting ?? false} args={{ subagent_type: 'self', description: 'compute' }}/>
    </RecoilRoot>);
...
describe('SubagentCall — ticker', () => {
    ...
    it('refreshes long live previews after the subagent ticker throttle window', () => {
        ...
        const progressForText = (text) => progressFromEvents({
            subagentRunId: 'run_a',
            subagentType: 'self',
            status: 'message_delta',
...
describe('SubagentCall — dialog content', () => {
    it('renders the prompt through MarkdownLite and expands it inline with the activity area', () => {
        render(<RecoilRoot>
        <SubagentCall toolCallId="call_prompt" initialProgress={1} isSubmitting={false} output="final answer" args={{
                subagent_type: 'self',
    ...
    it('falls back to the raw tool output when no content parts were recorded', () => {
        ...
        <SubagentCall toolCallId="call_fallback_2" initialProgress={1} output="raw final text" isSubmitting={false}/>
      </RecoilRoot>);
        openSubagentDialog();
    ...
    it('renders persistedContent parts when no live events are available (page-refresh flow)', () => {
        ...
        render(<RecoilRoot>
        <SubagentCall toolCallId="call_refresh" initialProgress={1} isSubmitting={false} persistedContent={persistedContent}/>
      </RecoilRoot>);
        openSubagentDialog();
    ...
    it('prefers persistedContent when both are populated (sync/reconnect canonical)', () => {
         ...
         */
        render(<RecoilRoot>
        <SubagentCall toolCallId="call_both_seeded" initialProgress={1} isSubmitting={false} persistedContent={[{ type: 'text', text: 'Persisted answer.' }]}/>
      </RecoilRoot>);
        openSubagentDialog();
