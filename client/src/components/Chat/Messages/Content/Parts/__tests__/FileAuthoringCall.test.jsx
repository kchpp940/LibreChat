...
jest.mock('~/components/Chat/Messages/Content/ProgressText', () => ({
    __esModule: true,
    default: ({ progress, inProgressText, finishedText, }) => <div data-testid="progress-text">{progress < 1 ? inProgressText : finishedText}</div>,
}));
jest.mock('../CodeWindowHeader', () => ({
    __esModule: true,
    default: ({ language }) => (<div data-testid="code-window-header" data-language={language}/>),
}));
...
describe('FileAuthoringCall', () => {
    it('shows create_file content args while the call is in progress', () => {
        render(<FileAuthoringCall toolName="create_file" initialProgress={0.5} isSubmitting={true} args={{
    ...
    it('keeps authored content visible alongside the output after create_file completes', () => {
        render(<FileAuthoringCall toolName="create_file" initialProgress={1} isSubmitting={false} args={{
                file_path: 'skills/demo/SKILL.md',
    ...
    it('prefers the output diff over the args preview after edit_file completes', () => {
        ...
        render(<FileAuthoringCall toolName="edit_file" initialProgress={1} isSubmitting={false} args={{
                file_path: 'skills/demo/SKILL.md',
                old_text: 'old line',
                new_text: 'new line',
            }} output={output}/>);
    ...
    it('shows edit_file replacement args while the call is in progress', () => {
        render(<FileAuthoringCall toolName="edit_file" initialProgress={0.5} isSubmitting={true} args={{
                file_path: 'skills/demo/SKILL.md',
    ...
    it('streams create_file content from partial JSON string args during run_step_delta', () => {
        render(<FileAuthoringCall toolName="create_file" initialProgress={0.5} isSubmitting={true} args={'{"file_path":"skills/demo/SKILL.md","content":"# Demo\\n\\nStreaming body so far'} output=""/>);
        expect(screen.getByTestId('progress-text')).toHaveTextContent('Creating SKILL.md');
        expect(screen.getByTestId('code-window-header')).toHaveAttribute('data-language', 'SKILL.md');
    ...
    it('streams edit_file replacement preview from partial JSON string args', () => {
        render(<FileAuthoringCall toolName="edit_file" initialProgress={0.5} isSubmitting={true} args={'{"file_path":"skills/demo/SKILL.md","old_text":"description: Old behavior","new_text":"description: New beh'} output=""/>);
        const preview = screen.getByText((_, element) => element?.tagName.toLowerCase() === 'code');
        expect(preview).toHaveTextContent('-description: Old behavior');
        expect(preview).toHaveTextContent('+description: New beh');
    });
    it('streams batched edit_file previews from a partial edits array', () => {
        const args = '{"file_path":"skills/demo/SKILL.md","edits":[' +
            '{"old_text":"first old","new_text":"first new"},' +
            '{"old_text":"second old","new_text":"second n';
        render(<FileAuthoringCall toolName="edit_file" initialProgress={0.5} isSubmitting={true} args={args} output=""/>);
        const preview = screen.getByText((_, element) => element?.tagName.toLowerCase() === 'code');
