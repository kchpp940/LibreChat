...
jest.mock('~/components/Chat/Input/Files/FileContainer', () => ({
    __esModule: true,
    default: ({ file, onClick }) => (<button type="button" data-testid="file-container" onClick={onClick}>
      {file.filename ?? ''}
    </button>),
}));
...
const originalScrollHeightDescriptor = Object.getOwnPropertyDescriptor(HTMLPreElement.prototype, 'scrollHeight');
...
const restoreScrollHeight = () => {
    if (originalScrollHeightDescriptor) {
        ...
        delete HTMLPreElement.prototype.scrollHeight;
    }
};
afterAll(() => {
    restoreScrollHeight();
...
describe('TextAttachment (via Attachment default export)', () => {
    ...
    it('falls through to FileAttachment when text is missing', () => {
        const noText = textAttachment({ text: undefined });
...
describe('AttachmentGroup', () => {
    ...
    it('routes text-bearing attachments through the text rendering path', () => {
        ...
        const attachments = [textAttachment({ file_id: 'a', filename: 'a.json' })];
