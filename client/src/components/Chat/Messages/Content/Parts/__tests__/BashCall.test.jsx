...
jest.mock('~/hooks', () => ({
    ...
    useProgress: (initialProgress) => initialProgress,
...
const renderBashCall = (args, commandField) => render(<RecoilRoot>
      <BashCall initialProgress={0.1} isSubmitting={true} args={args} output="" commandField={commandField}/>
    </RecoilRoot>);
describe('BashCall status text', () => {
    it.each([undefined, '', '{"command":"sleep 10"', '{"command":"sleep 10","timeout":'])('shows "Writing command" while args are missing or incomplete: %s', (args) => {
        renderBashCall(args);
    ...
    it.each(['{"command":"sleep 10"}', { command: 'sleep 10' }])('shows "Running command" once command args are complete: %s', (args) => {
        renderBashCall(args);
        expect(screen.getByTestId('progress-text')).toHaveTextContent('Running command');
    });
    it.each(['{"code":"echo hi"}', { code: 'echo hi' }])('can read bash PTC code args as the command: %s', (args) => {
