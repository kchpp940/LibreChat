...
jest.mock('~/components/Chat/Input/Files/FileContainer', () => ({
    __esModule: true,
    default: ({ file, displayName }) => (<div data-testid="file-container">{displayName ?? file.filename ?? ''}</div>),
}));
...
jest.mock('~/components/Chat/Messages/Content/Image', () => ({
    __esModule: true,
    default: ({ altText }) => <img alt={altText ?? ''} data-testid="image"/>,
}));
jest.mock('~/components/Messages/Content/Mermaid/Mermaid', () => ({
    __esModule: true,
    default: ({ children }) => (<div data-testid="mermaid-render">{children}</div>),
}));
jest.mock('~/utils', () => ({
...
const baseAttachment = (overrides = {}) => ({
    file_id: 'file-1',
    filename: 'unset',
...
const renderWithProbe = (ui, opts = {}) => {
    const streaming = opts.streaming ?? true;
    const initializeState = (snap) => {
    ...
    const utils = render(<RecoilRoot initializeState={initializeState}>
      <StateProbe onSnapshot={(snap) => {
            snapshot = snap;
        }}/>
      {ui}
    </RecoilRoot>);
...
describe('Attachment routing for tool artifacts', () => {
    it('renders an HTML artifact card (panel artifact) and exposes a download control', () => {
        ...
        expect(screen.getByRole('button', { name: /com_ui_download.*index\.html/i })).toBeInTheDocument();
    });
...
describe('ToolArtifactCard click behaviour', () => {
    ...
    it('renders sibling cards with the same file_id in one group without React key collisions', () => {
        ...
        try {
            ...
            const keyWarning = errorSpy.mock.calls.find((args) => typeof args[0] === 'string' && args[0].includes('same key'));
    ...
    it('does not ping-pong artifactsState when same file_id has divergent content across turns', () => {
        let snapshot = { ids: [], contentForDup: null };
        const ContentProbe = () => {
            const artifacts = useRecoilValue(store.artifactsState);
            React.useEffect(() => {
    ...
    it('focuses the latest card when multiple artifacts mount (legacy parity)', () => {
        ...
        const { getSnapshot } = renderWithProbe(<AttachmentGroup attachments={[olderHtml, newerHtml]}/>);
        expect(getSnapshot().currentArtifactId).toBe('tool-artifact-newer');
        expect(getSnapshot().artifactIds).toEqual(expect.arrayContaining(['tool-artifact-older', 'tool-artifact-newer']));
    });
    ...
    it('does NOT auto-open a streaming CODE artifact (test.py is click-to-open)', () => {
        ...
        render(<RecoilRoot initializeState={initializeState}>
        <StateProbe onSnapshot={(snap) => {
                snapshot = snap;
            }}/>
    ...
    it('does NOT re-auto-open on a second mount after the just-resolved flag is consumed', () => {
        ...
        const { unmount } = render(<RecoilRoot initializeState={initializeState}>
        <StateProbe onSnapshot={(snap) => {
                snapshot = snap;
            }}/>
        <Attachment attachment={xlsx}/>
        ...
        const secondInit = (snap) => {
            snap.set(store.isSubmittingFamily(0), false);
            snap.set(store.artifactsVisibility, false);
            // flag stays at default (false) — already consumed
        };
        render(<RecoilRoot initializeState={secondInit}>
        <StateProbe onSnapshot={(snap) => {
                snapshot = snap;
            }}/>
        <Attachment attachment={xlsx}/>
...
describe('AttachmentGroup routing', () => {
    ...
    it('keeps multiple downloadable files in their own collapsed group while images render outwardly', () => {
        ...
        expect(screen.getAllByTestId('file-container').map((chip) => chip.textContent)).not.toContain('c.json');
