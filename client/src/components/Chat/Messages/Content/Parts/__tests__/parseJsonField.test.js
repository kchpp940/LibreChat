import parseJsonField, { parseJsonFieldOccurrences, areToolCallArgsComplete, } from '../parseJsonField';
describe('parseJsonField', () => {
    describe('object args', () => {
        ...
        it('handles different field names — file_path', () => {
            expect(parseJsonField({ file_path: '/home/user/file.ts' }, 'file_path')).toBe('/home/user/file.ts');
        });
...
describe('parseJsonFieldOccurrences', () => {
    ...
    it('extracts repeated fields from a partial edits array in document order', () => {
        const partial = '{"file_path":"a.md","edits":[' +
...
describe('areToolCallArgsComplete', () => {
    it.each([undefined, '', '   ', '{"command":"ls"', '{"command":"ls","timeout":'])('returns false for missing or partial args %s', (args) => {
