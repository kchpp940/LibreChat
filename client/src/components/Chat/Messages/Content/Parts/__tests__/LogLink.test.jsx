...
jest.mock('~/utils', () => ({
    isHttpDownloadTarget: (target) => /^https?:\/\//i.test(target ?? ''),
    triggerDownload: (...args) => mockTriggerDownload(...args),
}));
describe('LogLink download routing', () => {
    ...
    it('navigates directly to http URLs when no stored file metadata is available', async () => {
        ...
        render(<LogLink href="https://cdn.example.com/uploads/report.pptx" filename={filename}>
        {filename}
      </LogLink>);
        fireEvent.click(screen.getByRole('link', { name: filename }));
        await waitFor(() => {
            expect(mockTriggerDownload).toHaveBeenCalledWith('https://cdn.example.com/uploads/report.pptx', 'report.pptx');
        });
    ...
    it('uses the authorized file download route when stored metadata is available', async () => {
        ...
        render(<LogLink user="user-1" file_id="file-1" filename={filename} source={FileSources.cloudfront} href="https://cdn.example.com/uploads/file.pdf">
        {filename}
      </LogLink>);
        fireEvent.click(screen.getByRole('link', { name: filename }));
        await waitFor(() => {
            expect(mockTriggerDownload).toHaveBeenCalledWith('https://cdn.example.com/signed/file.pdf', 'file.pdf');
        });
    ...
    it('keeps legacy code-output handles on the blob download path', async () => {
        ...
        render(<LogLink href="/api/files/code/download/session-1/file-1" filename={filename} source={FileSources.execute_code}>
        {filename}
      </LogLink>);
        fireEvent.click(screen.getByRole('link', { name: filename }));
        await waitFor(() => {
            expect(mockTriggerDownload).toHaveBeenCalledWith('blob:https://app.example.com/file', 'legacy.txt');
        });
