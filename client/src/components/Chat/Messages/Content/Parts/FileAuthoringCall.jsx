...
function editPreviewLines(prefix, text) {
    return text
        .split('\n')
        .map((line) => `${prefix}${line}`)
        .join('\n');
}
function formatEditPreview(edits) {
    return edits
        .map((edit, index) => {
...
function buildEditArgsPreview(args) {
...
export default function FileAuthoringCall({ toolName, isSubmitting, initialProgress = 0.1, args, output = '', attachments, hideAttachments = false, onExpand, }) {
    ...
    const { showCode, toggleCode, expandStyle, expandRef, progress, cancelled, hasError } = useToolCallState(initialProgress, isSubmitting, output, !!filePath || !!preview, onExpand);
    ...
    return (<>
      <div className="relative my-1.5 flex size-5 shrink-0 items-center gap-2.5">
        <ProgressText progress={progress} onClick={toggleCode} inProgressText={localize(isCreate ? 'com_ui_creating_file' : 'com_ui_editing_file', {
            0: fileName,
        })} finishedText={cancelled
            ? localize('com_ui_cancelled')
            : localize(isCreate ? 'com_ui_created_file' : 'com_ui_edited_file', { 0: fileName })} errorSuffix={hasError && !cancelled ? localize('com_ui_tool_failed') : undefined} icon={<Icon className={cn('size-4 shrink-0 text-text-secondary', progress < 1 && !cancelled && !hasError && 'animate-pulse')} aria-hidden="true"/>} hasInput={!!filePath || !!preview} isExpanded={showCode} error={cancelled}/>
      </div>
      <div style={expandStyle}>
        <div className="overflow-hidden" ref={expandRef}>
          {!!preview && (<div className="my-2 overflow-hidden rounded-lg border border-border-light bg-surface-secondary">
              ...
              {showOutputSection && (<pre className={cn('max-h-[300px] overflow-auto whitespace-pre-wrap break-words border-t border-border-light px-3 py-2.5 font-mono text-xs', hasError ? 'text-red-600 dark:text-red-400' : 'text-text-primary')}>
                  {output}
                </pre>)}
            </div>)}
        </div>
