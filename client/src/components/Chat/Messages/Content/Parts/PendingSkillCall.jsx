 ...
 */
export default function PendingSkillCall({ skillName, loaded, }) {
    const localize = useLocalize();
    const text = loaded
        ? localize('com_ui_skill_finished', { 0: skillName })
        : localize('com_ui_skill_running', { 0: skillName });
    return (<div className="relative my-1.5 flex size-5 shrink-0 items-center gap-2.5">
      <div className="progress-text-wrapper text-token-text-secondary relative -mt-[0.75px] h-5 w-full leading-5">
        <div className="progress-text-content absolute left-0 top-0 overflow-visible whitespace-nowrap" style={{ opacity: 1, transform: 'none' }}>
          <div className="inline-flex w-full items-center gap-2">
            <ScrollText className={cn('size-4 shrink-0 text-text-secondary', !loaded && 'animate-pulse')} aria-hidden="true"/>
