...
import { artifactTypeForAttachment, bySalience, byEntrySalience, displayFilename, isInternalSandboxArtifact, isTextAttachment, renderAttachmentKey, } from './attachmentTypes';
...
const LogContent = ({ output = '', renderImages, attachments }) => {
    ...
    const { imageAttachments, textAttachments, panelAttachments, mermaidAttachments, nonInlineAttachments, } = useMemo(() => {
        const imageAtts = [];
        const textAtts = [];
        const panelAtts = [];
        const mermaidAtts = [];
        ...
        attachments?.forEach((attachment) => {
            ...
            const expiresAt = 'expiresAt' in attachment && typeof attachment.expiresAt === 'number'
    ...
    return (<>
      ...
      {nonInlineAttachments.length > 0 && (<div>
          <p>{localize('com_generated_files')}</p>
          {nonInlineAttachments.map((file, index) => (<React.Fragment key={renderAttachmentKey('nonInline', file, index)}>
              {renderAttachment(file)}
              {index < nonInlineAttachments.length - 1 && ', '}
            </React.Fragment>))}
        </div>)}
      {panelAttachments.length > 0 && (<div className="mt-2 flex flex-wrap items-center gap-2">
          {panelAttachments.map(({ attachment, artifact }, index) => (<ToolArtifactCard key={renderAttachmentKey('artifact', attachment, index)} attachment={attachment} artifact={artifact}/>))}
        </div>)}
      {mermaidAttachments.length > 0 && (<div className="mt-2 flex flex-col gap-3">
          {mermaidAttachments.map(({ attachment, text }, index) => (<ToolMermaidArtifact key={renderAttachmentKey('mermaid', attachment, index)} attachment={attachment} text={text}/>))}
        </div>)}
      {textAttachments.length > 0 && (<div className="mt-2 flex flex-col gap-3">
          {textAttachments.map((file, index) => (<div key={renderAttachmentKey('text', file, index)} className="rounded-lg bg-surface-secondary p-3">
              {file.filename && (<div className="mb-1 truncate text-[10px] font-medium uppercase tracking-wide text-text-secondary">
                  {file.filepath ? (<LogLink href={file.filepath} filename={file.filename} file_id={file.file_id} user={file.user} source={file.source}>
                      {displayFilename(file.filename)}
                    </LogLink>) : (displayFilename(file.filename))}
                </div>)}
      ...
      {imageAttachments?.map((attachment, index) => (<Image width={attachment.width} height={attachment.height} key={renderAttachmentKey('image', attachment, index)} altText={attachment.filename} imagePath={attachment.filepath}/>))}
    </>);
};
