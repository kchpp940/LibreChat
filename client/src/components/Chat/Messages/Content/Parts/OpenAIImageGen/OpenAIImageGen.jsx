...
function computeCancelled(isSubmitting, initialProgress, hasError) {
...
export default function OpenAIImageGen({ initialProgress = 0.1, isSubmitting, toolName, args: _args = '', output, attachments, hideAttachments = false, }) {
    ...
    try {
        ...
        if (argsObj && typeof argsObj.size === 'string') {
            const [w, h] = argsObj.size.split('x').map((v) => parseInt(v, 10));
            if (!isNaN(w) && !isNaN(h)) {
                width = w;
                height = h;
            }
    ...
    const { width: imageWidth, height: imageHeight, filepath = null, filename = '', } = attachment || {};
    ...
    return (<>
      ...
      {isAgentStyle && !hideAttachments && (<div className="relative mb-2 flex w-full justify-start">
          <div ref={containerRef} className="w-full max-w-lg">
            {dimensions.width !== 'auto' && progress < 1 && (<PixelCard variant="default" progress={progress} randomness={0.6} width={dimensions.width} height={dimensions.height}/>)}
            {filepath && (<Image altText={filename} imagePath={filepath} width={Number(dimensions.width?.split('px')[0])} height={Number(dimensions.height?.split('px')[0])} args={parsedArgs}/>)}
          </div>
        </div>)}
    </>);
}
