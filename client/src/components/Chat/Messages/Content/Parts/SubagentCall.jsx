 ...
 */
function useThrottledValue(value, intervalMs, enabled) {
    ...
    const lastFireAtRef = useRef(Number.NEGATIVE_INFINITY);
    ...
    useEffect(() => {
        ...
        if (Object.is(throttledRef.current, latestValueRef.current))
            return;
        if (timerRef.current != null)
            return;
 ...
 */
export default function SubagentCall({ toolCallId, initialProgress, isSubmitting = false, args, output, attachments, persistedContent, hideAttachments = false, }) {
     ...
     */
    const liveParts = progress?.contentParts;
    ...
    const displayedTickerLines = useThrottledValue(tickerLines, SUBAGENT_TICKER_THROTTLE_MS, shouldThrottleTicker);
    ...
    const getHeaderText = () => {
        if (hasError)
            return localize('com_ui_subagent_errored');
        if (cancelled)
            return localize('com_ui_subagent_cancelled');
        if (running)
            return localize('com_ui_subagent_running');
     ...
     */
    const renderDialogPart = useCallback((part, idx, isLastPart, onToolExpand) => {
        return (<SubagentDialogPart key={`${toolCallId}-part-${idx}`} part={part} isSubmitting={running} showCursor={running && isLastPart} isLast={isLastPart} onToolExpand={onToolExpand}/>);
    }, [toolCallId, running]);
    /**
    ...
    const groupedParts = useMemo(() => {
        const withIdx = contentParts.map((part, idx) => ({ part, idx }));
    ...
    const renderDialogBody = () => {
        if (contentParts.length > 0) {
            return (<MessageContext.Provider value={dialogMessageContext}>
          {groupedParts.map((group) => {
                     ...
                     *  message view. */
                    return (<ToolCallGroup key={`${toolCallId}-group-${group.parts[0].idx}`} parts={group.parts} isSubmitting={running} isLast={group.parts.some((p) => p.idx === lastPartIndex)} renderPart={renderDialogPart} lastContentIdx={lastPartIndex}/>);
                })}
        ...
        if (output) {
            ...
            return (<MessageContext.Provider value={dialogMessageContext}>
          <SubagentDialogPart part={{
                    type: ContentTypes.TEXT,
                    text: output,
                }} isSubmitting={false} showCursor={false} isLast/>
        </MessageContext.Provider>);
        }
        return (<div className="text-sm italic text-text-secondary">
        {running
    ...
    return (<>
      <button type="button" onClick={() => setOpen(true)} className={cn('group my-1.5 flex w-full flex-col gap-1 rounded-lg border border-border-light bg-surface-secondary px-3 py-2 text-left transition hover:bg-surface-tertiary', running && 'animate-pulse-slow')} aria-label={headerText}>
        <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
          <div className={cn('flex h-5 w-5 shrink-0 items-center justify-center overflow-hidden rounded-full', running && !subagentAgent && 'animate-pulse text-primary')} aria-hidden="true">
            {subagentAgent ? (<MessageIcon message={{
                endpoint: EModelEndpoint.agents,
                isCreatedByUser: false,
            }} agent={subagentAgent}/>) : (<Users size={14}/>)}
          </div>
          <span className="shrink-0">{headerText}</span>
          {subagentNameLabel ? (<span className="min-w-0 flex-1 truncate font-normal text-text-secondary" title={subagentNameLabel}>
              {subagentNameLabel}
            </span>) : (<span className="flex-1"/>)}
          <ChevronRight size={14} className="shrink-0 text-text-secondary transition group-hover:translate-x-0.5" aria-hidden="true"/>
        </div>

        <ul className="w-full space-y-0.5 pl-5 font-mono text-xs text-text-secondary">
          {displayedTickerLines.length === 0 && running ? (<li className="truncate opacity-70">{localize('com_ui_subagent_waiting')}</li>) : null}
          {displayedTickerLines.map((line, i) => (<TickerLineView key={`${i}-${tickerLineKey(line)}`} line={line}/>))}
        </ul>
      </button>

      <OGDialog open={open} onOpenChange={setOpen}>
        <OGDialogContent className={cn('flex h-[min(85vh,56rem)] flex-col overflow-hidden p-0', 
        ...
        'w-[min(96vw,80rem)] max-w-[min(96vw,80rem)]')}>
          ...
          <div className="relative min-h-0 flex-1 border-t border-border-light bg-surface-primary">
            {!isAtBottom && (<button type="button" onClick={scrollDialogToBottom} aria-label={localize('com_ui_subagent_scroll_to_bottom')} className="absolute bottom-3 right-4 z-10 flex h-8 w-8 items-center justify-center rounded-full border border-border-light bg-surface-secondary text-text-secondary shadow-md transition hover:bg-surface-tertiary hover:text-text-primary">
    ...
    className="h-full overflow-y-auto px-3 py-3">
              <div ref={contentRef} className="flex max-w-full flex-grow flex-col gap-0">
                {prompt ? (<SubagentPrompt prompt={prompt} expanded={promptExpanded} onToggle={() => setPromptExpanded((expanded) => !expanded)}/>) : null}
                {renderDialogBody()}
              </div>
            </div>
          </div>
      ...
      {!hideAttachments && attachments && attachments.length > 0 && (<AttachmentGroup attachments={attachments}/>)}
    </>);
}
...
function tryPrompt(args) {
    try {
        return extractPrompt(JSON.parse(args));
    }
    catch {
        return undefined;
    }
}
function SubagentPrompt({ prompt, expanded, onToggle, }) {
    const localize = useLocalize();
    const headingId = useId();
    const contentId = useId();
    const toggleLabel = expanded ? localize('com_ui_collapse') : localize('com_ui_expand');
    return (<section aria-labelledby={headingId} className="mb-3 shrink-0 overflow-hidden rounded-lg border border-border-light bg-surface-secondary text-text-primary">
      <div className="flex min-h-[2.75rem] items-center justify-between gap-3 border-b border-border-light px-3 py-2">
        ...
        <button type="button" onClick={onToggle} aria-controls={contentId} aria-expanded={expanded} aria-label={toggleLabel} title={toggleLabel} className="inline-flex h-8 items-center gap-1.5 rounded-md px-2 text-xs font-medium text-text-secondary transition hover:bg-surface-tertiary hover:text-text-primary focus:outline-none focus:ring-2 focus:ring-ring">
      ...
      <div id={contentId} className={cn('relative min-w-0 px-4 py-3', expanded ? 'overflow-visible' : 'max-h-32 overflow-hidden')}>
        ...
        {!expanded && (<div className="pointer-events-none absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-surface-secondary to-transparent" aria-hidden="true"/>)}
      </div>
    </section>);
}
...
function ToolNameBadge({ name }) {
    return (<code className="shrink-0 rounded bg-surface-tertiary px-1 text-text-primary">{name}</code>);
}
...
function ToolIdentifier({ rawName, localize, }) {
    const parsed = parseToolName(rawName);
    if (parsed.mcpServer) {
        return (<span className="inline-flex min-w-0 shrink items-baseline gap-1">
 ...
 */
function TickerLineView({ line }) {
    const localize = useLocalize();
    if (line.kind === 'writing' || line.kind === 'reasoning') {
        const prefix = line.kind === 'writing'
            ? localize('com_ui_subagent_ticker_writing')
            : localize('com_ui_subagent_ticker_reasoning');
        return (<li className="flex w-full items-baseline gap-1 overflow-hidden text-text-primary">
        <span className="shrink-0">{prefix}:</span>
        <span dir="rtl" className="min-w-0 flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-left">
          {line.body}
        </span>
      </li>);
    }
    if (line.kind === 'using_tool') {
        ...
        return (<li className="flex w-full items-baseline gap-1 overflow-hidden whitespace-nowrap">
        <span className="shrink-0">{prefix}</span>
        <span className="flex min-w-0 flex-1 items-baseline gap-1 overflow-hidden">
          {line.toolNames.map((name, i) => (<span key={`${i}-${name}`} className="flex min-w-0 items-baseline gap-1">
              {i > 0 && <span className="shrink-0 text-text-tertiary">,</span>}
              <ToolIdentifier rawName={name} localize={localize}/>
            </span>))}
          {line.argsSnippet && (<span className="min-w-0 truncate text-text-tertiary">({line.argsSnippet})</span>)}
        </span>
      </li>);
    }
    ...
    return (<li className="flex w-full items-baseline gap-1 overflow-hidden text-text-warning">
 ...
 */
function SubagentDialogPart({ part, isSubmitting, showCursor, isLast, onToolExpand, }) {
    ...
    if (part.type === ContentTypes.TOOL_CALL) {
        ...
        if (!tc)
            return null;
        return (<ToolCall args={tc.args ?? ''} output={tc.output ?? ''} initialProgress={tc.progress ?? 0.1} isSubmitting={isSubmitting} isLast={isLast} name={tc.name ?? ''} onExpand={onToolExpand}/>);
    }
    return null;
}
