...
const codeBlockMock = CodeBlock;
const LANGS = ['python', 'javascript', 'typescript', 'bash', 'json', 'sql', 'go', 'rust'];
const buildMessage = (sections) => {
    const parts = [];
    for (let i = 0; i < sections; i += 1) {
        parts.push(`## Section ${i + 1}`, '');
        parts.push(`This is paragraph ${i + 1} explaining the code below with some **bold** and ` +
...
const makePrefixes = (content, steps) => {
    const prefixes = [];
...
const NewMarkdown = ({ content }) => (<Markdown content={content} isLatestMessage={true}/>);
const measure = (Component, prefixes) => {
    codeBlockMock.mockClear();
    let totalMs = 0;
    const onRender = (_id, _phase, actualDuration) => {
        totalMs += actualDuration;
    };
    const tree = (content) => (<Profiler id="bench" onRender={onRender}>
      <RecoilRoot>
...
describe('Markdown streaming benchmark (OLD whole-message vs NEW per-block)', () => {
    it('reports render cost across a simulated stream', () => {
        ...
        const minMs = (rs) => Math.min(...rs.map((r) => r.totalMs));
