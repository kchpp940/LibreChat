 ...
 */
function SkillPills({ skills, source = 'manual', }) {
    const localize = useLocalize();
    if (!skills || skills.length === 0) {
        return null;
    }
    const ariaLabelKey = source === 'always-apply'
        ? 'com_ui_skills_always_apply_invoked'
        : 'com_ui_skills_manual_invoked';
    return (<div className="flex flex-wrap gap-1.5 py-0.5" role="list" aria-label={localize(ariaLabelKey)}>
      {skills.map((name) => (<span key={name} role="listitem" data-skill-source={source} className="inline-flex items-center gap-1 rounded-full border border-border-light bg-surface-secondary px-2 py-1 text-xs text-text-secondary">
          {source === 'always-apply' ? (<Pin className="h-3 w-3 text-cyan-500" aria-hidden="true"/>) : (<ScrollText className="h-3 w-3 text-cyan-500" aria-hidden="true"/>)}
