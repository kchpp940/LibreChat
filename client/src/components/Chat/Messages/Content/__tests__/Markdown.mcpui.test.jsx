...
jest.mock('@mcp-ui/client', () => ({
    UIResourceRenderer: ({ resource }) => (<div data-testid="ui-resource-renderer" data-resource-uri={resource?.uri}/>),
}));
...
const mockUseGetMessagesByConvoId = useGetMessagesByConvoId;
...
describe('Markdown with MCP UI markers (resource IDs)', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        currentTestMessages = [];
        mockUseMessageContext.mockReturnValue({ messageId: 'msg-weather' });
        ...
        mockUseLocalize.mockReturnValue(((key) => key));
    });
    it('renders two UIResourceRenderer components for markers with resource IDs across separate attachments', () => {
        ...
        mockUseGetMessagesByConvoId.mockReturnValue({ data: currentTestMessages });
        const content = [
...
describe('Markdown table rendering', () => {
    ...
    it('wraps GFM tables in a horizontally scrollable container', () => {
        ...
        expect(screen.getByRole('table').parentElement).toHaveClass('markdown-table-wrapper', 'w-full', 'max-w-full');
    });
