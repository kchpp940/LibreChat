...
jest.mock('~/hooks', () => ({
    ...
    useProgress: (initialProgress) => (initialProgress >= 1 ? 1 : initialProgress),
...
jest.mock('../ToolCallInfo', () => ({
    __esModule: true,
    default: ({ attachments, ...props }) => (<div data-testid="tool-call-info" data-attachments={JSON.stringify(attachments)}>
      {JSON.stringify(props)}
    </div>),
}));
jest.mock('../ProgressText', () => ({
    __esModule: true,
    default: ({ onClick, inProgressText, finishedText, subtitle, }) => (<div data-testid="progress-text" onClick={onClick}>
      {finishedText || inProgressText}
      {subtitle && <span data-testid="subtitle">{subtitle}</span>}
    </div>),
}));
jest.mock('../Parts', () => ({
    AttachmentGroup: ({ attachments }) => (<div data-testid="attachment-group">{JSON.stringify(attachments)}</div>),
}));
...
describe('ToolCall', () => {
    ...
    describe('attachment group rendering', () => {
        ...
        it('should not render AttachmentGroup when hideAttachments is true (grouped path)', () => {
            ...
            renderWithRecoil(<ToolCall {...mockProps} attachments={attachments} hideAttachments/>);
            expect(screen.queryByTestId('attachment-group')).not.toBeInTheDocument();
        });
        it('should render AttachmentGroup when hideAttachments is false explicitly', () => {
            ...
            renderWithRecoil(<ToolCall {...mockProps} attachments={attachments} hideAttachments={false}/>);
    ...
    describe('authentication flow', () => {
        it('should show sign-in button when auth URL is provided', () => {
            ...
            window.open = jest.fn();
            renderWithRecoil(<ToolCall {...mockProps} initialProgress={0.5} // Less than 1 so it's not complete
             auth="https://auth.example.com" isSubmitting={true} // Should be submitting for auth to show
            />);
            const signInButton = screen.getByText('Sign in to auth.example.com');
            expect(signInButton).toBeInTheDocument();
            fireEvent.click(signInButton);
            expect(window.open).toHaveBeenCalledWith('https://auth.example.com', '_blank', 'noopener,noreferrer');
            window.open = originalOpen;
        });
        it('should not show auth section when cancelled', () => {
            renderWithRecoil(<ToolCall {...mockProps} auth="https://auth.example.com" initialProgress={0.5} isSubmitting={false} // Not submitting + progress < 1 = cancelled
            />);
        ...
        it('should not show auth section when progress is complete', () => {
            renderWithRecoil(<ToolCall {...mockProps} auth="https://auth.example.com" initialProgress={1} isSubmitting={false}/>);
            expect(screen.queryByText('Sign in to auth.example.com')).not.toBeInTheDocument();
        });
    });
    describe('edge cases', () => {
    ...
    describe('MCP OAuth detection', () => {
        ...
        it('should detect MCP OAuth from delimiter in tool-call name', () => {
            renderWithRecoil(<ToolCall {...mockProps} name={`oauth${d}my-server`} initialProgress={0.5} isSubmitting={true} auth="https://auth.example.com"/>);
        ...
        it('should preserve full server name when it contains the delimiter substring', () => {
            renderWithRecoil(<ToolCall {...mockProps} name={`oauth${d}foo${d}bar`} initialProgress={0.5} isSubmitting={true} auth="https://auth.example.com"/>);
            const subtitle = screen.getByTestId('subtitle');
        ...
        it('should display server name (not "oauth") as function_name for OAuth tool calls', () => {
            renderWithRecoil(<ToolCall {...mockProps} name={`oauth${d}my-server`} initialProgress={1} isSubmitting={false} output="done" auth="https://auth.example.com"/>);
        ...
        it('should display server name even when auth is cleared (post-completion)', () => {
            ...
            renderWithRecoil(<ToolCall {...mockProps} name={`oauth${d}my-server`} initialProgress={1} isSubmitting={false} output="done"/>);
            const progressText = screen.getByTestId('progress-text');
            expect(progressText.textContent).toContain('Completed my-server');
            expect(progressText.textContent).not.toContain('Completed oauth');
        });
        it('should fallback to auth URL redirect_uri when name lacks delimiter', () => {
            const authUrl = 'https://oauth.example.com/authorize?redirect_uri=' +
                encodeURIComponent('https://app.example.com/api/mcp/my-server/oauth/callback');
            renderWithRecoil(<ToolCall {...mockProps} name="bare_name" initialProgress={0.5} isSubmitting={true} auth={authUrl}/>);
            const subtitle = screen.getByTestId('subtitle');
            expect(subtitle.textContent).toBe('via my-server');
        });
        it('should display server name (not raw tool-call ID) in fallback path finished text', () => {
            const authUrl = 'https://oauth.example.com/authorize?redirect_uri=' +
                encodeURIComponent('https://app.example.com/api/mcp/my-server/oauth/callback');
            renderWithRecoil(<ToolCall {...mockProps} name="bare_name" initialProgress={1} isSubmitting={false} output="done" auth={authUrl}/>);
            const progressText = screen.getByTestId('progress-text');
            expect(progressText.textContent).toContain('Completed my-server');
        ...
        it('should show normalized server name when it contains _mcp_ after prefixing', () => {
            ...
            renderWithRecoil(<ToolCall {...mockProps} name={`oauth${d}oauth${d}server`} initialProgress={0.5} isSubmitting={true} auth="https://auth.example.com"/>);
            const subtitle = screen.getByTestId('subtitle');
        ...
        it('should not misidentify non-MCP action auth as MCP via fallback', () => {
            ...
            renderWithRecoil(<ToolCall {...mockProps} name="action_name" initialProgress={0.5} isSubmitting={true} auth={authUrl}/>);
    ...
    describe('A11Y-04: screen reader status announcements', () => {
        it('includes sr-only aria-live region for status announcements', () => {
            renderWithRecoil(<ToolCall {...mockProps} initialProgress={1} isSubmitting={false} name="test_func" output="result"/>);
