...
jest.mock('~/utils', () => ({
    mapAttachments: () => ({}),
    groupSequentialToolCalls: (parts) => parts.map((p) => ({ type: 'single', part: p })),
}));
jest.mock('~/Providers', () => ({
    MessageContext: {
...
jest.mock('../Parts/PendingSkillCall', () => ({
    __esModule: true,
    default: ({ skillName, loaded }) => (<div data-testid="pending-skill-call" data-skill={skillName} data-loaded={String(loaded)}/>),
}));
...
jest.mock('../Part', () => ({
    __esModule: true,
    default: ({ part }) => (<div data-testid={`real-part-${part.type}`}/>),
}));
...
describe('ContentParts — interim skill cards', () => {
    ...
    it('flips pending cards to loaded once any real content part arrives', () => {
        const content = [
            { type: ContentTypes.TEXT, text: 'streamed' },
        ];
    ...
    it('renders nothing when manualSkills is empty and content is undefined', () => {
        const { container } = render(<ContentParts {...baseProps} content={undefined} manualSkills={[]}/>);
    ...
    it('renders pending skill cards above parallel content', () => {
        ...
        expect(skillCard.compareDocumentPosition(parallelRenderer)).toBe(Node.DOCUMENT_POSITION_FOLLOWING);
    });
