...
jest.mock('~/hooks', () => ({
    useLocalize: () => (key, params) => `${key}:${params?.[0] ?? ''}`,
}));
...
describe('SkillPills', () => {
    ...
    it('switches aria-label and data attribute for source="always-apply"', () => {
        ...
        expect(screen.getByRole('list')).toHaveAttribute('aria-label', 'com_ui_skills_always_apply_invoked:');
        const items = screen.getAllByRole('listitem');
    ...
    it('renders the pin-icon variant for always-apply (no ScrollText)', () => {
        const { container: alwaysApply } = render(<SkillPills skills={['legal']} source="always-apply"/>);
