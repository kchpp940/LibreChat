...
jest.mock('../ToolOutput', () => ({
    StackedToolIcons: ({ toolNames }) => (<span data-testid="stacked-icons" data-tool-names={toolNames.join(',')}/>),
    getMCPServerName: () => '',
}));
...
jest.mock('~/utils', () => ({
    ...
    getToolDisplayLabel: (name) => ['execute_code', 'bash_tool', 'run_tools_with_code', 'run_tools_with_bash'].includes(name)
        ? 'Code'
        : name,
}));
jest.mock('../Parts', () => ({
...
const makePart = (id, output = 'done', name = 'fetch_image', args = '{}') => ({
...
const renderGroup = (props) => render(<RecoilRoot>
      <ToolCallGroup {...props}/>
    </RecoilRoot>);
const mockScheduleMessageContentLayoutReconcile = scheduleMessageContentLayoutReconcile;
describe('ToolCallGroup image hoisting', () => {
    ...
    const baseProps = {
        ...
        isLast: false,
        lastContentIdx: 1,
        renderPart: (_p, idx) => (<div data-testid={`inner-${idx}`} key={idx}>
        {'inner'}
      </div>),
    };
    beforeEach(() => {
    ...
    it('does not render tool bodies for an initially collapsed large completed group', () => {
        ...
        const renderPart = jest.fn((_p, idx) => (<div data-testid={`inner-${idx}`} key={idx}>
        {'inner'}
      </div>));
        renderGroup({
            ...baseProps,
    ...
    it('summarizes mixed bash PTC and bash_tool calls as one Code tool family', () => {
        ...
        expect(screen.getByTestId('stacked-icons')).toHaveAttribute('data-tool-names', 'bash_tool,bash_tool');
    });
});
