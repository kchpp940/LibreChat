...
jest.mock('~/components/Chat/Messages/Content/Image', () => ({
    __esModule: true,
    default: ({ altText, imagePath }) => (<div data-testid="image-component" data-alt={altText} data-src={imagePath}/>),
}));
jest.mock('@librechat/client', () => ({
    PixelCard: ({ progress }) => (<div data-testid="pixel-card" data-progress={progress}/>),
}));
...
jest.mock('../Parts/OpenAIImageGen/ProgressText', () => ({
    __esModule: true,
    default: ({ progress, error }) => (<div data-testid="progress-text" data-progress={progress} data-error={String(error)}/>),
}));
describe('OpenAIImageGen', () => {
    ...
    describe('image visibility', () => {
        ...
        it('shows Image when filepath is available', () => {
            render(<OpenAIImageGen {...defaultProps} initialProgress={1} isSubmitting={false} attachments={[
                    {
                        filename: 'cat.png',
    ...
    describe('args parsing', () => {
        ...
        it('handles object args', () => {
            render(<OpenAIImageGen {...defaultProps} args={{ prompt: 'a dog', quality: 'low', size: '512x512' }}/>);
            expect(screen.getByTestId('progress-text')).toBeInTheDocument();
        });
    });
    describe('cancellation', () => {
        it('shows error state when output contains error', () => {
            render(<OpenAIImageGen {...defaultProps} output="Error processing tool call" isSubmitting={false} initialProgress={0.5}/>);
