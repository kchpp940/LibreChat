...
describe('MemoryInfo', () => {
    const createMemoryArtifact = (type, key, value) => ({
        type,
        key,
    ...
    describe('Error Memory Display', () => {
        test('displays error section when memory is already exceeded', () => {
            ...
            expect(screen.getByText('Memory storage already full - exceeded by 150 tokens. Delete existing memories before adding new ones.')).toBeInTheDocument();
        });
        test('displays error when memory would exceed limit', () => {
            ...
            render(<MemoryInfo memoryArtifacts={memoryArtifacts}/>);
            expect(screen.getByText('Memory Storage Full')).toBeInTheDocument();
            expect(screen.getByText('Cannot save - would exceed limit by 50 tokens. Delete existing memories to make space.')).toBeInTheDocument();
        });
        test('displays multiple error messages', () => {
            ...
            render(<MemoryInfo memoryArtifacts={memoryArtifacts}/>);
            expect(screen.getByText('Memory storage already full - exceeded by 100 tokens. Delete existing memories before adding new ones.')).toBeInTheDocument();
            expect(screen.getByText('Cannot save - would exceed limit by 25 tokens. Delete existing memories to make space.')).toBeInTheDocument();
        });
        test('applies correct styling to error messages', () => {
            ...
            render(<MemoryInfo memoryArtifacts={memoryArtifacts}/>);
            const errorMessage = screen.getByText('Cannot save - would exceed limit by 50 tokens. Delete existing memories to make space.');
    ...
    describe('Mixed Memory Types', () => {
        test('displays all sections when different memory types are present', () => {
            ...
            expect(screen.getByText('Cannot save - would exceed limit by 200 tokens. Delete existing memories to make space.')).toBeInTheDocument();
        });
    ...
    describe('Edge Cases', () => {
        ...
        test('returns null when no memories of any type exist', () => {
            const memoryArtifacts = [{ type: 'unknown', key: 'test' }];
