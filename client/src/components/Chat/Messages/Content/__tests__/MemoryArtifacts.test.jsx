...
jest.mock('../MemoryInfo', () => ({
    __esModule: true,
    default: ({ memoryArtifacts }) => (<div data-testid="memory-info">
      {memoryArtifacts.map((artifact, index) => (<div key={index} data-testid={`memory-artifact-${artifact.type}`}>
          {artifact.type}: {artifact.key}
        </div>))}
    </div>),
}));
describe('MemoryArtifacts', () => {
    const createMemoryAttachment = (type, key) => ({
        type: Tools.memory,
        [Tools.memory]: {
            type,
            key,
    ...
    describe('Memory Artifacts Filtering', () => {
        test('filters and passes only memory-type attachments to MemoryInfo', () => {
            const attachments = [
                createMemoryAttachment('update', 'memory1'),
                { type: 'file' }, // Non-memory attachment
                createMemoryAttachment('error', 'system'),
            ];
            render(<MemoryArtifacts attachments={attachments}/>);
            // Click to expand
    ...
    describe('Edge Cases', () => {
        ...
        test('handles attachments with no memory artifacts', () => {
            const attachments = [{ type: 'file' }, { type: 'image' }];
            render(<MemoryArtifacts attachments={attachments}/>);
            expect(screen.queryByRole('button')).not.toBeInTheDocument();
        });
