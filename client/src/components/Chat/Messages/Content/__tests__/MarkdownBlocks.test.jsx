...
jest.mock('~/components/Messages/Content/CodeBlock', () => ({
    __esModule: true,
    default: ({ lang, blockIndex }) => (<div data-testid="cb" data-block-index={String(blockIndex)} data-lang={String(lang)}/>),
}));
/** The previous whole-message renderer: a single ReactMarkdown under one set of providers. */
const OldMarkdown = ({ content }) => (<ArtifactProvider>
    <CodeBlockProvider>
      <ReactMarkdown 
/** @ts-ignore */
remarkPlugins={getRemarkPlugins()} 
/** @ts-ignore */
rehypePlugins={getRehypePlugins()} components={getMarkdownComponents()}>
        {content}
      </ReactMarkdown>
    </CodeBlockProvider>
  </ArtifactProvider>);
...
const indicesIn = (container) => Array.from(container.querySelectorAll('[data-testid="cb"]')).map((el) => ({
    idx: el.getAttribute('data-block-index'),
    lang: el.getAttribute('data-lang'),
}));
const streamThrough = (Component, content) => {
    const lines = content.split('\n');
    const { container, rerender } = render(<RecoilRoot>
...
const NewMarkdown = ({ content }) => (<Markdown content={content} isLatestMessage={false}/>);
describe('MarkdownBlocks code-block index parity', () => {
    it('assigns document-order indices on a direct render (matches whole-message renderer)', () => {
        const { container: oldC } = render(<RecoilRoot>
        <OldMarkdown content={MIXED}/>
      </RecoilRoot>);
        const { container: newC } = render(<RecoilRoot>
    ...
    it('streamed indices match a fresh direct render (stable for stored execution results)', () => {
        ...
        const { container: fresh } = render(<RecoilRoot>
        <Markdown content={MIXED} isLatestMessage={false}/>
      </RecoilRoot>);
