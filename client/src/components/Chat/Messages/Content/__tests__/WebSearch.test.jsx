...
jest.mock('~/components/Web/SourceHovercard', () => ({
    FaviconImage: ({ domain }) => (<span data-testid="favicon" data-domain={domain}/>),
    getCleanDomain: (url) => {
        try {
...
jest.mock('~/components/Web/Sources', () => ({
    StackedFavicons: ({ sources }) => (<span data-testid="stacked-favicons" data-count={sources.length}/>),
}));
jest.mock('lucide-react', () => ({
    Globe: () => <span data-testid="globe-icon"/>,
    ChevronDown: () => <span data-testid="chevron-icon"/>,
}));
function makeSource(link, title) {
    return { link, title };
}
function makeSearchResults(turns) {
    const results = {};
    for (const [turn, data] of Object.entries(turns)) {
        results[turn] = { turn: Number(turn), ...data };
    }
    return results;
}
function makeAttachment(turn, data) {
    return {
        type: Tools.web_search,
        [Tools.web_search]: { ...data, turn },
    };
}
function renderWebSearch({ searchResults, attachments, isSubmitting = false, isLast = false, initialProgress = 1, output, }) {
    return render(<RecoilRoot>
      <SearchContext.Provider value={{ searchResults }}>
        <WebSearch initialProgress={initialProgress} isSubmitting={isSubmitting} isLast={isLast} output={output} attachments={attachments}/>
...
describe('WebSearch', () => {
    describe('turn scoping', () => {
        ...
        it('two instances under the same SearchContext show distinct sources', () => {
            const { container: container0 } = render(<RecoilRoot>
          <SearchContext.Provider value={{ searchResults }}>
            <WebSearch initialProgress={1} isSubmitting={false} attachments={[makeAttachment(0, searchResults['0'])]}/>
          </SearchContext.Provider>
        </RecoilRoot>);
            const { container: container1 } = render(<RecoilRoot>
          <SearchContext.Provider value={{ searchResults }}>
            <WebSearch initialProgress={1} isSubmitting={false} attachments={[makeAttachment(1, searchResults['1'])]}/>
          </SearchContext.Provider>
        </RecoilRoot>);
            const links0 = Array.from(container0.querySelectorAll('a[href]')).map((a) => a.getAttribute('href'));
            const links1 = Array.from(container1.querySelectorAll('a[href]')).map((a) => a.getAttribute('href'));
    ...
    describe('processedSources scoping', () => {
        it('shows processed sources only from ownTurn during streaming', () => {
            const searchResults = makeSearchResults({
                0: {
                    organic: [
                        { link: 'https://a.com', title: 'A', processed: true },
                        { link: 'https://b.com', title: 'B', processed: false },
                    ],
                },
                1: {
                    organic: [{ link: 'https://c.com', title: 'C', processed: true }],
                },
            });
            const attachments = [makeAttachment(0, searchResults['0'])];
            renderWebSearch({
