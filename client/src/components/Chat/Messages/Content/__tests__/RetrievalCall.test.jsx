...
jest.mock('../ProgressText', () => ({
    __esModule: true,
    default: ({ onClick, inProgressText, finishedText, icon, hasInput, isExpanded, error, errorSuffix, }) => (<div onClick={onClick} data-testid="progress-text" data-in-progress={inProgressText} data-finished={finishedText} data-has-input={hasInput} data-expanded={isExpanded} data-error={error} data-error-suffix={errorSuffix}>
      {icon}
      {finishedText || inProgressText}
    </div>),
}));
jest.mock('../ToolOutput', () => ({
    ToolIcon: ({ type, isAnimating }) => (<span data-testid="tool-icon" data-type={type} data-animating={isAnimating}/>),
    OutputRenderer: ({ text }) => <pre data-testid="output-renderer">{text}</pre>,
    isError: (output) => typeof output === 'string' && output.toLowerCase().includes('error'),
}));
jest.mock('~/utils', () => ({
...
jest.mock('../FilePreviewDialog', () => ({
    __esModule: true,
    default: ({ open, fileId, fileName }) => open ? (<div data-testid="file-preview-dialog" data-file-id={fileId}>
        {fileName}
      </div>) : null,
}));
const defaultProps = {
...
const renderRetrievalCall = (props = {}) => render(<RecoilRoot>
      <RetrievalCall {...defaultProps} {...props}/>
    </RecoilRoot>);
const mockedUseGetFiles = jest.requireMock('~/data-provider').useGetFiles;
beforeEach(() => {
...
describe('RetrievalCall - file preview resolution', () => {
    ...
    it('keeps multiple parsed results clickable when only one attachment source is available', () => {
        ...
        fireEvent.click(screen.getByTestId('progress-text'));
        expect(screen.getAllByRole('button', { name: 'Preview: Tutorial Imazing.pdf' })).toHaveLength(2);
    });
});
