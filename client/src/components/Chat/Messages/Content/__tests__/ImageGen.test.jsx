...
const getProgressLabel = (progress) => progress >= 1 ? 'Image created' : 'Generating image...';
jest.mock('../Parts/OpenAIImageGen/ProgressText', () => ({
    __esModule: true,
    default: ({ toolName, progress, error, onClick, hasInput, isExpanded, }) => (<button data-testid="progress-text" data-tool-name={toolName} data-progress={progress} data-error={error} data-has-input={hasInput} data-is-expanded={isExpanded} onClick={onClick} type="button">
      {error ? 'Error' : getProgressLabel(progress)}
    </button>),
}));
jest.mock('@librechat/client', () => ({
    PixelCard: (props) => (<div data-testid="pixel-card" data-progress={props.progress}/>),
}));
...
const renderImageGen = (props = {}) => render(<RecoilRoot>
...
describe('ImageGen - LGCY-01: Legacy and agent-style unification', () => {
    it('accepts legacy props (initialProgress + args only)', () => {
        ...
        <ImageGen initialProgress={1} isSubmitting={false} toolName="" args='{"prompt":"a cat"}' output=""/>
      </RecoilRoot>);
