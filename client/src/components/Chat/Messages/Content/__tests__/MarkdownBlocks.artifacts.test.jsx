...
jest.mock('~/components/Artifacts/ArtifactButton', () => ({
    __esModule: true,
    default: ({ artifact }) => (<div data-testid="art" data-index={String(artifact?.index)} data-id={String(artifact?.identifier)}/>),
}));
const artifact = (id, title) => `:::artifact{identifier="${id}" type="text/markdown" title="${title}"}\nhello ${id}\n:::`;
const wrap = (ui) => (<MemoryRouter>
    <RecoilRoot>
...
const readArtifacts = async () => (await screen.findAllByTestId('art')).map((el) => ({
