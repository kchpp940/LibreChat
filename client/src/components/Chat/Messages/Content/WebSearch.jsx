...
function getUniqueDomainSources(sources, max) {
    const seen = new Set();
    const result = [];
...
function SourceFaviconStack({ sources }) {
    const visible = getUniqueDomainSources(sources, MAX_VISIBLE_FAVICONS);
    return (<div className="flex items-center" aria-hidden="true">
      {visible.map((source, i) => (<div key={source.link} className={cn('relative flex items-center justify-center rounded-full border border-border-medium bg-surface-secondary', 'h-[22px] w-[22px]', i > 0 && '-ml-2.5')} style={{ zIndex: MAX_VISIBLE_FAVICONS - i }}>
          <FaviconImage domain={getCleanDomain(source.link)} className="size-3 rounded-full"/>
        </div>))}
    </div>);
}
export default function WebSearch({ initialProgress: progress = 0.1, isSubmitting, isLast, output, attachments, onExpand, }) {
    ...
    const hasResults = useMemo(() => attachments?.some((att) => att.type === Tools.web_search && att[Tools.web_search]) ?? false, [attachments]);
    ...
    const processedSources = useMemo(() => {
        ...
        return [...(result.organic || []), ...(result.topStories || [])].filter((source) => source.processed === true);
    }, [searchResults, complete, finalizing, ownTurn]);
    const showSources = processedSources.length > 0;
    const progressText = useMemo(() => {
        let text = ownTurn !== '0' ? 'com_ui_web_searching_again' : 'com_ui_web_searching';
        if (showSources) {
    ...
    if (complete) {
        ...
        <button type="button" className={cn('tool-status-text group flex items-center gap-2 rounded-full py-1 transition-colors', hasSourceData
                ? 'text-text-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-heavy'
                : 'pointer-events-none text-text-secondary')} disabled={!hasSourceData} onClick={hasSourceData ? handleToggleSources : undefined} aria-expanded={hasSourceData ? showSourceList : undefined} aria-label={hasSourceData
                ? `${completedText} - ${localize(sourceCount === 1 ? 'com_ui_web_search_source' : 'com_ui_web_search_sources', { count: sourceCount })}`
                : completedText}>
          {hasSourceData ? (<SourceFaviconStack sources={allSources}/>) : (<Globe className="size-4 shrink-0 text-text-secondary" aria-hidden="true"/>)}
          <span className="font-medium">{completedText}</span>
          {hasSourceData && (<ChevronDown className={cn('size-3.5 shrink-0 text-text-secondary transition-transform duration-200', showSourceList && 'rotate-180')} aria-hidden="true"/>)}
        </button>
        {hasSourceData && (<div style={sourceExpandStyle}>
            <div className="overflow-hidden" ref={sourceExpandRef}>
              <div className="my-2 max-h-[280px] overflow-y-auto rounded-lg border border-border-light">
                {allSources.map((source, i) => {
                    const domain = getCleanDomain(source.link);
                    return (<a key={source.link} href={source.link} target="_blank" rel="noopener noreferrer" className={cn('flex items-center gap-2.5 px-3 py-2 transition-colors hover:bg-surface-hover', i > 0 && 'border-t border-border-light')}>
    ...
    return (<div className="my-1 flex items-center gap-2.5">
      <span className="sr-only" aria-live="polite" aria-atomic="true">
        {progressText}
      </span>
