...
const AgentHandoff = ({ name, args: _args = '' }) => {
    ...
    return (<div className="my-1">
      <button type="button" className={cn('tool-status-text flex appearance-none items-center gap-2.5 bg-transparent text-text-secondary', hasInfo
            ...
            : 'pointer-events-none')} disabled={!hasInfo} onClick={hasInfo ? () => setShowInfo(!showInfo) : undefined} aria-expanded={hasInfo ? showInfo : undefined} aria-label={`${localize('com_ui_transferred_to')} ${targetAgent?.name || localize('com_ui_agent')}`}>
        ...
        {hasInfo && (<ChevronDown className={cn('ml-1 h-3 w-3 transition-transform', showInfo && 'rotate-180')} aria-hidden="true"/>)}
      </button>
      <div style={expandStyle}>
        <div className="overflow-hidden" ref={expandRef}>
          {hasInfo && (<div className="ml-8 mt-2 rounded-lg border border-border-light bg-surface-secondary p-3 text-xs">
