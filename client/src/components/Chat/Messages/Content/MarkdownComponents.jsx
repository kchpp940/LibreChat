...
const isSingleLineCode = (children) => {
...
export const code = memo(function MarkdownCode({ className, children, }) {
    ...
    if (isMath) {
        ...
        return (<MermaidErrorBoundary code={content}>
        <Mermaid id={`mermaid-${blockIndex}`}>{content}</Mermaid>
      </MermaidErrorBoundary>);
    }
    else if (isSingleLine) {
        return (<code onDoubleClick={handleDoubleClick} className={className}>
        {children}
      </code>);
    }
    else {
        return (<CodeBlock lang={lang ?? 'text'} codeChildren={children} blockIndex={blockIndex} allowExecution={canRunCode}/>);
    }
});
code.displayName = 'MarkdownCode';
export const codeNoExecution = memo(function MarkdownCodeNoExecution({ className, children, }) {
    const match = /language-(\w+)/.exec(className ?? '');
    const lang = match && match[1];
    if (lang === 'math') {
        return children;
...
export const a = memo(function MarkdownAnchor({ href, children }) {
    ...
    return (<a href={filepath?.startsWith('files/')
            ? `${domainServerBaseUrl}/${filepath}`
            : `${domainServerBaseUrl}/files/${filepath}`} {...props}>
      {children}
    </a>);
});
a.displayName = 'MarkdownAnchor';
export const p = memo(function MarkdownParagraph({ children }) {
    return <p className="mb-2 whitespace-pre-wrap">{children}</p>;
});
p.displayName = 'MarkdownParagraph';
export const table = memo(function MarkdownTable({ children }) {
    return (<div className="markdown-table-wrapper w-full max-w-full">
      <table>{children}</table>
    </div>);
});
table.displayName = 'MarkdownTable';
export const img = memo(function MarkdownImage({ src, alt, title, className, style, }) {
