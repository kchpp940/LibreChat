 ...
 */
export default function SiblingHeader({ agentId, messageId, conversationId, isSubmitting, }) {
    ...
    const { displayName, displayEndpoint, displayModel, agent } = useMemo(() => {
        ...
        if (agentId) {
            ...
            const foundAgent = agentsMap?.[baseAgentId];
            if (foundAgent) {
                return {
    ...
    return (<div className="mb-2 flex items-center justify-between gap-2 border-b border-border-light pb-2">
      ...
      <button type="button" onClick={handleBranch} disabled={!messageId || !agentId || isSubmitting || branchMessage.isLoading} className={cn('flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-md', 'text-text-secondary transition-colors hover:bg-surface-hover hover:text-text-primary', 'focus:outline-none focus:ring-2 focus:ring-border-medium focus:ring-offset-1', 'disabled:cursor-not-allowed disabled:opacity-50', (!messageId || !agentId || isSubmitting) && 'invisible')} aria-label={localize('com_ui_branch_message')} title={localize('com_ui_branch_message')}>
