 ...
 */
const renderedCodeLang = (lang) => /language-(\w+)/.exec(`language-${lang}`)?.[1] ?? '';
/**
 ...
 */
const parseToMdast = (content) => fromMarkdown(content, {
...
export function splitMarkdownIntoBlocks(content) {
    ...
    const requiresWholeMessage = children.some((node) => node.type === 'html' || containsDefinition(node));
