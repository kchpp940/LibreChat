...
describe('MessageIcon arePropsEqual', () => {
    it('returns true when agent reference changes but display fields are identical', () => {
        ...
        expect(arePropsEqual({ iconData: baseIconData, agent: agent1 }, { iconData: baseIconData, agent: agent2 })).toBe(true);
    });
    it('returns false when agent name changes', () => {
        expect(arePropsEqual({ iconData: baseIconData, agent: makeAgent({ name: 'Atlas' }) }, { iconData: baseIconData, agent: makeAgent({ name: 'Hermes' }) })).toBe(false);
    });
    ...
    it('returns true when assistant reference changes but display fields are identical', () => {
        ...
        expect(arePropsEqual({ iconData: baseIconData, assistant: asst1 }, { iconData: baseIconData, assistant: asst2 })).toBe(true);
    });
    it('returns false when assistant name changes', () => {
        expect(arePropsEqual({ iconData: baseIconData, assistant: makeAssistant({ name: 'Helper' }) }, { iconData: baseIconData, assistant: makeAssistant({ name: 'Wizard' }) })).toBe(false);
    });
    it('returns false when assistant avatar changes', () => {
        expect(arePropsEqual({ iconData: baseIconData, assistant: makeAssistant({ metadata: { avatar: '/a.png' } }) }, { iconData: baseIconData, assistant: makeAssistant({ metadata: { avatar: '/b.png' } }) })).toBe(false);
    });
    it('returns true when iconData reference changes but fields are identical', () => {
        ...
        expect(arePropsEqual({ iconData: iconData1, agent: makeAgent() }, { iconData: iconData2, agent: makeAgent() })).toBe(true);
    });
    it('returns false when iconData endpoint changes', () => {
        expect(arePropsEqual({ iconData: { ...baseIconData, endpoint: 'agents' } }, { iconData: { ...baseIconData, endpoint: 'openAI' } })).toBe(false);
    });
    it('returns false when iconData iconURL changes', () => {
        expect(arePropsEqual({ iconData: { ...baseIconData, iconURL: '/a.png' } }, { iconData: { ...baseIconData, iconURL: '/b.png' } })).toBe(false);
    });
    ...
    it('returns true when agent transitions from undefined to object with undefined display fields', () => {
        ...
        expect(arePropsEqual({ iconData: baseIconData, agent: undefined }, { iconData: baseIconData, agent: agentNoFields })).toBe(true);
    });
    it('returns false when agent transitions from defined to undefined', () => {
        expect(arePropsEqual({ iconData: baseIconData, agent: makeAgent() }, { iconData: baseIconData, agent: undefined })).toBe(false);
    });
});
