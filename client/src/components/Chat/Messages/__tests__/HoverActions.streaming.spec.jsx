...
import { ChatContext, MessagesViewProvider } from '~/Providers';
...
jest.mock('~/hooks', () => {
    ...
    const useMemoizedChatContext = jest.requireActual('~/hooks/Messages/useMemoizedChatContext').default;
    return {
        useMessageProcess,
        useMemoizedChatContext,
        useContentMetadata: () => ({ hasParallelContent: false }),
        useLocalize: () => (key) => key,
        useMessageActions: ({ chatContext, message, }) => ({
            ask: chatContext.ask,
            edit: false,
...
function DerivedStreamingRow() {
    ...
    const chatContext = useMemo(() => ({
        ...
        getMessages: () => queryClient.getQueryData([QueryKeys.messages, conversation.conversationId]),
        setMessages: (messages) => {
            queryClient.setQueryData([QueryKeys.messages, conversation.conversationId], messages);
        },
...
function renderStreamingRow() {
    ...
    queryClient.setQueryData([QueryKeys.messages, conversation.conversationId], [userMessage, optimisticAssistantMessage]);
...
describe('streaming hover actions', () => {
    ...
    it('does not mount hover actions while an optimistic assistant row is replaced', async () => {
        ...
        act(() => {
            queryClient.setQueryData([QueryKeys.messages, conversation.conversationId], [userMessage, canonicalAssistantMessage]);
        });
        expect(screen.queryByTestId('hover-buttons')).toBeNull();
        expect(mockHoverButtonsRenderCount).toBe(0);
