...
jest.mock('~/components/Endpoints/ConvoIconURL', () => {
    const ConvoIconURL = (props) => {
        iconRenderCount.current += 1;
        return <div data-testid="convo-icon-url" data-icon-url={props.iconURL}/>;
    };
...
jest.mock('~/components/Endpoints/Icon', () => {
    const Icon = (props) => {
        iconRenderCount.current += 1;
        return <div data-testid="icon" data-icon-url={props.iconURL}/>;
    };
...
describe('MessageIcon render cycles', () => {
    ...
    it('renders same-origin absolute model spec icon URLs directly', () => {
        render(<MessageIcon iconData={{
                ...baseIconData,
                iconURL: '/assets/clickhouse-logo.svg',
            }}/>);
        expect(screen.getByTestId('convo-icon-url')).toHaveAttribute('data-icon-url', '/assets/clickhouse-logo.svg');
    });
