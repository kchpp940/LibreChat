...
jest.mock('~/data-provider', () => ({
    useGetMessagesByConvoId: (...args) => mockUseGetMessagesByConvoId(...args),
}));
jest.mock('~/Providers', () => ({
    useMessagesConversation: (...args) => mockUseMessagesConversation(...args),
    useMessagesSubmission: (...args) => mockUseMessagesSubmission(...args),
}));
jest.mock('~/hooks', () => ({
    useLocalize: () => (key, opts) => opts ? `${key}|${JSON.stringify(opts)}` : key,
}));
jest.mock('@librechat/client', () => ({
    HoverCard: ({ children }) => <>{children}</>,
    HoverCardTrigger: ({ children, asChild }) => asChild ? children : <div>{children}</div>,
    HoverCardPortal: ({ children }) => <>{children}</>,
    HoverCardContent: ({ children, className }) => (<div data-testid="hover-card-content" className={className}>
      {children}
    </div>),
}));
...
describe('MessageNav', () => {
    ...
    describe('accessibility', () => {
        ...
        it('labels each indicator with its role-specific key and localized preview', () => {
            ...
            expect(assistantInd.getAttribute('aria-label')).toMatch(/^com_ui_message_nav_go_to_assistant\|/);
        });
        ...
        it('chevron buttons expose a disabled state when there is nothing to navigate to', () => {
            ...
            const prev = container.querySelector('button[aria-label="com_ui_message_nav_previous"]');
            const next = container.querySelector('button[aria-label="com_ui_message_nav_next"]');
    ...
    describe('scroll behavior', () => {
        it('schedules a rAF when an indicator is clicked', () => {
            ...
            const target = container.querySelectorAll('[data-msg-id]')[2];
            act(() => {
        ...
        it('scrolls the indicator column fully down when the final message is visible', () => {
            const messages = Array.from({ length: 15 }, (_, i) => buildMessage({
                messageId: `m-${i}`,
                text: `message ${i}`,
                isCreatedByUser: i % 2 === 0,
            }));
            const { container, scrollable } = renderNav(messages);
            const column = container.querySelector('nav > div');
        ...
        it('scrolls the indicator column fully down when the chat reaches bottom with stale offsets', () => {
            ...
            document.body.appendChild(scrollable);
            const { container } = render(<MessageNav scrollableRef={{ current: scrollable }}/>);
            act(() => {
                jest.advanceTimersByTime(250);
            });
            const column = container.querySelector('nav > div');
        ...
        it('keeps per-instance scroll tokens isolated across mounted MessageNav instances', () => {
            ...
            const { container: navA } = render(<MessageNav scrollableRef={{ current: domA.scrollable }}/>);
            act(() => {
                jest.advanceTimersByTime(250);
            });
            mockUseGetMessagesByConvoId.mockReturnValue({ data: messagesB });
            const domB = buildDom(messagesB);
            const { container: navB } = render(<MessageNav scrollableRef={{ current: domB.scrollable }}/>);
            act(() => {
                jest.advanceTimersByTime(250);
            });
            const steps = [];
            ...
            const indA = navA.querySelectorAll('[data-msg-id]')[2];
            const indB = navB.querySelectorAll('[data-msg-id]')[2];
            act(() => {
    ...
    describe('focus management', () => {
        it('moves focus to the target message when an indicator is clicked', () => {
            ...
            const { container } = renderNav(messages);
            const indicator = container.querySelectorAll('[data-msg-id]')[1];
            act(() => {
    ...
    describe('drag to scroll', () => {
        function setupDraggableNav() {
            const messages = Array.from({ length: 5 }, (_, i) => buildMessage({
                messageId: `m-${i}`,
                text: `message ${i}`,
                isCreatedByUser: i % 2 === 0,
            }));
            const result = renderNav(messages);
            const column = result.container.querySelector('nav > div');
            column.getBoundingClientRect = () => ({ top: 0, bottom: 50, height: 50 });
            const ribs = Array.from(column.querySelectorAll('[data-msg-id]'));
            const writes = [];
    ...
    describe('observers', () => {
        ...
        it('unobserves messages that are removed', async () => {
            ...
            expect(scrollable.contains(removed)).toBe(false);
        });
    ...
    describe('dom-driven refresh', () => {
        it('refreshes entries when a .message-render id attribute changes (SSE lifecycle)', async () => {
            ...
            const streamingNode = document.getElementById('client-uuid');
            await act(async () => {
