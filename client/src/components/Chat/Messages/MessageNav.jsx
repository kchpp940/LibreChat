...
function buildFallbackEntry(node, id) {
...
function getMessageEntries(root, messagesById) {
    const nodes = root.querySelectorAll('.message-render');
    const entries = [];
...
function computeTargetScroll(container, el, scrollMargin) {
...
const indicatorButtonClasses = cn('flex h-[5px] items-center justify-center rounded-sm', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-xheavy');
const MessageIndicator = memo(function MessageIndicator({ entry, isActive, isCurrent, label, onSelect, }) {
    return (<HoverCard openDelay={150}>
      <HoverCardTrigger asChild>
        <button type="button" onClick={() => onSelect(entry.id)} className={cn(indicatorButtonClasses, entry.isUser ? 'w-4' : 'w-6')} aria-label={label} aria-current={isCurrent ? 'true' : undefined} data-msg-id={entry.id}>
          <span className={cn('block w-full rounded-full transition-all duration-200', isActive
...
const chevronButtonClasses = cn('rounded-md p-0.5 text-text-tertiary transition-colors', 'group-hover/nav:text-text-secondary group-focus-within/nav:text-text-secondary', 'group-hover/nav:hover:text-text-primary', 'group-hover/nav:disabled:opacity-30 group-focus-within/nav:disabled:opacity-30', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border-xheavy');
function MessageNav({ scrollableRef }) {
    ...
    const getCurrentVisibleId = useCallback(() => {
        let nextId = null;
    ...
    const scrollToStart = useCallback((id) => {
    ...
    const scrollToImmediate = useCallback((id) => {
        const el = document.getElementById(id);
        if (!el) {
            return;
        }
    ...
    const focusMessage = useCallback((id) => {
        const el = document.getElementById(id);
        if (!el) {
            return;
        }
    ...
    const focusNav = useCallback(() => {
        ...
        const target = nav.querySelector('[aria-current="true"]') ??
            nav.querySelector('[data-msg-id]');
        if (!target) {
            return false;
        }
        target.focus();
    ...
    const scrubTo = useCallback((clientY) => {
        const col = columnRef.current;
        if (!col) {
            return;
        }
        const ribs = col.querySelectorAll('[data-msg-id]');
    ...
    useEffect(() => {
        ...
        const offsetsTop = new Array(entries.length);
        const offsetsBottom = new Array(entries.length);
        ...
        const tick = () => {
            ...
            const firstInd = col.children[first];
            const lastInd = col.children[last];
            if (!firstInd || !lastInd) {
                return;
            }
    ...
    const jumpToPrevious = useCallback(() => {
        ...
        const scrollTop = container.scrollTop;
        const scrollMargin = scrollMarginRef.current !== 0
    ...
    return (<nav ref={navRef} aria-label={localize('com_ui_message_nav')} aria-keyshortcuts="Shift+Alt+M" className={cn('group/nav absolute right-2 top-1/2 z-40 hidden max-h-[min(24rem,calc(100%-2rem))]', '-translate-y-1/2 flex-col items-center gap-1.5 rounded-full px-1 py-2 md:flex', 'opacity-30 transition-opacity duration-300', 'hover:bg-black/5 hover:opacity-100 dark:hover:bg-white/5', 'focus-within:bg-black/5 focus-within:opacity-100 dark:focus-within:bg-white/5')}>
      <button type="button" onClick={jumpToPrevious} disabled={!canGoUp} className={chevronButtonClasses} aria-label={localize('com_ui_message_nav_previous')}>
        <ChevronUp className="h-4 w-4"/>
      </button>

      <div ref={columnRef} onPointerDown={handlePointerDown} className="flex min-h-0 cursor-grab touch-none select-none flex-col items-center gap-1.5 overflow-y-auto active:cursor-grabbing [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        {entries.map((entry) => (<MessageIndicator key={entry.id} entry={entry} isActive={visibleIds.has(entry.id)} isCurrent={currentId === entry.id} onSelect={handleSelect} label={localize(entry.isUser ? 'com_ui_message_nav_go_to_user' : 'com_ui_message_nav_go_to_assistant', { 0: entry.preview.slice(0, 30) })}/>))}
      </div>

      <button type="button" onClick={jumpToNext} disabled={!canGoDown} className={chevronButtonClasses} aria-label={localize('com_ui_message_nav_next')}>
