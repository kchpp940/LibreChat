...
const chevronDown = (<svg width="1em" height="1em" viewBox="0 0 20 20" fill="currentColor">
...
const PopoverButton = ({ children, setting, onClick, setActiveSetting, timeoutRef, hoverInfo, hoverTitle, hoverDescription, label, }) => {
    const localize = useLocalize();
    return (<Ariakit.HovercardProvider placement="right-start">
      <div className="flex flex-col items-center">
        <Ariakit.HovercardAnchor render={<Ariakit.Button onClick={() => onClick(setting)} onMouseEnter={() => {
            ...
            }} className="mx-0.5 w-14 flex-1 rounded-xl border-2 border-border-medium bg-surface-secondary text-text-secondary transition duration-200 ease-in-out hover:bg-surface-hover hover:text-text-primary" aria-label={label}>
              {children}
        ...
        {((hoverInfo != null && hoverInfo !== '') ||
            ...
            (hoverDescription != null && hoverDescription !== '')) && (<Ariakit.Hovercard gutter={16} shift={40} flip={false} className="z-[999] w-80 rounded-2xl border border-border-medium bg-surface-secondary p-4 text-text-primary shadow-md" portal={true} unmountOnHide={true}>
...
const CheckboxOption = ({ id, checked, onToggle, labelKey, infoKey, showToastOnCheck = false, }) => {
    const localize = useLocalize();
    const { showToast } = useToastContext();
    return (<Ariakit.HovercardProvider placement="right-start">
      <div className="flex items-center justify-between">
        <Ariakit.HovercardAnchor render={<div className="flex items-center">
              <Ariakit.Checkbox id={id} checked={checked} onChange={(e) => {
              ...
              <label htmlFor={id} className="ml-2 cursor-pointer select-none text-sm text-text-secondary hover:text-text-primary">
                {localize(labelKey)}
              </label>
            </div>}/>
      ...
      <Ariakit.Hovercard gutter={14} shift={40} flip={false} className="z-[999] w-80 rounded-2xl border border-border-medium bg-surface-secondary p-4 text-text-primary shadow-md" portal={true} unmountOnHide={true}>
        <div className="space-y-2">
...
export default function Fork({ messageId, conversationId: _convoId, forkingSupported = false, latestMessageId, isLast = false, }) {
    const localize = useLocalize();
    const { showToast } = useToastContext();
    ...
    return (<>
      <Ariakit.PopoverAnchor store={popoverStore} render={<button className={buttonStyle} onClick={(e) => {
                if (rememberGlobal) {
                    e.preventDefault();
                    forkConvo.mutate({
                        messageId,
            ...
            }} type="button" aria-label={localize('com_ui_fork_open_menu')}>
            <GitFork size="19" aria-hidden="true"/>
          </button>}/>
      <Ariakit.Popover store={popoverStore} gutter={10} className={`popover-animate ${isActive ? 'open' : ''} flex w-60 flex-col gap-3 overflow-hidden rounded-2xl border border-border-medium bg-surface-secondary p-2 px-4 shadow-lg`} style={{
            outline: 'none',
            pointerEvents: 'auto',
            zIndex: 50,
        ...
        <div className="flex h-8 w-full items-center justify-center text-sm text-text-primary">
          ...
          <Ariakit.HovercardProvider placement="right-start">
            <div className="ml-auto flex h-6 w-6 items-center justify-center gap-1">
              <Ariakit.HovercardAnchor render={<button className="flex h-5 w-5 cursor-help items-center rounded-full text-text-secondary" aria-label={localize('com_ui_fork_info_button_label')}>
            ...
            <Ariakit.Hovercard gutter={19} shift={40} flip={false} className="z-[999] w-80 rounded-2xl border border-border-medium bg-surface-secondary p-4 text-text-primary shadow-md" portal={true} unmountOnHide={true}>
        ...
        <div className="flex h-full w-full items-center justify-center gap-1">
          {forkOptionsConfig.map((opt) => (<PopoverButton key={opt.setting} setActiveSetting={setActiveSetting} timeoutRef={timeoutRef} onClick={onClick} setting={opt.setting} label={opt.label} hoverTitle={opt.hoverTitle} hoverDescription={opt.hoverDescription}>
              {opt.icon}
            </PopoverButton>))}
        </div>
        <CheckboxOption id="split-target-checkbox" checked={splitAtTarget} onToggle={setSplitAtTarget} labelKey="com_ui_fork_split_target" infoKey="com_ui_fork_info_start"/>
        <CheckboxOption id="remember-checkbox" checked={remember} onToggle={(checked) => {
            if (checked)
                showToast({ message: localize('com_ui_fork_remember_checked'), status: 'info' });
            setRemember(checked);
        }} labelKey="com_ui_fork_remember" infoKey="com_ui_fork_info_remember" showToastOnCheck/>
      </Ariakit.Popover>
    </>);
}
