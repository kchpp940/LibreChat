...
import { Input, Label, OGDialog, OGDialogTitle, SelectDropDown, OGDialogContent, } from '@librechat/client';
import { cn, defaultTextProps, removeFocusOutlines, mapEndpoints, getConvoSwitchLogic, } from '~/utils';
...
const EditPresetDialog = ({ exportPreset, submitPreset, triggerRef, }) => {
    ...
    useEffect(() => {
        ...
        const models = modelsConfig[presetEndpoint];
        if (!models) {
            return;
        }
        if (!models.length) {
    ...
    const switchEndpoint = useCallback((newEndpoint) => {
        ...
        const { newEndpointType } = getConvoSwitchLogic({
            ...
            modularChat: true,
            conversation: null,
            endpointsConfig: queryClient.getQueryData([QueryKeys.endpoints]) ?? {},
        });
        setOptions({
    ...
    return (<OGDialog open={presetModalVisible} onOpenChange={handleOpenChange} triggerRef={triggerRef}>
        ...
        <div className="flex w-full flex-col gap-2 px-1 pb-4 md:gap-4">
          ...
          <div className="grid w-full gap-2 md:grid-cols-2 md:gap-4">
            <div className="flex w-full flex-col">
              ...
              <Input id="preset-name" value={title ?? ''} onChange={onTitleChange} placeholder={localize('com_endpoint_set_custom_name')} className={cn(defaultTextProps, 'flex h-10 max-h-10 w-full resize-none px-3 py-2', removeFocusOutlines)}/>
            </div>
            <div className="flex w-full flex-col">
              <Label htmlFor="endpoint" className="mb-1 text-left text-sm font-medium">
                {localize('com_endpoint')}
              </Label>
              <SelectDropDown value={endpoint || ''} setValue={switchEndpoint} showLabel={false} emptyTitle={true} searchPlaceholder={localize('com_endpoint_search')} availableValues={availableEndpoints}/>
            </div>
          </div>

          {/* PopoverButtons section */}
          <div className="flex w-full">
            <PopoverButtons buttonClass="ml-0 w-full border border-border-medium p-2 h-[40px] justify-center mt-0" iconClass="hidden lg:block w-4" endpoint={endpoint} endpointType={endpointType} model={model}/>
          </div>
          ...
          <div className="w-full flex-1">
            <EndpointSettings conversation={preset} setOption={setOption} isPreset={true} className="text-text-primary"/>
          </div>

          {/* Action buttons */}
          <div className="flex justify-end gap-2 border-t border-border-medium pt-2 md:pt-4">
            <button onClick={exportPreset} className="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 md:px-4">
              {localize('com_endpoint_export')}
            </button>
            <button onClick={submitPreset} className="rounded-md bg-green-500 px-3 py-2 text-sm font-medium text-white hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 md:px-4">
