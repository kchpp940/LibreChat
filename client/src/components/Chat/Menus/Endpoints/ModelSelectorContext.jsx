...
import { useAgentDefaultPermissionLevel, useSelectorEffects, useKeyDialog, useEndpoints, useLocalize, } from '~/hooks';
...
const ModelSelectorContext = createContext(undefined);
...
export function ModelSelectorProvider({ children, startupConfig }) {
    const agentsMap = useAgentsMapContext();
    const assistantsMap = useAssistantsMapContext();
    const { data: endpointsConfig } = useGetEndpointsQuery();
    const { endpoint, model, spec, agent_id, assistant_id, getConversation, newConversation } = useModelSelectorChatContext();
    ...
    const getModelDisplayName = useCallback((endpoint, model) => {
    ...
    const setDebouncedSearchValue = useMemo(() => debounce((value) => {
        setSearchValueState(value);
    }, 200), []);
    const setEndpointSearchValue = useCallback((endpoint, value) => {
        setEndpointSearchValues((prev) => ({
            ...prev,
            [endpoint]: value,
        }));
    }, []);
    const handleSelectSpec = useCallback((spec) => {
    ...
    const handleSelectModel = useCallback((endpoint, model) => {
        if (isAgentsEndpoint(endpoint.value)) {
            onSelectEndpoint?.(endpoint.value, {
                agent_id: model,
