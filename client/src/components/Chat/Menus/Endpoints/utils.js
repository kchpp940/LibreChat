...
export function filterItems(items, searchValue, agentsMap, assistantsMap, localize) {
    ...
    return items.filter((item) => {
        ...
        const itemMatches = item.label.toLowerCase().includes(searchTermLower) ||
            ...
            (item.showMarketplace === true &&
                localize != null &&
                [localize('com_agents_marketplace'), localize('com_ui_marketplace')].some((label) => label.toLowerCase().includes(searchTermLower)));
        if (itemMatches) {
            return true;
        }
...
export function filterModels(endpoint, models, searchValue, agentsMap, assistantsMap) {
    const searchTermLower = searchValue.trim().toLowerCase();
    if (!searchTermLower) {
        return models;
    }
    return models.filter((modelId) => {
        ...
        if (isAgentsEndpoint(endpoint.value) && agentsMap && agentsMap[modelId]) {
            ...
            modelName =
                typeof assistant.name === 'string' && assistant.name ? assistant.name : modelId;
        }
        return modelName.toLowerCase().includes(searchTermLower);
    });
}
export function getSelectedIcon({ mappedEndpoints, selectedValues, modelSpecs, endpointsConfig, }) {
    ...
    if (endpoint && model) {
        ...
        if (selectedEndpoint.modelIcons?.[model]) {
            ...
            return React.createElement('div', { className: 'h-5 w-5 overflow-hidden rounded-full' }, React.createElement('img', {
                src: iconUrl,
                alt: model,
                className: 'h-full w-full object-cover',
            }));
        }
        return (selectedEndpoint.icon ||
...
export const getDisplayValue = ({ localize, mappedEndpoints, selectedValues, modelSpecs, agentsMap, }) => {
