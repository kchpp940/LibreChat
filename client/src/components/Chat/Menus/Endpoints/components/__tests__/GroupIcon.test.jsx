...
jest.mock('~/hooks/Endpoint/Icons', () => {
    ...
    const createIcon = (iconKey) => ({ className, endpoint }) => React.createElement('span', {
        className,
...
describe('GroupIcon', () => {
    ...
    it('resolves known endpoint asset aliases case-insensitively', () => {
        ...
        expect(screen.getByRole('img', { name: 'OpenRouter' })).toHaveAttribute('src', 'assets/openrouter.png');
    });
    it('resolves known endpoint asset aliases to shipped file paths', () => {
        render(<GroupIcon iconURL="Helicone" groupName="Helicone"/>);
        expect(screen.getByRole('img', { name: 'Helicone' })).toHaveAttribute('src', 'assets/helicone.svg');
    });
