...
describe('SpecDescription', () => {
    ...
    it('renders HTML descriptions with inline images', () => {
        const { container } = render(<SpecDescription description='<span>Powered by <img src="/assets/claude.png" alt="Claude" /> Claude</span>'/>);
    ...
    it('strips scripts, event handlers, and unsafe URLs from HTML descriptions', () => {
        const { container } = render(<SpecDescription description='<span onclick="alert(1)">Safe<script>alert(1)</script><img src="javascript:alert(1)" onerror="alert(1)"></span>'/>);
