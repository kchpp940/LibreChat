 ...
 */
function PendingManualSkillsChips({ conversationId }) {
    ...
    return (<div className="flex flex-wrap gap-1.5 px-2 pt-2" role="list" aria-label={localize('com_ui_skills_queued')}>
      {skills.map((name) => (<span key={name} role="listitem" className="inline-flex items-center gap-1 rounded-full border border-border-light bg-surface-secondary px-2 py-0.5 text-xs text-text-secondary">
          <ScrollText className="h-3 w-3 text-cyan-500" aria-hidden="true"/>
          <span className="max-w-[12rem] truncate">{name}</span>
          <button type="button" aria-label={localize('com_ui_remove_skill_var', { 0: name })} onClick={() => remove(name)} className="-mr-0.5 ml-0.5 rounded-full p-0.5 text-text-secondary hover:bg-surface-tertiary hover:text-text-primary">
