 ...
 */
export function filterSkillsForPopover(skills, ctx) {
    const { agentSkillIds, isActive } = ctx;
    if (agentSkillIds != null && agentSkillIds.length === 0) {
        return [];
    }
    const agentSet = agentSkillIds != null && agentSkillIds.length > 0 ? new Set(agentSkillIds) : null;
    const result = [];
...
function SkillsCommandContent({ index, textAreaRef, conversationId, agentId, }) {
    const localize = useLocalize();
    const setShowSkillsPopover = useSetRecoilState(store.showSkillsPopoverFamily(index));
    const setEphemeralAgent = useSetRecoilState(ephemeralAgentByConvoId(conversationId));
    const setPendingManualSkills = useSetRecoilState(store.pendingManualSkillsByConvoId(conversationId));
    ...
    const { data, isLoading, isError, fetchNextPage, hasNextPage, isFetchingNextPage } = useSkillsInfiniteQuery({ limit: 50 });
    ...
    const handleSelect = useCallback((mention) => {
        if (!mention) {
            return;
        }
        setSearchValue('');
        ...
        setPendingManualSkills((prev) => prev.includes(mention.value) ? prev : [...prev, mention.value]);
    ...
    const rowRenderer = ({ index, key, style, }) => {
        ...
        return (<MentionItem index={index} type="skill" key={key} style={style} onClick={() => {
                if (timeoutRef.current) {
                    clearTimeout(timeoutRef.current);
                }
    ...
    return (<div className="absolute bottom-28 z-10 w-full space-y-2">
      <div className="popover border-token-border-light rounded-2xl border bg-surface-tertiary-alt p-2 shadow-lg">
        <input ref={initInputRef} placeholder={localize('com_ui_skills_command_placeholder')} className="mb-1 w-full border-0 bg-surface-tertiary-alt p-2 text-sm focus:outline-none dark:text-gray-200" autoComplete="off" value={searchValue} onKeyDown={(e) => {
            if (e.key === 'Escape') {
                setOpen(false);
                setShowSkillsPopover(false);
                textAreaRef.current?.focus();
        ...
        {open && (isLoading || isFetchingNextPage) && matches.length === 0 && (<div className="flex h-32 items-center justify-center text-text-primary">
            <Spinner />
          </div>)}
        {open && isError && (<div className="p-4 text-center text-sm text-text-secondary">
            {localize('com_ui_skills_load_error')}
          </div>)}
        {open && !isLoading && !isFetchingNextPage && !isError && matches.length === 0 && (<div className="p-4 text-center text-sm text-text-secondary">
...
const SkillsCommand = memo(function SkillsCommand({ index, textAreaRef, conversationId, agentId, }) {
    const show = useRecoilValue(store.showSkillsPopoverFamily(index));
    if (!show) {
        return null;
    }
    return (<SkillsCommandContent index={index} textAreaRef={textAreaRef} conversationId={conversationId} agentId={agentId}/>);
});
