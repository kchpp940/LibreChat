import React, { memo, useRef, useMemo, useState, useEffect, forwardRef, useReducer, useCallback, } from 'react';
...
const BadgeWrapper = React.memo(forwardRef(({ badge, isEditing, isInChat, onToggle, onDelete, onMouseDown, badgeRefs }, ref) => {
    ...
    return (<div ref={(el) => {
        ...
        }} onMouseDown={(e) => onMouseDown(e, badge, isActive)} className={isEditing ? 'ios-wiggle badge-icon h-full' : 'badge-icon h-full'}>
          <Badge id={badge.id} icon={badge.icon} label={badge.label} isActive={isActive} isEditing={isEditing} isAvailable={badge.isAvailable} isInChat={isInChat} onToggle={() => onToggle(badge)} onBadgeAction={() => onDelete(badge.id)}/>
        </div>);
}), (prevProps, nextProps) => prevProps.badge.id === nextProps.badge.id &&
...
function BadgeRow({ showEphemeralBadges, conversationId, specName, isSubmitting, onChange, onToggle, isInChat, }) {
    const [orderedBadges, setOrderedBadges] = useState([]);
    ...
    const badges = useMemo(() => allBadges.filter((badge) => badge.isAvailable !== false), [allBadges]);
    const toggleBadge = useRecoilCallback(({ snapshot, set }) => async (badgeAtom) => {
    ...
    const calculateInsertIndex = useCallback((currentMouseX) => {
    ...
    const handleMouseDown = useCallback((e, badge, isActive) => {
    ...
    const handleDelete = useCallback((badgeId) => {
        const newBadges = orderedBadges.filter((b) => b.id !== badgeId);
        setOrderedBadges(newBadges);
        onChange(newBadges.map((badge) => ({ id: badge.id })));
    }, [orderedBadges, onChange]);
    const handleBadgeToggle = useCallback((badge) => {
        if (badge.atom) {
    ...
    return (<BadgeRowProvider conversationId={conversationId} specName={specName} isSubmitting={isSubmitting}>
      <div ref={containerRef} className="relative flex flex-wrap items-center gap-2">
        {showEphemeralBadges === true && <ToolsDropdown />}
        {tempBadges.map((badge, index) => (<React.Fragment key={badge.id}>
            {dragState.draggedBadge && dragState.insertIndex === index && ghostBadge && (<div className="badge-icon h-full">
                <Badge id={ghostBadge.id} icon={ghostBadge.icon} label={ghostBadge.label} isActive={dragState.draggedBadgeActive} isEditing={isEditing} isAvailable={ghostBadge.isAvailable} isInChat={isInChat}/>
              </div>)}
            <BadgeWrapper badge={badge} isEditing={isEditing} isInChat={isInChat} onToggle={handleBadgeToggle} onDelete={handleDelete} onMouseDown={handleMouseDown} badgeRefs={badgeRefs}/>
          </React.Fragment>))}
        {dragState.draggedBadge && dragState.insertIndex === tempBadges.length && ghostBadge && (<div className="badge-icon h-full">
            <Badge id={ghostBadge.id} icon={ghostBadge.icon} label={ghostBadge.label} isActive={dragState.draggedBadgeActive} isEditing={isEditing} isAvailable={ghostBadge.isAvailable} isInChat={isInChat}/>
          </div>)}
        ...
        {ghostBadge && (<div className="ghost-badge h-full" style={{
                position: 'absolute',
                top: 0,
                left: 0,
            ...
            <Badge id={ghostBadge.id} icon={ghostBadge.icon} label={ghostBadge.label} isActive={dragState.draggedBadgeActive} isAvailable={ghostBadge.isAvailable} isInChat={isInChat} isEditing isDragging/>
          </div>)}
      </div>
      <ToolDialogs />
    </BadgeRowProvider>);
