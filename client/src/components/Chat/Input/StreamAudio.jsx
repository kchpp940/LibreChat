...
function timeoutPromise(ms, message) {
    return new Promise((_, reject) => setTimeout(() => reject(new Error(message ?? 'Promise timed out')), ms));
}
...
export default function StreamAudio({ index = 0 }) {
    ...
    const getMessages = useCallback(() => queryClient.getQueryData([QueryKeys.messages, queryParam]), [queryParam, queryClient]);
    useEffect(() => {
        ...
        async function fetchAudio() {
            ...
            try {
                ...
                if (chunks.length) {
                    ...
                    const targetMessage = latestMessages.find((msg) => msg.messageId === latestMessage?.messageId);
    ...
    <audio ref={audioRef} controls controlsList="nodownload nofullscreen noremoteplayback" style={{
            position: 'absolute',
            overflow: 'hidden',
            display: 'none',
            height: '0px',
            width: '0px',
        }} src={globalAudioURL ?? undefined} id={globalAudioId} autoPlay/>);
}
