...
const ToolsDropdown = ({ disabled }) => {
    ...
    const { skills, webSearch, artifacts, fileSearch, mcpServerManager, codeInterpreter, searchApiKeyForm, } = context ?? {};
    const { setIsDialogOpen: setIsSearchDialogOpen, menuTriggerRef: searchMenuTriggerRef } = searchApiKeyForm ?? {};
    const { isPinned: isSearchPinned, setIsPinned: setIsSearchPinned, authData: webSearchAuthData, } = webSearch ?? {};
    ...
    if (fileSearchEnabled && canUseFileSearch) {
        dropdownItems.push({
          ...
          <button type="button" onClick={(e) => {
                ...
                }} className={cn('rounded p-1 transition-all duration-200', 'hover:bg-surface-secondary hover:shadow-sm', !isFileSearchPinned && 'text-text-secondary hover:text-text-primary')} aria-label={isFileSearchPinned ? 'Unpin' : 'Pin'}>
    ...
    if (canUseWebSearch && webSearchEnabled) {
        dropdownItems.push({
          ...
          <div className="flex items-center gap-1">
            {showWebSearchSettings && (<button type="button" onClick={(e) => {
                        e.stopPropagation();
                        setIsSearchDialogOpen?.(true);
                    }} className={cn('rounded p-1 transition-all duration-200', 'hover:bg-surface-secondary hover:shadow-sm', 'text-text-secondary hover:text-text-primary')} aria-label="Configure web search" ref={searchMenuTriggerRef}>
                <div className="h-4 w-4">
            ...
            <button type="button" onClick={(e) => {
                ...
                }} className={cn('rounded p-1 transition-all duration-200', 'hover:bg-surface-secondary hover:shadow-sm', !isSearchPinned && 'text-text-secondary hover:text-text-primary')} aria-label={isSearchPinned ? 'Unpin' : 'Pin'}>
              <div className="h-4 w-4">
    ...
    if (canUseSkills && skillsEnabled) {
        dropdownItems.push({
            ...
            hideOnClick: false,
            render: (props) => (<div {...props} data-testid="tools-menu-skills">
          <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={(e) => {
                ...
                }} className={cn('rounded p-1 transition-all duration-200', 'hover:bg-surface-secondary hover:shadow-sm', !isSkillsPinned && 'text-text-secondary hover:text-text-primary')} aria-label={isSkillsPinned ? localize('com_ui_unpin') : localize('com_ui_pin')}>
            <div className="h-4 w-4">
        ...
        dropdownItems.push({
          ...
          <div className="flex items-center gap-1">
            <button type="button" onClick={(e) => {
                ...
                }} className={cn('rounded p-1 transition-all duration-200', 'hover:bg-surface-secondary hover:shadow-sm', !isCodePinned && 'text-text-primary hover:text-text-primary')} aria-label={isCodePinned ? 'Unpin' : 'Pin'}>
              <div className="h-4 w-4">
    ...
    if (artifactsEnabled && setIsArtifactsPinned != null) {
        dropdownItems.push({
            hideOnClick: false,
            render: (props) => (<ArtifactsSubMenu {...props} isArtifactsPinned={isArtifactsPinned ?? false} setIsArtifactsPinned={setIsArtifactsPinned} artifactsMode={artifacts?.toggleState} handleArtifactsToggle={handleArtifactsToggle} handleShadcnToggle={handleShadcnToggle} handleCustomToggle={handleCustomToggle}/>),
        });
    }
    ...
    const menuTrigger = (<TooltipAnchor render={<Ariakit.MenuButton disabled={isDisabled} id="tools-dropdown-button" aria-label="Tools Options" className={cn('flex size-9 items-center justify-center rounded-full p-1 hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-opacity-50', isPopoverActive && 'bg-surface-hover')}>
          <div className="flex w-full items-center justify-center gap-2">
            <Settings2 className="size-5" aria-hidden="true"/>
          </div>
        </Ariakit.MenuButton>} id="tools-dropdown-button" description={localize('com_ui_tools')} disabled={isDisabled}/>);
    return (<DropdownPopup itemClassName="flex w-full cursor-pointer rounded-lg items-center justify-between hover:bg-surface-hover gap-5" menuId="tools-dropdown-menu" isOpen={isPopoverActive} setIsOpen={setIsPopoverActive} modal={true} unmountOnHide={true} trigger={menuTrigger} items={dropdownItems} iconClassName="mr-0"/>);
};
