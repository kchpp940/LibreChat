...
jest.mock('../Image', () => {
    return function MockImage({ url, progress, source }) {
...
jest.mock('../FileContainer', () => {
    return function MockFileContainer({ file }) {
        return (<div data-testid="mock-file-container">
...
describe('FileRow', () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        mockUseLocalize.mockReturnValue((key) => {
            const translations = {
    ...
    const renderFileRow = (files) => {
        return render(<FileRow files={files} setFiles={mockSetFiles} setFilesLoading={mockSetFilesLoading}/>);
    };
    ...
    describe('Empty State', () => {
        ...
        it('should render nothing when files is undefined', () => {
            const { container } = render(<FileRow files={undefined} setFiles={mockSetFiles} setFilesLoading={mockSetFilesLoading}/>);
            expect(container.firstChild).toBeNull();
        });
    });
    describe('Preview Cache Integration', () => {
        it('should prefer preview blob URL over filepath for zero-flicker rendering', () => {
            const file = createMockFile({
                ...
                filepath: '/images/68c98b26901ebe2d87c193a2/c0fe1b93-ba3d-456c-80be-9a492bfd9ed0__image.png',
                progress: 1,
            });
            const filesMap = new Map();
            filesMap.set(file.file_id, file);
        ...
        it('should fall back to filepath when no preview exists', () => {
            ...
            renderFileRow(filesMap);
            const imageUrl = screen.getByTestId('image-url').textContent;
            expect(imageUrl).toBe('/images/68c98b26901ebe2d87c193a2/c0fe1b93-ba3d-456c-80be-9a492bfd9ed0__image.png');
        });
    });
});
