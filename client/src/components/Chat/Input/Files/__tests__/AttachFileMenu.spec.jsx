...
jest.mock('@librechat/client', () => {
    ...
    return {
        FileUpload: R.forwardRef((props, ref) => R.createElement('div', { 'data-testid': 'file-upload' }, props.children, R.createElement('input', {
            ref,
            multiple: true,
            type: 'file',
        ...
        DropdownPopup: (props) => R.createElement('div', null, R.createElement('div', { onClick: () => props.setIsOpen(!props.isOpen) }, props.trigger), props.isOpen &&
            R.createElement('div', { 'data-testid': 'dropdown-menu' }, props.items.map((item, idx) => R.createElement('button', { key: idx, onClick: item.onClick, 'data-testid': `menu-item-${idx}` }, item.label)))),
...
const mockUseSharePointFileHandlingNoChatContext = jest.requireMock('~/hooks/Files/useSharePointFileHandling').useSharePointFileHandlingNoChatContext;
...
function setupMocks(overrides = {}) {
    ...
    mockUseLocalize.mockReturnValue((key) => translations[key] || key);
...
function renderMenu(props = {}) {
    return render(<QueryClientProvider client={queryClient}>
      <RecoilRoot>
        <AttachFileMenu conversationId="test-convo" files={new Map()} setFiles={() => { }} setFilesLoading={() => { }} conversation={null} {...props}/>
      </RecoilRoot>
    </QueryClientProvider>);
}
function openMenu() {
...
describe('AttachFileMenu', () => {
    ...
    describe('Agent Capabilities', () => {
        ...
        it('passes File Search resource when the file input changes before React state commits', () => {
            ...
            expect(handleFileChange).toHaveBeenNthCalledWith(2, expect.any(Object), EToolResources.file_search);
        });
    });
