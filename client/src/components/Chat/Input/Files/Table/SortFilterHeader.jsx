...
export function SortFilterHeader({ column, title, className = '', filters, valueMap, ariaLabel, }) {
        ...
        if (filters) {
            ...
            Object.entries(filters).forEach(([_key, values]) => {
                values.forEach((value) => {
                    ...
                    const filterValue = translationKey != null && translationKey.length
    ...
    return (<div className={cn('flex items-center space-x-2', className)}>
      <DropdownPopup portal={false} isOpen={isOpen} setIsOpen={setIsOpen} trigger={<TooltipAnchor description={ariaLabel || title} side="top" render={<Menu.MenuButton aria-sort={ariaSort} aria-label={ariaLabel} aria-pressed={column.getIsFiltered() ? 'true' : 'false'} aria-current={sortState ? 'true' : 'false'} className={cn('inline-flex items-center gap-2 rounded-lg px-2 py-0 text-xs transition-colors hover:bg-surface-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-[open]:bg-surface-hover sm:px-2 sm:py-2 sm:text-sm', column.getIsFiltered() && 'border-b-2 border-b-border-xheavy')}>
                <span>{title}</span>
                {column.getIsFiltered() ? (<ListFilter className="icon-sm" aria-hidden="true"/>) : (<ListFilter className="icon-sm text-text-secondary" aria-hidden="true"/>)}
                {(() => {
                    const sortState = column.getIsSorted();
                    if (sortState === 'desc') {
              ...
              </Menu.MenuButton>}/>} items={dropdownItems} menuId={menuId} className="z-[1001]"/>
    </div>);
}
