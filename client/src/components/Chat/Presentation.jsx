...
export default function Presentation({ children }) {
    ...
    useEffect(() => {
        ...
        const map = JSON.parse(filesToDelete ?? '{}');
        const files = Object.values(map)
            .filter((file) => file.filepath != null && file.source && !(file.embedded ?? false) && file.temp_file_id)
            .map((file) => ({
    ...
    return (<DragDropWrapper className="relative flex w-full grow overflow-hidden bg-presentation">
