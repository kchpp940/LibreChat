...
function PluginAuthForm({ plugin, onSubmit, isEntityTool }) {
    ...
    return (<div className="flex w-full flex-col items-center gap-2">
      <div className="grid w-full gap-6 sm:grid-cols-2">
        <form className="col-span-1 flex w-full flex-col items-start justify-start gap-2" method="POST" onSubmit={handleSubmit((auth) => onSubmit({
            pluginKey: plugin?.pluginKey ?? '',
            action: 'install',
            auth,
            isEntityTool,
        }))}>
          {authConfig.map((config, i) => {
            ...
            return (<div key={`${authField}-${i}`} className="flex w-full flex-col gap-1">
                <label htmlFor={authField} className="mb-1 text-left text-sm font-medium text-gray-700/70 dark:text-gray-50/70">
                  {config.label}
                </label>
                <HoverCard openDelay={300}>
                  <HoverCardTrigger className="grid w-full items-center gap-2">
                    {config.sensitive === false ? (<input type="text" autoComplete="off" {...sharedProps}/>) : (<SecretInput autoComplete="new-password" data-lpignore="true" data-1p-ignore="true" controlsOnHover {...sharedProps}/>)}
                  </HoverCardTrigger>
                  <PluginTooltip content={config.description} position="right"/>
                </HoverCard>
                {errors[authField] && (<span role="alert" className="mt-1 text-sm text-red-400">
                    {String(errors?.[authField]?.message ?? '')}
                  </span>)}
              </div>);
        })}
          <button disabled={allFieldsOptional ? isSubmitting : !isDirty || !isValid || isSubmitting} type="button" className="btn btn-primary relative" onClick={() => {
            handleSubmit((auth) => onSubmit({
                pluginKey: plugin?.pluginKey ?? '',
                action: 'install',
                auth,
                isEntityTool,
