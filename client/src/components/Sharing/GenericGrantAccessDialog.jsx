...
import { Label, Button, Spinner, Skeleton, OGDialog, OGDialogTitle, OGDialogClose, OGDialogContent, OGDialogTrigger, useToastContext, } from '@librechat/client';
import { usePeoplePickerPermissions, useResourcePermissionState, useCopyToClipboard, useCanSharePublic, useLocalize, } from '~/hooks';
...
export default function GenericGrantAccessDialog({ resourceName, resourceDbId, resourceId, resourceType, onGrantAccess, disabled = false, buttonClassName, children, }) {
    ...
    const { config, permissionsData, isLoadingPermissions, permissionsError, updatePermissionsMutation, currentShares, currentIsPublic, currentPublicRole, isPublic, setIsPublic, publicRole, setPublicRole, } = useResourcePermissionState(resourceType, resourceDbId, isModalOpen);
    /** State for unified list of all shares (existing + newly added) */
    const [allShares, setAllShares] = useState([]);
    const [hasChanges, setHasChanges] = useState(false);
    const [defaultPermissionId, setDefaultPermissionId] = useState(config?.defaultViewerRoleId);
    ...
    const handleAddFromSearch = (newShares) => {
        const sharesToAdd = newShares.filter((newShare) => !allShares.some((existing) => existing.idOnTheSource === newShare.idOnTheSource));
    ...
    const handleRemoveShare = (idOnTheSource) => {
        setAllShares(allShares.filter((s) => s.idOnTheSource !== idOnTheSource));
        setHasChanges(true);
    };
    // Handler for changing individual share permissions
    const handleRoleChange = (idOnTheSource, newRole) => {
        setAllShares(allShares.map((s) => s.idOnTheSource === idOnTheSource ? { ...s, accessRoleId: newRole } : s));
        setHasChanges(true);
    };
    // Handler for public access toggle
    const handlePublicToggle = (isPublicValue) => {
    ...
    const handlePublicRoleChange = (role) => {
        setPublicRole(role);
        setHasChanges(true);
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const originalSharesMap = new Map(currentShares.map((share) => [`${share.type}-${share.idOnTheSource}`, share]));
            const allSharesMap = new Map(allShares.map((share) => [`${share.type}-${share.idOnTheSource}`, share]));
    ...
    const hasAtLeastOneOwner = allShares.some((share) => share.accessRoleId === config?.defaultOwnerRoleId) ||
    ...
    const TriggerComponent = children ? (children) : (<Button size="sm" variant="outline" aria-label={localize('com_ui_share_var', {
            0: config?.getShareMessage(resourceName),
        })} type="button" disabled={disabled} className={cn('h-9', buttonClassName)}>
      <div className="flex min-w-[32px] items-center justify-center gap-2 text-blue-500">
        <span className="flex h-6 w-6 items-center justify-center">
          <Share2Icon className="icon-md h-4 w-4"/>
        </span>
        {totalCurrentShares > 0 && (<Label className="cursor-pointer text-sm font-medium text-text-secondary">
            {totalCurrentShares}
          </Label>)}
      </div>
    </Button>);
    return (<OGDialog open={isModalOpen} onOpenChange={setIsModalOpen} modal>
      ...
      <OGDialogContent className="max-h-[90vh] w-11/12 overflow-y-auto md:max-w-3xl">
        ...
        <div className="space-y-6 p-2">
          ...
          <div className="space-y-4">
            ...
            {hasPeoplePickerAccess && (<div className="space-y-2">
                ...
                <UnifiedPeopleSearch onAddPeople={handleAddFromSearch} placeholder={localize('com_ui_search_people_placeholder')} typeFilter={peoplePickerTypeFilter} excludeIds={allShares.map((s) => s.idOnTheSource)}/>
                ...
                if (allShares.length === 0 && !hasChanges) {
                    return (<div className="rounded-lg border-2 border-dashed border-border-light p-8 text-center">
                ...
                return (<div className="space-y-2">
                      {!hasAtLeastOneOwner && hasChanges && (<div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-center">
                          <div className="flex items-center justify-center gap-2 text-sm text-red-600 dark:text-red-400">
                            <UserX className="h-4 w-4" aria-hidden="true"/>
                            {localize('com_ui_at_least_one_owner_required')}
                          </div>
                        </div>)}
                      <SelectedPrincipalsList principles={allShares} onRemoveHandler={handleRemoveShare} resourceType={resourceType} onRoleChange={(id, newRole) => handleRoleChange(id, newRole)}/>
                    </div>);
            })()}
              </div>)}
          </div>
          ...
          {canSharePublic && (<>
              ...
              <PublicSharingToggle isPublic={isPublic} publicRole={publicRole} onPublicToggle={handlePublicToggle} onPublicRoleChange={handlePublicRoleChange} resourceType={resourceType}/>
            </>)}

          {/* Footer Actions */}
          <div className="flex justify-between pt-4">
            <div className="flex gap-2">
              {resourceId && resourceUrl && (<Button variant="outline" onClick={() => {
                if (isCopying)
                    return;
            ...
            }} disabled={isCopying} className={cn('shrink-0', isCopying ? 'cursor-default' : '')} aria-label={localize('com_ui_copy_url_to_clipboard')} title={isCopying
                ? config?.getCopyUrlMessage()
                : localize('com_ui_copy_url_to_clipboard')}>
                  {isCopying ? (<CopyCheck className="h-4 w-4" aria-hidden="true"/>) : (<Link className="h-4 w-4" aria-hidden="true"/>)}
                </Button>)}
            </div>
            <div className="flex gap-2">
              <PeoplePickerAdminSettings />
              <OGDialogClose asChild>
                <Button variant="outline" onClick={handleCancel} aria-label={localize('com_ui_cancel')}>
                  {localize('com_ui_cancel')}
                </Button>
              </OGDialogClose>
              <Button variant="submit" onClick={handleSave} disabled={updatePermissionsMutation.isLoading ||
            !submitButtonActive ||
            (hasChanges && !hasAtLeastOneOwner)} className="min-w-[120px]" aria-label={localize('com_ui_save_changes')}>
                {updatePermissionsMutation.isLoading ? (<div className="flex items-center gap-2">
                    <Spinner className="h-4 w-4"/>
                    {localize('com_ui_saving')}
                  </div>) : (localize('com_ui_save_changes'))}
              </Button>
            </div>
          </div>
        </div>
