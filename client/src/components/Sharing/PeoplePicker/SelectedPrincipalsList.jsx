...
export default function SelectedPrincipalsList({ principles, onRemoveHandler, className = '', onRoleChange, resourceType = ResourceType.AGENT, }) {
    ...
    return (<div className={`space-y-3 ${className}`}>
      <div className="space-y-2">
        {principles.map((share) => {
            ...
            return (<div key={share.idOnTheSource + '-principalList'} className="bg-surface flex items-center justify-between rounded-2xl border border-border p-3">
              ...
              <div className="flex flex-shrink-0 items-center gap-2">
                {lockOwner ? (<span className="px-3 py-2 text-sm font-medium text-text-secondary">
                    {localize('com_ui_role_owner')}
                  </span>) : (!!share.accessRoleId &&
                    !!onRoleChange && (<AccessRolesPicker resourceType={resourceType} selectedRoleId={share.accessRoleId} onRoleChange={(newRole) => {
                        onRoleChange?.(share.idOnTheSource, newRole);
                    }} className="min-w-0"/>))}
                {!lockOwner && (<Button variant="outline" onClick={() => onRemoveHandler(share.idOnTheSource)} className="h-9 w-9 p-0 hover:border-destructive/10 hover:bg-destructive/10 hover:text-destructive" aria-label={localize('com_ui_remove_user', { 0: displayName })}>
