...
export default function AccessRolesPicker({ id, resourceType = ResourceType.AGENT, selectedRoleId = AccessRoleIds.AGENT_VIEWER, onRoleChange, className = '', }) {
    ...
    const getLocalizedRoleInfo = (roleId) => {
    ...
    const dropdownItems = filteredRoles.map((role) => {
    ...
    return (<div className={className} id={id}>
      <DropdownPopup menuId="access-roles-menu" isOpen={isOpen} setIsOpen={setIsOpen} trigger={<Ariakit.MenuButton aria-label={selectedRoleInfo?.description || 'Select role'} className={cn('flex items-center justify-between gap-2 rounded-xl border border-border-light bg-transparent px-3 py-2 text-sm transition-colors hover:bg-surface-tertiary')}>
            <span className="font-medium">
              {selectedRoleInfo?.name || localize('com_ui_select')}
            </span>
            <ChevronDown className="h-4 w-4 text-text-secondary" aria-hidden="true"/>
          </Ariakit.MenuButton>} items={dropdownItems} className="w-[280px]"/>
    </div>);
}
