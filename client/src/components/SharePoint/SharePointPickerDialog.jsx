...
import { OGDialog, OGDialogTitle, OGDialogPortal, OGDialogOverlay, OGDialogContent, } from '@librechat/client';
import { useSharePointPicker, useLocalize } from '~/hooks';
export default function SharePointPickerDialog({ isOpen, onOpenChange, onFilesSelected, disabled = false, isDownloading = false, downloadProgress = null, maxSelectionCount, }) {
    ...
    const containerCallbackRef = React.useCallback((node) => {
    ...
    return (<OGDialog open={isOpen} onOpenChange={handleOpenChange}>
      <OGDialogPortal>
        <OGDialogOverlay className="bg-black/50"/>
        <OGDialogContent className="sharepoint-picker-bg fixed left-1/2 top-1/2 z-50 h-[680px] max-h-[90vh] max-w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-lg border bg-surface-primary p-2 shadow-lg focus:outline-none" showCloseButton={true}>
          ...
          <div ref={containerCallbackRef} className="sharepoint-picker-bg relative flex p-2">
            ...
            {isDownloading && (<div className="absolute inset-0 z-10 flex items-center justify-center rounded-lg bg-black/30 backdrop-blur-sm">
                <div className="mx-4 w-full max-w-sm rounded-lg bg-surface-primary p-6 shadow-lg">
                  <div className="text-center">
                    ...
                    {downloadProgress && (<div className="space-y-2">
                    ...
                    1: downloadProgress.total,
                        ...
                        {downloadProgress.currentFile && (<p className="truncate text-xs text-text-tertiary">
                    ...
                    0: Math.round((downloadProgress.completed / downloadProgress.total) * 100),
                })}
                        </p>
                        {downloadProgress.failed.length > 0 && (<p className="text-xs text-red-500">
                            {localize('com_files_download_failed', {
                        0: downloadProgress.failed.length,
                    })}
                          </p>)}
                      </div>)}
                    {!downloadProgress && (<p className="text-sm text-text-secondary">
