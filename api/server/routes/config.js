...
let capabilityLoaderRegistered = false;

function ensureCapabilityLoader(tenantId) {
  if (capabilityLoaderRegistered) {
    return;
  }
  ...
  capabilityLoaderRegistered = true;
}
...
function buildPostLoginPayload() {
  ...
  const payload = {
    ...
    // These capability-flavored fields on startupConfig are preserved for
    // backwards compatibility only. New code MUST read from the unified
    // `capabilities` object instead, which is the single source of truth
    // generated by CapabilityRegistry. Direct reads of these fields in
    // client code are forbidden by `check-capability-usage.mjs`.
    sharedLinksEnabled,
...
router.get('/', async function (req, res) {
  try {
    ...
    const payload = {
      ...
      // ── LEGACY / COMPAT FIELDS ──
      // Artifact / file-related capability fields preserved for backwards
      // compatibility. New code MUST read from `capabilities.artifacts`
      // and `capabilities.files` instead of these flat fields.
    ...
    if (webSearch) {
      // ── LEGACY / COMPAT FIELD ──
      // Use `capabilities.webSearch` from the unified registry instead.
    ...
    ensureCapabilityLoader(tenantId);
    payload.capabilities = await capabilityRegistry.getCapabilities();
